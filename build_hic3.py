# -*- coding: utf-8 -*-
"""Builds the HIC.3 master policy draft: JSON for review + SQL for the Supabase SQL Editor.
...
Compliance with the statutory obligations governing these services — the biomedical waste rules, food safety licensing, and pollution control authorisation — is treated as a minimum condition of operating, not as an achievement."""

PROCEDURE_STEPS = [
"""1. Engineering controls that support infection prevention — what they are and who owns them
...
A register of all infection-relevant engineering systems is maintained, identifying for each: the areas it serves, the performance it must meet, the maintenance it requires and at what frequency, the person responsible, and where its records are kept. Any change to these systems — a new area brought into service, an air handling unit replaced, a duct rerouted — is notified to the Infection Control Team before it is made, because a change made for energy or cost reasons can silently remove a control the hospital depends on.""",

"""2. Ventilation of operating theatres
...
Theatre doors are kept closed during procedures and traffic is kept to the minimum necessary, since the pressure regime and the air change rate both depend on it. A theatre that fails any parameter is taken out of use for elective work until it is corrected, and the decision is recorded.""",

"""3. Ventilation and pressure relationships in other areas
...
The pressure relationship of each critical area is verified at an interval of [Hospital to define] and the result recorded. Where a permanent monitoring device is fitted, its reading is checked and logged daily by the area in-charge; a reading outside limits is reported the same day.""",

"""4. Filter maintenance and validation of critical-area ventilation
...
Validation of critical-area ventilation — operating theatres and any other area {{HOSPITAL_NAME}} designates — is carried out at an interval of [Hospital to define] and covers at minimum the air change rate, the pressure differential, high-efficiency filter integrity, and temperature and humidity. Validation is performed by [Hospital to define] and the report is retained and tabled at the Infection Prevention and Control Committee. A failed parameter generates a recorded corrective action, a re-test, and a decision on whether the area may continue in use meanwhile.""",

"""5. Environmental surveillance of critical areas
...
The detailed specification of potable water quality and the wider water safety plan sit with the facility management and safety standards; what this policy fixes is that water testing results reach the Infection Prevention and Control Committee and that an adverse result triggers the same investigation and corrective action cycle as any other environmental exceedance.""",

"""7. Physical separation of clean and contaminated flows
...
Where the existing building cannot achieve full separation, the compensating measure — containment, timing, cleaning between uses — is written down, and the limitation is recorded as a risk in the infection control risk register rather than left as an informal practice.""",

"""8. Infection control risk assessment before construction or renovation
...
A copy of the approved assessment is held on site for the duration of the work and is available to any member of staff who asks.""",

"""9. Determining the class of precautions
...
Where the class is disputed, the more stringent classification applies. The cost of unnecessary containment is trivial next to the cost of a post-operative or post-transplant fungal infection.""",

"""10. Containment and dust control during the work
...
The barrier and containment measures are inspected daily while the work is in progress by a named person, and the inspection is recorded. A breach is corrected immediately and the work stopped meanwhile if the breach is significant.""",

"""11. Contractor control and worker movement
...
The contract with the contractor makes these obligations explicit and provides a consequence for breach. The person at {{HOSPITAL_NAME}} responsible for the contract — [Hospital to define] — is responsible for enforcing them, and repeated breaches are escalated to the Infection Prevention and Control Committee.""",

"""12. Clearance and reoccupation after the work
...
A written clearance is issued by the Infection Control Team and the engineering function jointly, recording what was verified and the date of reoccupation. The clearance is retained with the project's risk assessment.""",

"""13. The cleaning programme — zones and responsibility
...
Each area has a named person accountable for its cleanliness — for clinical areas the area in-charge, for the cleaning work itself the housekeeping supervisor. Where housekeeping is outsourced, the contractor's supervisor is named and the contract incorporates this procedure.""",

"""14. Cleaning frequency by area and by surface
...
Spot cleaning of any visible soiling is immediate and is not deferred to the scheduled round. Any surface visibly soiled with blood or body fluid is dealt with under step 18.""",

"""15. Cleaning agents and disinfectants — selection and preparation
...
Selection and reprocessing of disinfectants for instruments and devices, including the classification of devices as critical, semi-critical or non-critical and the choice of high-level disinfectant, is a separate matter governed by the sterilisation and disinfection policy.""",

"""16. Cleaning method, equipment and colour coding
...
Housekeeping staff wear the protective equipment specified for the task, and perform hand hygiene after removing gloves.""",

"""17. Discharge, transfer and terminal cleaning
...
Completion of terminal cleaning is recorded and signed, and the bed is not shown as available until it is.""",

"""18. Blood and body fluid spills
...
A spill kit containing the necessary materials is available in every clinical area and its contents are checked at an interval of [Hospital to define]. Spills of chemical, cytotoxic or mercury-containing material are not covered here and are managed under the hazardous material procedure.""",

"""19. Verification that cleaning happened and worked
...
Findings are fed back to the individual cleaner and the supervisor promptly, since feedback weeks later changes nothing. Repeated failure in an area is treated as a training or resourcing problem to be fixed rather than solely as a disciplinary matter, and persistent failure is reported to the Infection Prevention and Control Committee.""",

"""20. Biomedical waste — segregation at the point of generation
...
A colour-coding chart with pictures, in the languages the staff of {{HOSPITAL_NAME}} read, is displayed at every point of waste generation.""",

"""21. Containers, labelling and bin management
...
Bins are inspected daily by the floor supervisor against a checklist covering presence, condition, correct lining, fill level, closure and labelling, and the inspection is recorded.""",

"""22. Internal transport and central storage
...
No untreated biomedical waste is stored beyond 48 hours. Where this cannot be met, the reason and the authorisation obtained are recorded. The storage area is cleaned and disinfected daily and after every collection.""",

"""23. Handover, manifests and regulatory records
...
Barcoding or such other tracking system as the rules require is applied to bags and containers handed over.""",

"""24. Protection, immunisation and training of waste handlers
...
Training attendance and immunisation status are recorded by name and are reviewed at the Infection Prevention and Control Committee. A handler who has not been trained does not handle waste.""",

"""25. Waste-related incidents and spills
...
Every waste-related incident — a spill, a sharps injury, a wrongly segregated container discovered downstream, a missed collection, a rejected consignment — is recorded and reviewed at the Infection Prevention and Control Committee for what it reveals about the system rather than the individual. Where the cause is a bin in the wrong place, a container of the wrong size, or a collection frequency that does not match generation, the system is changed.""",

"""26. Linen — handling and segregation at the point of use
...
Staff handling used linen wear gloves and an apron, and perform hand hygiene after removing them. Colour coding and bagging requirements are displayed in every sluice and linen store of {{HOSPITAL_NAME}}.""",

"""27. Transport and holding of soiled linen
...
Bags are not opened, sorted or counted in clinical areas or in the clean linen store. Where sorting is necessary, it is done in the laundry's designated soiled area, by staff wearing the specified protective equipment.""",

"""28. Washing parameters
...
Where laundry is outsourced, {{HOSPITAL_NAME}} obtains the wash records or an equivalent assurance from the contractor at an interval of [Hospital to define], and inspects the contractor's premises at an interval of [Hospital to define]. Delegating the washing does not delegate the duty to know it was done correctly.""",

"""29. Clean linen — handling, storage and distribution
...
The person accountable for food safety at {{HOSPITAL_NAME}} is [Hospital to define]. Food service is included in the scope of the Infection Prevention and Control Committee, which reviews kitchen audit results, food handler health records and any suspected foodborne incident.""",

"""31. Food handler health, hygiene and training
...
Food handlers are trained in food safety at induction and at an interval of [Hospital to define]. {{HOSPITAL_NAME}} maintains at least one trained and certified food safety supervisor for every 25 food handlers, as the food safety training and certification scheme requires, and retains the certificate.""",

"""32. Receipt and storage of food
...
Refrigerator and freezer temperatures are checked and recorded at a frequency of [Hospital to define], with an out-of-range reading generating an immediate corrective action, a decision on the affected food, and a record of both.""",

"""33. Temperature control in preparation, holding and service
...
Where {{HOSPITAL_NAME}} retains reference samples of food served, the sample, the date and the meal are recorded and the sample held for [Hospital to define], so that a suspected foodborne incident can be investigated rather than argued about.""",

"""34. Kitchen cleaning, pest control and waste
...
Kitchen waste is removed frequently in covered containers, is not accumulated in the kitchen, and is kept entirely separate from biomedical waste streams. Kitchen waste containers are cleaned after each emptying.""",

"""35. Training of support services staff
...
Records of training are kept by name, date, topic and trainer, and note that competence was observed where a technique was taught.""",

"""36. Monitoring and audit of support services
...
Where a service is outsourced, audit findings are communicated formally to the contractor and, where performance does not improve, escalated through the contract. Contract renewal decisions take account of the audit record.""",

"""37. Records retained under this policy
...
Records are retained for a period of [Hospital to define], which is not less than any period prescribed by applicable law — including the retention periods required by the biomedical waste rules — or by the accreditation standard, and are stored so that they remain legible, retrievable and confidential.""",
...
The review specifically checks that the outsourced services described here are still the ones actually contracted, and that the named responsibilities still correspond to posts that exist and are filled. Revisions are approved by the Committee, endorsed by management, and issued with the superseded version withdrawn from every point of use."""
]

RESPONSIBILITY = """The Infection Prevention and Control Committee owns this policy, reviews audit and environmental results from every support service, and escalates unresolved failures and unfunded requirements to management.
...
All staff are responsible for segregating waste correctly at the point they generate it and for reporting spills, breaches and unsafe conditions immediately."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Hospital Infection Control chapter, standard HIC.3.
...
OE_MAPPING = [
    {
        "oe_code": "HIC.3.a",
        "requirement": "Appropriate engineering controls are in place to prevent infection",
        "steps": "Steps 1-7",
        "evidence": "Engineering systems register; HVAC/AHU preventive maintenance records; filter change log with technician signatures; critical-area validation reports covering air change rate, pressure differential and HEPA integrity; OT temperature and humidity logs; environmental air and surface sampling results with corrective actions; water tank cleaning and water quality test reports",
        "responsible": "Engineering / Maintenance In-charge of {{HOSPITAL_NAME}}, jointly with the Infection Control Officer",
    },
    {
        "oe_code": "HIC.3.b",
        "requirement": "A plan is designed and implemented to reduce infection risk during construction and renovation",
        "steps": "Steps 8-12",
        "evidence": "Signed infection control risk assessment for every project, recording activity type, patient risk group and class of precautions; daily containment inspection log; contractor briefing register; post-work clearance certificate signed jointly by engineering and the Infection Control Team, with any clearance sampling results",
        "responsible": "Engineering / Projects In-charge and the commissioning department, with approval by the Infection Prevention and Control Committee",
    },
    {
        "oe_code": "HIC.3.c",
        "requirement": "Housekeeping procedures are adhered to",
        "steps": "Steps 13-19",
        "evidence": "Written housekeeping procedure with zone-wise frequencies; area cleaning checklists with cleaner and supervisor sign-off; disinfectant preparation and dilution records; colour-coding chart displayed at equipment stores; terminal cleaning records; spill kit check records; supervisor and Infection Control Nurse inspection reports; environmental surface swab results",
        "responsible": "Housekeeping Supervisor (or the contractor's supervisor where outsourced), audited independently by the Infection Control Nurse",
    },
    {
        "oe_code": "HIC.3.d",
        "requirement": "Biomedical waste is handled appropriately and safely",
        "steps": "Steps 20-25",
        "evidence": "Current State Pollution Control Board authorisation; daily BMW record by category and department; bin inspection checklists; manifests / consignment notes from the common treatment facility; annual report submitted by 30 June and published on the website of {{HOSPITAL_NAME}}; waste handler training records and hepatitis B immunisation status; waste incident and sharps injury records",
        "responsible": "Biomedical Waste Officer / Waste Management In-charge of {{HOSPITAL_NAME}}",
    },
    {
        "oe_code": "HIC.3.e",
        "requirement": "Laundry and linen management processes are adhered to",
        "steps": "Steps 26-29",
        "evidence": "Linen management procedure; wash cycle temperature and hold-time records per load (or contractor wash records where outsourced); linen flow audit checklists from soiled collection to clean storage; clean linen store inspection log; contractor premises inspection reports; linen condition and par-level audit",
        "responsible": "Laundry In-charge / Housekeeping Supervisor, with the contract manager where laundry is outsourced",
    },
    {
        "oe_code": "HIC.3.f",
        "requirement": "Kitchen sanitation and food handling issues are adhered to",
        "steps": "Steps 30-34",
        "evidence": "Current FSSAI licence (and the contractor's licence where catering is outsourced); approved supplier list with review dates; food handler annual medical examination and enteric immunisation records; food safety supervisor certification at one per 25 food handlers; refrigerator, hot-holding and core cooking temperature logs; kitchen cleaning schedule with sign-off; pest control visit and treatment records; kitchen hygiene audit reports",
        "responsible": "Dietary / Food Services In-charge of {{HOSPITAL_NAME}} (or the catering contractor's manager), overseen by the Infection Prevention and Control Committee",
    },
]
...
sql = f"""-- HIC.3 master policy draft -- {STANDARD_CODE}
...
_bad = [i for i, s in enumerate(PROCEDURE_STEPS, 1) if not _step_re.match(s)]
assert not _bad, f"steps not matching renderer regex: {_bad}"
_misnumbered = [i for i, s in enumerate(PROCEDURE_STEPS, 1) if not s.startswith(f"{i}. ")]
assert not _misnumbered, f"step number prefix mismatch: {_misnumbered}"

# --- report ------------------------------------------------------------------
mapped = set()
for m in OE_MAPPING:
    norm = m["steps"].replace("Steps", "").replace("Step", "").replace(" and ", ", ")
...
print("all steps match renderer regex: True")
print("CR bytes in SQL:", _raw.count(b"\r"))
print("mapping covers all 6 OEs:", sorted(m["oe_code"] for m in OE_MAPPING) == sorted(OE_CODES))
print("every mapping entry has evidence+responsible:",
      all(m.get("evidence") and m.get("responsible") for m in OE_MAPPING))
print("steps referenced by mapping:", f"{min(mapped)}-{max(mapped)}")
print("steps NOT referenced:", sorted(set(range(1, len(PROCEDURE_STEPS) + 1)) - mapped))
print("out-of-range refs:", sorted(x for x in mapped if x > len(PROCEDURE_STEPS)) or "none")
# Placeholder audit — see policy_placeholder_audit.py. The count that used to sit
# here counted json.dumps(draft), which both included the internal-only
# universal_facts_checklist and missed every "[Hospital to define — guidance]" variant.
