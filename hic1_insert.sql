-- HIC.1 master policy draft — HIC.1
...
--   procedure_steps text[]  -- confirmed against live schema
--
-- procedure_steps is written as an array[...] literal. If that column is ever changed to
-- jsonb, this insert fails with a type error; regenerate with
-- PROCEDURE_STEPS_IS_JSONB = True in build_hic1.py. Re-check types with:
...
Reporting an infection control problem, a breach or a near miss is expected of every member of staff and is treated as a contribution to safety. {{HOSPITAL_NAME}} does not take disciplinary action against a person for reporting in good faith; it reserves such action for wilful or repeated disregard of a known precaution.$q$,
  array[
    $s$1. How the infection prevention and control programme is structured
...
The programme covers, at minimum: standard and transmission-based precautions; hand hygiene; safe injection and sharps practice; cleaning, disinfection and sterilisation; environmental hygiene; linen and laundry; biomedical waste; surveillance of healthcare associated infection; antimicrobial stewardship; staff health and post-exposure management; isolation facilities; and education of staff, patients and the community.$s$,
    $s$2. The Infection Prevention and Control Committee (IPCC)
...
The names and contact details of the current team members are displayed in every clinical area of {{HOSPITAL_NAME}} so that any member of staff can reach them without going through a supervisor.$s$,
    $s$4. The infection control manual and what it must contain
...
Revisions are approved by the IPCC before they take effect. When a document is revised, the superseded version is withdrawn from every point of use and one dated copy is retained by the Infection Control Team as a record — an obsolete policy left in a ward folder is a source of error, not a record. Each policy carries an amendment history showing what changed, why, and on whose approval.$s$,
    $s$6. Annual infection risk assessment
...
Risks are ranked so that the programme's effort follows the largest risks rather than the most visible ones. The completed assessment is tabled at the IPCC and retained.$s$,
    $s$7. The written annual programme
...
The case definitions, the counting of device days, the calculation of rates and the benchmarking and feedback method are set out in the hospital's surveillance policy and are not restated here. What this policy fixes is ownership: surveillance data is collected by the Infection Control Team, reviewed at every IPCC meeting, and fed back to the clinical area it came from. Data that is collected but never returned to the people whose practice it describes does not change practice.$s$,
    $s$9. Environmental surveillance within the programme
...
Each improvement project in the annual programme identifies how it addresses all five.$s$,
    $s$11. Education and training under the programme
...
Attendance is recorded by name, date, topic and trainer, and the records are retained. Where a technique is being taught — hand hygiene, PPE removal, spill management — the record notes that competence was observed and not merely that the session was attended.$s$,
    $s$12. Reviewing the programme against an infection control assessment tool
...
The review is led by the Infection Control Officer or Infection Control Nurse. It is completed honestly against evidence that can be produced on request, and not from recollection — a score that cannot be evidenced is recorded as not met.$s$,
    $s$13. Acting on the assessment findings
...
Findings that need money or posts that the committee cannot authorise are escalated to management under step 14.$s$,
    $s$14. Resources — the infection control budget
...
An unfunded infection control requirement is treated as an open risk: it is entered in the hospital's risk register and reviewed at each IPCC meeting until it is resolved.$s$,
    $s$15. Resources — consumables and continuity of supply
...
Substitution of a specified product with an alternative during a shortage requires the agreement of the Infection Control Officer, so that an ineffective or incompatible agent is not introduced under supply pressure.$s$,
    $s$16. Resources — staffing the programme
...
Persons appointed to infection control roles receive formal training for the role, and the certificates or training records are retained. Where {{HOSPITAL_NAME}} cannot meet the benchmark, the shortfall is recorded as a risk under step 14 together with the interim arrangement being relied on, rather than left undocumented.$s$,
    $s$17. Resources — infrastructure at the point of care
...
A deficiency that cannot be corrected on the spot is logged with a target date and tracked to closure at the IPCC.$s$,
    $s$18. Isolation and barrier nursing facilities
...
An inventory of isolation equipment is maintained and checked at an interval of [Hospital to define] so that the room is ready before it is needed rather than being equipped at the moment of admission.$s$,
    $s$19. Airborne infection isolation
...
Patients requiring airborne precautions are not cohorted with patients under other precautions and are not placed in an open bay other than as an explicitly recorded emergency measure while transfer is arranged.$s$,
    $s$21. Barrier nursing practice in an isolation area
...
The precautions themselves — which apply to which route of transmission, and the sequence for putting on and removing protective equipment — are set out in the transmission-based precautions policy and are not restated here.$s$,
    $s$22. Isolation register and review of isolation use
...
Competence in barrier nursing is maintained by practical drills at an interval of [Hospital to define], with the observations and any corrective action recorded.$s$,
    $s$23. Information, education and communication for patients, visitors and the community
...
Activities are recorded with the date, topic, audience and approximate number reached, and the record is reviewed by the IPCC so that the programme is adjusted rather than merely repeated.$s$,
    $s$24. Participation in community outbreaks and pandemics
...
The plan is tested at an interval of [Hospital to define] by drill or tabletop exercise, and the findings are used to revise it. Participation and findings are recorded.$s$,
    $s$25. Notification of communicable disease to the public health authorities
...
A cluster of infection among patients or staff of {{HOSPITAL_NAME}} is reported internally to the Infection Control Officer on the same day it is suspected, and externally where the disease or the scale requires it.$s$,
    $s$26. Recognising and responding to an outbreak within the hospital
...
The outbreak record is retained and the lessons are reflected in revisions to the relevant policy and in the next annual risk assessment.$s$,
    $s$27. Reporting programme performance to management
...
Management records its consideration of the report and any decision taken. Where management declines or defers a request, the reason and the interim control are recorded under step 14.$s$,
    $s$28. Records retained under this policy
...
Records are retained for a period of [Hospital to define], which is not less than any period prescribed by applicable law or by the accreditation standard, and are stored so that they remain legible, retrievable and confidential.$s$,
    $s$29. Review of this policy

This policy is reviewed by the IPCC at least once every [Hospital to define], and earlier where step 5 requires it. The review considers whether the structure described here still matches how {{HOSPITAL_NAME}} actually operates, and the policy is amended where it does not — a policy describing a committee that no longer meets, or a post no longer filled, is worse than no policy at all. Revisions are approved by the IPCC, endorsed by management, and issued under step 5.$s$
  ],
  $q$Management of {{HOSPITAL_NAME}} is accountable for the existence, funding and standing of the infection prevention and control programme, for approving the resources it requires, and for acting on what the IPCC reports.
...
The administrative or purchase function is responsible for maintaining the supply of infection control consumables, and the engineering function for the ventilation, water and physical conditions the programme depends on.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Hospital Infection Control chapter, standard HIC.1.
...
- Internal documents of {{HOSPITAL_NAME}}: infection prevention and control practices policy, surveillance policy, environmental cleaning policy, sterilisation and instrument reprocessing policy, biomedical waste management policy, occupational health policy, and the outbreak and pandemic response plan.$q$,
...
The current version is available to all staff at [Hospital to define — intranet location or nursing station folder]. Superseded versions are withdrawn from all points of use on issue of a revision, and one dated copy of each is retained by the Infection Control Team.$q$,
...
This document is not issued by, endorsed by, or affiliated with NABH, the World Health Organization, the National Centre for Disease Control, or any other body named in it. Wording is original; no text has been reproduced from the standards or guidelines referenced.$q$,
  $q$[{"oe_code": "HIC.1.a", "requirement": "The infection prevention and control programme is documented and aims at preventing and reducing healthcare associated infection risk", "steps": "Steps 1, 4-11 and 26"}, {"oe_code": "HIC.1.b", "requirement": "The programme is reviewed using an infection control assessment tool", "steps": "Steps 12-13"}, {"oe_code": "HIC.1.c", "requirement": "A multidisciplinary infection control committee and an infection control team coordinate all IPC activities", "steps": "Steps 2-3"}, {"oe_code": "HIC.1.d", "requirement": "IEC programme for the community, and participation in managing community outbreaks and pandemics", "steps": "Steps 23-25"}, {"oe_code": "HIC.1.e", "requirement": "Management makes the resources required for the programme available", "steps": "Steps 14-17"}, {"oe_code": "HIC.1.f", "requirement": "Isolation and barrier nursing facilities are available", "steps": "Steps 18-22"}]$q$::jsonb,
