...
function LoginScreen(props){ return <AuthForm variant="page" {...props} />; }
...
export default function App() {
  ...
  const generateShcoFullPDF = async () => {
    ...
    try {
      ...
      const relevantOEs = shcoFullAssessType==='final'        ? coreCommOEs
                        ...
                        : shcoFullOes;
      const scopeDesc = shcoFullAssessType==='final'
        ? `${relevantOEs.length} (Core + Commitment; ${shcoFullOes.length} total across all levels)`
        : shcoFullAssessType==='surveillance'
        ? `${relevantOEs.length} (Core + Commitment + Achievement; ${shcoFullOes.length} total across all levels)`
        : `${relevantOEs.length} (all OE levels)`;
      ...
      doc.text('OEs in scope: ' + scopeDesc, W/2, statY+36, {align:'center'});
      ...
      const summaryRows=[
        ...
        ['Total OEs in Scope', scopeDesc,                                    '#eef4f9', 9],
      ...
      summaryRows.forEach(([lbl,val,col,valSize])=>{
        ...
        doc.text(lbl,80,y-1);
        doc.setFontSize(valSize||11); doc.setTextColor(col);
  ...
  if(authState==="homepage") return <HomepageScreen onLogin={u=>{setUser(u);setAuthState("setup");}} />;
  if(authState==="recovery") return <RecoveryScreen user={user} onDone={()=>{setUser(null);setAuthState("login");setContext(null);}}/>;
  if(authState==="login") return <LoginScreen onLogin={u=>{setUser(u);setAuthState("setup");}} onBackToHomepage={()=>{window.history.pushState({screen:"homepage"},"",window.location.pathname);setAuthState("homepage");}} initialError={authErrorMsg}/>;
