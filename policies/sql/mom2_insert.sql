-- MOM.2 master policy -- UNAPPROVED DRAFT for review.
...
--
-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 3, printed pages 76-77
...
insert into public.shco_policy_masters (
  ...
  'MOM.2',
  'MOM',
  array['MOM.2.a', 'MOM.2.b', 'MOM.2.c', 'MOM.2.d', 'MOM.2.e', 'MOM.2.f'],
  $q$Storage and Availability of Medications$q$,
  $q$This document sets out how {{HOSPITAL_NAME}} stores medications so that they are available where they are required: a clean, safe and secure environment that incorporates the manufacturer's recommendations; inventory control throughout the organisation; a defined list of high-risk medications including look-alike/sound-alike items and a mechanism for storing them; high-risk medications stored where they are clinically necessary; a defined emergency-medication list stored uniformly; and emergency medications available at all times and replenished promptly when used.
...
- Counselling about medicines is governed by PRE (not yet drafted). The medical record itself is governed by IMS (not yet drafted).$q$,
  $q${{HOSPITAL_NAME}} stores medications in a clean, safe and secure environment, incorporating the manufacturer's recommendations. A cupboard that is unlocked because it is inconvenient, or a refrigerator used for food and medicines together, is not that environment.
...
{{HOSPITAL_NAME}} keeps emergency medications available all the time and replenishes them promptly when used. A drawer that was emptied at the last crash and not refilled is not availability.$q$,
  array[
    $s$1. Clean, safe and secure storage incorporating the manufacturer's recommendations
...
Pharmacy oversight of floor-stock, as the pharmacy-committee policy requires, includes that these locations are known to the pharmacy.$s$,
    $s$2. Inventory control throughout the organisation
...
Acquisition remains the pharmacy-committee policy. This step stores and accounts for what was acquired. After-hours obtainment under that policy is deducted here when the pharmacy reopens.$s$,
    $s$3. High-risk medications including look-alike/sound-alike — list and storage mechanism
...
The list and the storage mechanism are held at [Hospital to define — where the high-risk and look-alike/sound-alike list and the storage mechanism are held], including in every location that stocks an item on the list. A list that lives only in the pharmacy is not a mechanism the ward can use.$s$,
    $s$4. High-risk medications stored where clinically necessary
...
This step does not extend NDPS storage into a ward cupboard; MOM.8 still owns that cupboard even where clinical necessity exists.$s$,
    $s$5. Emergency-medication list defined and stored uniformly
...
The list and the uniform-storage method are held at [Hospital to define — where the emergency-medication list and the uniform-storage method are held], including in every named location. The multidisciplinary medication committee reviews the list when services change.$s$,
    $s$6. Emergency medications available all the time and replenished promptly
...
Expired emergency stock is removed and replaced before it is the only stock. Pharmaceutical waste of the removed item follows HIC.3.$s$,
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a look-alike pick, an empty emergency drawer, a high-risk item stored where it was not needed, or a revision of the pharmacy-committee, resuscitation, emergency-care, transfusion or controlled-agents policies that this document hands work to, exposes a gap.$s$
  ...
  $q$The head of the institution is accountable for {{HOSPITAL_NAME}} storing medicines in a clean, safe and secure way, for a high-risk list that is actually used, and for emergency medicines that are present when they are needed.
...
All staff are expected to treat an unlocked medicine store, a look-alike pair stored as if they were ordinary neighbours, and an emergency drawer left empty after use, as defects, and to report them.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Management of Medication chapter, standard MOM.2.
...
This document is not issued by, endorsed by, or affiliated with NABH, the World Health Organization, the National Centre for Disease Control, the Food Safety and Standards Authority of India, any Pollution Control Board, or any other body named in it. Wording is original; no text has been reproduced from the standards, rules or guidelines referenced.$q$,
  $q$[{"oe_code": "MOM.2.a", "requirement": "Medications are stored in a clean, safe and secure environment, incorporating the manufacturer's recommendations.", "steps": "Steps 1, 7", "evidence": "The named storage locations and the method that keeps them clean, safe and secure; any local operating figures taken from the manufacturer's recommendation rather than invented as mandates; records of expired stock removed into the HIC.3 stream; the recorded division that NDPS/chemo/radioactive storage is MOM.8; the audit sample at step 7 of locations that are not a food refrigerator or an unlocked corridor trolley", "responsible": "Person in charge of the pharmacy oversees locations; nursing staff in each location keep them clean, safe and secure; quality or accreditation coordinator audits"}, {"oe_code": "MOM.2.b", "requirement": "Sound inventory control practices guide storage of medications throughout the organisation.", "steps": "Steps 2, 7", "evidence": "The written inventory-control method covering pharmacy and every out-of-pharmacy stock location; sample records of what is held, expiry and near-expiry handling; records of after-hours obtainment deducted when the pharmacy reopened; the audit sample at step 7 of inventory that includes ward and emergency-area stock, not only the pharmacy", "responsible": "Person in charge of the pharmacy runs inventory; location staff count and report; quality or accreditation coordinator audits"}, {"oe_code": "MOM.2.c", "requirement": "The organisation defines a list of high-risk medications, including look-alike/sound-alike medications, and a mechanism for storing them.", "steps": "Steps 3, 4, 7", "evidence": "This hospital's dated high-risk list including look-alike/sound-alike pairs, matching the current formulary and the service directory rather than an unadapted ISMP high-alert or confused-names printout left as decoration, and showing that a pair this hospital does not stock is not on the list and that a concentrated electrolyte this hospital does stock is on the list if the hospital's method treats it as high-risk; the written storage mechanism stating how those items are separated, how they are labelled in the location, who may access them, and how a pick is checked, without a mandated colour, shelf height or named labelling convention, and showing the same idea in every location that holds the item rather than a theatre habit and a ward habit; the recorded statement that ISMP lists, WHO LASA patient-safety solution, Tall Man lettering and the Malaysian LASA guide (chapter references 3, 4, 5, 7, 12, 14) are frameworks the hospital may use and are not this hospital's mandated list or mandated lettering; the recorded division that NDPS, chemo and radioactive agents named as high-risk are still stored under MOM.8, and that non-NDPS sedative or anaesthetic high-risk items are stored here while COP.9 and COP.10 own the clinical act; the location of the list and mechanism in the pharmacy and in every location that stocks an item on the list, rather than a pharmacy-only file; committee records showing the list updated when the formulary changed; induction or briefing records of staff who pick from those locations; the audit sample at step 7 of a local list and a mechanism in use rather than a pinned international table with unchanged shelves", "responsible": "Person in charge of the pharmacy holds the list and the mechanism; multidisciplinary medication committee keeps the list current; location staff apply the mechanism; quality or accreditation coordinator audits"}, {"oe_code": "MOM.2.d", "requirement": "High-risk medications are stored in areas where it is clinically necessary.", "steps": "Steps 4, 3, 7", "evidence": "The written map of which high-risk item is stored in which area and why; records showing a concentrated item not needed on a general ward was not stored there by default; the recorded MOM.8 boundary for NDPS even where clinical necessity exists; the audit sample at step 7", "responsible": "Person in charge of the pharmacy and the committee decide locations; location staff do not add high-risk stock by habit; quality or accreditation coordinator audits"}, {"oe_code": "MOM.2.e", "requirement": "The list of emergency medications is defined and is stored uniformly.", "steps": "Steps 5, 6, 7", "evidence": "This hospital's dated emergency-medication list, matching the services defined under AAC.1 and the emergency work this hospital actually holds out rather than a generic tertiary-hospital cart, and showing that NDPS items on the list remain in the MOM.8 cupboard rather than creating a second narcotic drawer, and that sedative or anaesthetic items on the list are stored here as stock while COP.9 and COP.10 own the clinical act; the written uniform-storage method naming the locations that hold the list and the arrangement used in each, so that a person who can find an item in one named location can find the same item in the same relative place in the others, without printing a crash-cart contents list as a NABH mandate; the recorded split that COP.3 owns kit presence for cardio-pulmonary resuscitation in named areas and restoration of reusable equipment, COP.2 owns that the ambulance is stocked with this hospital's emergency medicines, and this document owns the list, the uniform storage, availability and replenishment; records of the ambulance using this list insofar as it carries those medicines; the location of the list and the arrangement in every named location rather than only in the quality office; committee records showing the list reviewed when services changed; induction or briefing records of staff who open those drawers; the audit sample at step 7 of the same list stored in the same arrangement in named locations rather than a different crash habit in each room", "responsible": "Person in charge of the pharmacy holds the list and the uniform arrangement; location staff keep each named location to that arrangement; COP.3 owns kit presence; COP.2 owns ambulance stocking against this list; quality or accreditation coordinator audits"}, {"oe_code": "MOM.2.f", "requirement": "Emergency medications are available all the time and are replenished promptly when used.", "steps": "Steps 6, 5, 7", "evidence": "The written completeness-check and replenishment method, including this hospital's definition of prompt refill without a mandated number of minutes; sample checks covering nights, weekends and holidays; records of replenishment after use coordinated with COP.3 kit restoration; records of expired emergency stock replaced before it was the only stock; the audit sample at step 7 of drawers that were not left empty after use", "responsible": "Location staff check and report gaps; pharmacy replenishes medicines; COP.3 restores the kit; quality or accreditation coordinator audits"}]$q$::jsonb,
