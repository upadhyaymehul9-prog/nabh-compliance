-- HIC.5 master policy -- DRAFT for review. Do NOT set status = 'approved' here;
...
insert into public.shco_policy_masters (
  ...
  'HIC.5',
  'HIC',
  array['HIC.5.a', 'HIC.5.b', 'HIC.5.c', 'HIC.5.d', 'HIC.5.e', 'HIC.5.f'],
  $q$Infection Prevention and Control Surveillance$q$,
  $q$This document sets out how {{HOSPITAL_NAME}} measures infection. It defines what is counted, how a case is decided, what it is counted against, how the resulting numbers are analysed, what happens when they move, and who is told.
...
Every other infection control policy of {{HOSPITAL_NAME}} makes a claim about what the hospital does. This one is how those claims are tested.$q$,
  $q$This policy applies to every patient area of {{HOSPITAL_NAME}} — inpatient wards, critical care and high-dependency areas, operation theatres and procedure rooms, the labour room, emergency, day-care, dialysis, and outpatient areas where invasive procedures are performed — and to the support services whose output is measured under it, including housekeeping, laundry, kitchen and waste handling. It binds the laboratory of {{HOSPITAL_NAME}}, or the outsourced laboratory acting for it, in respect of the alerts and reports this policy requires.
...
What this policy does not cover: the governance and resourcing of the infection control programme; the practices set out in the clinical areas policy; the engineering, construction, waste, laundry and kitchen requirements of the support services policy; instrument reprocessing and sterilisation; and the selection of antimicrobial agents and the structure of the stewardship arrangement. Each has its own policy. This policy supplies those policies with data and receives from them the practices it measures.$q$,
  $q${{HOSPITAL_NAME}} holds that a hospital cannot manage infection it does not measure, and cannot trust a measurement it has not defined in advance.
...
Surveillance is carried out by the Infection Control Team and reported to the Infection Prevention and Control Committee. It is not a function of the treating team, though the treating team supplies much of its raw material and receives all of its output.$s$,
    $s$2. The annual surveillance risk assessment
...
The subjects reviewed at each assessment include at a minimum device-associated infection for each device in use at {{HOSPITAL_NAME}}, surgical site infection for the procedures performed, hand hygiene compliance, multi-drug-resistant organisms, the effectiveness of housekeeping, and any infection risk particular to the services or the setting of {{HOSPITAL_NAME}} — [Hospital to define].$s$,
    $s$3. The written surveillance plan
...
Case-finding is documented daily, including on days when nothing was found. A gap in the record is indistinguishable from a day on which no one looked.$s$,
    $s$5. Case definitions — the rule against local modification
...
An infection is attributed to {{HOSPITAL_NAME}} only where it was not present or incubating at the time of admission. The definition set in force states the day threshold and the window rules that decide this, and those rules are applied as published. Infections judged present on admission are recorded as such and reported separately, because they are part of the hospital's infection burden even though they are not part of its infection rate.$s$,
    $s$6. Catheter-associated urinary tract infection — deciding a case
...
Each confirmed case is notified to the treating unit and to the Infection Control Officer, and is included in the line list for the period. Where the case involves an organism on the list at step 22, it also enters the register at step 23. The preventive actions that should have applied to this patient are set out in the infection prevention policy and are not restated here; a confirmed case triggers a review of whether they were performed, and the result of that review is recorded with the case.$s$,
    $s$7. Central line-associated bloodstream infection — deciding a case
...
Peripheral intravenous cannulae are outside this definition. Bloodstream infection arising from a peripheral cannula is recorded and trended separately under step 15, because it is a real and under-recognised risk in hospitals whose central line use is low, and a policy that counts only central lines will not see it.$s$,
    $s$8. Ventilator-associated events and ventilator-associated pneumonia — deciding a case
...
Recorded for each event: patient identifier, unit, intubation date, the daily oxygenation parameters that establish the baseline and the deterioration, temperature and white cell count where the tier requires them, antimicrobial start and duration, respiratory specimen results, the tier reached, and the criteria met at each tier.$s$,
    $s$9. Surgical site infection — deciding a case
...
Recorded for each case: patient identifier, procedure and date, surgeon, wound class, duration of operation, the risk factors the stratification requires, date of onset, depth, organism and susceptibility, criteria met, and whether the case was detected during admission, on readmission, or through the post-discharge follow-up at step 10.$s$,
    $s$10. Post-discharge surveillance for surgical site infection
...
Cases detected after discharge are counted against the period of the operation, not the period of detection. Assigning them to the month they were found produces a rate that is impossible to interpret and that moves for reasons unconnected to the surgery.$s$,
    $s$11. Device days — how the denominator is counted
...
The daily count sheets are retained as the primary record. A monthly total with no daily sheets behind it cannot be validated and is treated as unverified data under step 16.$s$,
    $s$12. Patient days, admissions and procedures — the other denominators
...
The reason for this care is arithmetic. A numerator error changes a rate by one case. A denominator error changes every rate in the period, in the same direction, invisibly.$s$,
    $s$13. Calculating the rates
...
Which of the above are under active surveillance in the current period, as against being reviewed only when a signal appears, is stated in the surveillance plan and decided at the annual risk assessment. {{HOSPITAL_NAME}} does not claim to survey all of them continuously if it does not.$s$,
    $s$16. Data quality and validation
...
The validation results are reported to the Infection Prevention and Control Committee with the surveillance data, not separately and not on request. A rate presented without its data quality context invites more confidence than it has earned.$s$,
    $s$17. Analysis — trends, thresholds and what a movement means
...
What a movement does not mean. The Infection Control Officer explicitly considers, and records having considered, whether an apparent change is an artefact before treating it as real: a change in the definition set, a change in case-finding intensity or in the person doing it, a change in testing or culturing practice, a change in denominator collection, a change in case mix, or simple small-number variation. A rate built on a small denominator will cross most thresholds eventually without anything having changed, and treating that as a finding wastes the credibility the system needs for the occasion when the change is real.$s$,
    $s$18. Benchmarking — internal and external
...
Where {{HOSPITAL_NAME}} participates in a surveillance network that supplies risk-adjusted comparison, that comparison is used in preference to unadjusted published rates, and the network and the period are named.$s$,
    $s$19. Hand hygiene compliance — what is monitored
...
The observed figure is affected by the presence of the observer, and {{HOSPITAL_NAME}} states this rather than pretending otherwise. It is the reason the indirect measure at step 21 is tracked alongside, and the reason the reported number is treated as an indicator of direction rather than as an exact truth.$s$,
    $s$21. Hand hygiene compliance — calculation, indirect measures and reporting
...
The compliance trend is read against the infection rates at step 17. The relationship between the two is real but is neither immediate nor proportionate, and {{HOSPITAL_NAME}} does not present a fall in infection rates as proof that a hand hygiene intervention worked, nor a stable infection rate as proof that it did not.$s$,
    $s$22. Multi-drug-resistant organisms — defining the list
...
Where the laboratory of {{HOSPITAL_NAME}} is outsourced, these requirements are written into the contract, and the contract states the laboratory's obligation to report against the agreed standard and to raise the alerts at step 23.$s$,
    $s$23. Multi-drug-resistant organisms — the capture mechanism
...
Where the same organism with a similar susceptibility pattern appears in more than one patient in the same unit within the period at step 17, the Infection Control Officer treats it as a possible transmission event and enters the outbreak route at step 31, whether or not the number reaches any threshold. Genuinely unrelated acquisition is a reasonable finding of an investigation; it is not a safe assumption to start from.$s$,
    $s$24. Highly virulent and epidemiologically significant infections — immediate alert
...
Every alert raised is logged with the date, time, raiser, recipient and the action taken, including alerts that proved unfounded. The log is reviewed by the Committee, since a falling number of alerts is more likely to indicate that people have stopped raising them than that the hospital has become safer.$s$,
    $s$25. Cumulative antibiogram and resistance trend reporting
...
calculated by area and by surface type. The breakdown by surface type is what makes it actionable: a consistent failure on one class of surface across many areas indicates a gap in the procedure or the training, while scattered failures in one area indicate a problem with that area's staffing or time.$s$,
    $s$28. Environmental and water sampling — when it is done and when it is not
...
Hand and glove cultures of staff, and routine culturing of staff generally, are not performed as surveillance. They are done only as part of a specific outbreak investigation where the hypothesis requires them, with the consent and the involvement of the individuals concerned, and with the result handled under the confidentiality provisions at step 38.$s$,
    $s$29. Housekeeping surveillance — reporting and action
...
The relationship between housekeeping effectiveness and infection rates is reported but not overstated. Environmental transmission is one route among several, and a hospital whose cleaning improves while its infection rates do not has not necessarily wasted the effort. The measurement is reported on its own merits.$s$,
    $s$30. Outbreak — definition and the thresholds that trigger one
...
Authority. The Infection Control Officer may enter the outbreak route and initiate the immediate measures at step 32 on their own authority, without waiting for a committee, a manager or the head of the institution. The deputy named in the surveillance plan holds the same authority in the Infection Control Officer's absence, so that the authority exists at every hour of every day — [Hospital to define — name the Infection Control Officer and the deputy, with 24-hour contact details].$s$,
    $s$31. Outbreak — detection and the duty to report a suspicion
...
Every suspicion received is logged with the date, time, source and the action taken, whether or not it was investigated. The log is reviewed by the Infection Prevention and Control Committee.$s$,
    $s$32. Outbreak — the immediate response
...
Time of entry into the route is recorded, because the interval between the first case and the first control measure is the single most useful number in the outbreak report.$s$,
    $s$33. Outbreak — investigation
...
Record what was excluded. Hypotheses considered and rejected, with the reason, are recorded alongside the one pursued. An investigation that reports only its conclusion cannot be evaluated, and cannot be resumed intelligently if the outbreak recurs.$s$,
    $s$34. Outbreak — control measures
...
All staff are responsible for cooperating with surveillance, for recording accurately the data this policy depends on, and for reporting immediately any suspicion of a cluster or an outbreak — a duty that belongs to every member of staff of every grade and is exercised directly.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Hospital Infection Control chapter, standard HIC.5.
...
- Internal documents of {{HOSPITAL_NAME}}: infection prevention and control programme policy, infection prevention and control practices policy, infection prevention and control in support services policy, prevention of healthcare associated infections and staff occupational health policy, sterilisation and disinfection policy, antimicrobial usage policy, occupational health policy, records retention policy, and the outbreak and pandemic response plan.$q$,
