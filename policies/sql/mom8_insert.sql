-- MOM.8 master policy -- UNAPPROVED DRAFT for review.
...
--
-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 3, printed page 80
...
insert into public.shco_policy_masters (
  ...
  'MOM.8',
  'MOM',
  array['MOM.8.a', 'MOM.8.b', 'MOM.8.c', 'MOM.8.d', 'MOM.8.e'],
  $q$Safe Use of Narcotics, Chemotherapeutic Agents and Radioactive Agents$q$,
  $q$This document sets out how {{HOSPITAL_NAME}} uses narcotic drugs and psychotropic substances, chemotherapeutic agents and radioactive agents in a safe manner: which of those three classes this hospital actually uses; who may prescribe them; how those in use are stored securely; how chemotherapy and radioactive agents, if used, are prepared and by whom they are administered; and the record of usage, administration and disposal.
...
This document accepts the handoff from the procedural-sedation and anaesthesia policies of {{HOSPITAL_NAME}}: the clinical act of sedating or anaesthetising remains those policies; the Narcotic Drugs and Psychotropic Substances Act, 1985 cupboard, register and destruction live here.$q$,
  $q$This policy applies to every location at {{HOSPITAL_NAME}} in which a narcotic drug, a psychotropic substance, a chemotherapeutic agent or a radioactive agent used as therapy is prescribed, stored, prepared, administered or disposed of. It binds the clinicians who prescribe those agents, the staff who hold keys or access to secure storage, the persons who prepare chemotherapy or radioactive agents where those services exist, the persons who administer them, and whoever keeps the usage, administration and disposal record.

It covers: safe use of those three classes; prescription by appropriate caregivers; secure storage; proper and safe preparation of chemotherapy and radioactive agents and administration by qualified personnel; and a proper record of usage, administration and disposal of all three classes.
...
- Administration of ordinary medicines is governed by the administration policy of {{HOSPITAL_NAME}} (MOM.6, sibling). Preparation of chemotherapy and radioactive agents, and administration of those two by qualified personnel, are this document (MOM.8.d). MOM.6 does not write a cytotoxic reconstitution method.
...
- Colour-coded waste from cytotoxic or chemotherapy material is governed by the support-services infection-control policy of {{HOSPITAL_NAME}} (HIC.3, approved). This policy requires that such waste enters that stream; it does not restate colour categories, storage times or common-treatment-facility handover. Chemo waste is not named in this document's statutory paragraph.
- Personal protective equipment for preparation of chemotherapy is governed by the infection-prevention-and-control-practices policy of {{HOSPITAL_NAME}} (HIC.2, approved). This document requires that those practices are applied at preparation; it does not rewrite PPE selection or donning. Preparation safety as a medication process is MOM.8.d.
...
- The medical record itself is governed by the information-management policies of {{HOSPITAL_NAME}} (IMS, not yet drafted). This policy owns the usage, administration and disposal content written for these classes.$q$,
  $q${{HOSPITAL_NAME}} uses narcotic drugs and psychotropic substances, chemotherapeutic agents and radioactive agents safely, and only the classes it actually uses. A class that is not a defined service is recorded as absent against the service directory. Absence is not a copied bunker.
...
{{HOSPITAL_NAME}} requires that chemotherapy and radioactive agents, where used, are prepared properly and safely and administered by qualified personnel. A hospital that does not use those agents does not invent a preparation room.

{{HOSPITAL_NAME}} keeps a proper record of usage, administration and disposal of each class in use.

{{HOSPITAL_NAME}} does not treat diagnostic radiology as a radioactive agent used as therapy, and does not treat ordinary medication storage as the secure storage of these three classes.$q$,
  array[
    $s$1. Narcotic drugs and psychotropic substances, chemotherapeutic agents and radioactive agents are used safely
...
This step does not print the NDPS schedules, does not print a destruction method as a numbered mandate, and does not print chemotherapeutic doses. Those values, where a class is in use, are hospital-defined at the later steps: the cupboard and who holds access at step 3, the qualified preparers at step 4, the register at step 5. What this step requires is that the hospital names the classes it uses, records absence for the rest, and applies the later steps only to the classes in use.

The written safe-use guidance — the named classes, the recorded absences, and the pointer to steps 2 to 5 for the classes in use — is held at [Hospital to define — where the written safe-use guidance for these three classes is held]. A location that holds a class this hospital has not named does not use it.$s$,
    $s$2. Prescribed by appropriate caregivers
...
Prescription of these classes still follows the prescribing and order-writing policies of {{HOSPITAL_NAME}} for how the order is written. This step owns who is allowed to write it.$s$,
    $s$3. Stored securely
...
How and where each class in use is stored securely — the cupboard or other secure location, who holds access, and how access is recorded — are [Hospital to define — how and where each class in use is stored securely, including the cupboard or location and who holds access]. This document does not print an NDPS schedule table and does not mandate a numbered lock type. It requires that storage of these classes is secure in a way ordinary medication storage under the medication-storage policy of {{HOSPITAL_NAME}} is not asked to be, and that narcotic drugs and psychotropic substances used here are stored in a manner that can satisfy the Narcotic Drugs and Psychotropic Substances Act, 1985. Chemotherapeutic agents in use are stored so that they are not handled as ordinary ward stock. Radioactive agents used as therapy, if any, are stored under the conditions the Atomic Energy (Radiation Protection) Rules, 2004 and the authorisation from the Atomic Energy Regulatory Board require; diagnostic-radiology rooms remain under the imaging policies.

Keys or access left in a drawer, a narcotic kept in the general drug trolley overnight, or a cytotoxic stored next to look-alike ordinary ampoules, is not secure storage of these classes.$s$,
    $s$4. Chemotherapy and radioactive agents prepared properly and safely, and administered by qualified personnel

Chemotherapy and radioactive agents, where this hospital uses them, are prepared properly and safely, and administered by qualified personnel. If either class is recorded as absent at step 1, this step is that recorded absence. It is not a reconstitution SOP, a hot laboratory, or a qualified-person list invented for assessment.

Who may prepare chemotherapy and who may prepare radioactive agents used as therapy, where those classes are in use, and where preparation is done, are [Hospital to define — who may prepare chemotherapy and radioactive agents used as therapy, and where preparation is done, or the recorded absence if the class is not used]. Who may administer each of those classes in use is [Hospital to define — who may administer chemotherapy and radioactive agents used as therapy, or the recorded absence if the class is not used]. This document does not print chemotherapeutic doses and does not print a reconstitution recipe. Preparation safety is this step as a medication process. Personal protective equipment at preparation follows the infection-prevention policy of {{HOSPITAL_NAME}}; it is applied, not rewritten. Waste from cytotoxic preparation enters the biomedical-waste stream of {{HOSPITAL_NAME}}; colours are not restated here.

Narcotic drugs and psychotropic substances are not reconstituted as chemotherapy. Their safe use and secure storage are steps 1 and 3; their administration as a clinical act of sedation or anaesthesia remains under those clinical policies; administration as analgesia remains under the administration policy of {{HOSPITAL_NAME}} with the account at step 5.

A chemotherapeutic dose prepared in the operation theatre is still prepared and administered under this step. The surgical act is the procedures-and-operation-theatre policy of {{HOSPITAL_NAME}}.$s$,
    $s$5. Record of usage, administration and disposal
...
A register that does not match the cupboard, or a disposal that is not written, is not a proper record.$s$,
    $s$6. Records, review and the order of operations

The named classes, the recorded absences against the service directory, the authorised prescribers, the secure-storage arrangement, the qualified preparers and administrators where chemotherapy or radioactive agents are used, and the usage-administration-disposal record of each class in use, are retrievable.

The quality or accreditation coordinator audits a sample of these records at [Hospital to define — the audit interval for narcotic, chemotherapeutic and radioactive-agent records] for: classes in use matching the service directory, and unused classes recorded as absent rather than documented as a bunker that does not exist; prescription only by the appropriate caregivers; secure storage of classes in use; preparation and administration of chemotherapy or radioactive agents only where the class is in use and only by the named persons; a usage, administration and disposal record that matches the cupboard; and no diagnostic-radiology licence offered as MOM.8 radioactive-agent evidence.

This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a class was used that the directory does not define, a narcotic was found outside secure storage, a destruction was unrecorded, or a copied chemo SOP was found in a hospital that does not give chemotherapy, or when the storage, administration, sedation, anaesthesia, imaging or infection-control policies that this document hands work to are revised.$s$
  ],
  $q$The head of the institution is accountable for {{HOSPITAL_NAME}} using only the named classes, storing them securely, and keeping the usage, administration and disposal record, and for recorded absence where a class is not a defined service.

The named lead for these three classes authors and keeps current the written safe-use guidance at step 1, the authorised-prescriber list at step 2, the secure-storage arrangement at step 3, the qualified-person lists at step 4 where those classes are used, and the register method at step 5. The named lead is [Hospital to define — the named lead for narcotic drugs and psychotropic substances, chemotherapeutic agents and radioactive agents].

Prescribers of these classes prescribe only if they are on the appropriate-caregiver list. Staff who hold access to secure storage do not leave that access unattended. Persons who prepare or administer chemotherapy or radioactive agents do so only if named at step 4.
...
All staff are expected to treat an unlocked narcotic, a chemotherapeutic preparation by an unnamed person, and a copied bunker for a class this hospital does not use, as defects, and to report them.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Management of Medication chapter, standard MOM.8.
...
Several requirements in this document are statutory rather than advisory — in particular those arising under the Narcotic Drugs and Psychotropic Substances Act, 1985, the Drugs and Cosmetics Act, 1940 insofar as chemotherapeutic agents are medicines, and the Atomic Energy Act, 1962 and the Atomic Energy (Radiation Protection) Rules, 2004 insofar as this hospital uses radioactive agents as therapy. Statutory requirements change, and State authorities impose additional or stricter conditions. {{HOSPITAL_NAME}} is responsible for verifying the current text of any rule cited here and the conditions attached to its own authorisations and licences; this document does not constitute legal advice.
...
This document is not issued by, endorsed by, or affiliated with NABH, the World Health Organization, the National Centre for Disease Control, the Food Safety and Standards Authority of India, any Pollution Control Board, or any other body named in it. Wording is original; no text has been reproduced from the standards, rules or guidelines referenced.$q$,
  $q$[{"oe_code": "MOM.8.a", "requirement": "Narcotic drugs and psychotropic substances, chemotherapeutic agents and radioactive agents are used safely.", "steps": "Steps 1, 6", "evidence": "The written safe-use guidance naming which of the three classes this hospital actually uses, with a recorded absence against the service directory for any class it does not use, showing that absence is a record and not a copied cytotoxic bunker, hot laboratory or nuclear-medicine SOP invented for assessment, and showing that narcotic drugs and psychotropic substances remain in scope if any such medicine is used (as they almost always are) because the Narcotic Drugs and Psychotropic Substances Act, 1985 applies to that use; the recorded acceptance of the handoff from the procedural-sedation and anaesthesia policies, that those policies own the clinical act and that cupboard, register and destruction of narcotic drugs and psychotropic substances live in this document, not in COP.9 or COP.10; the recorded distinction that diagnostic radiology licences and imaging radiation-safety devices remain under the imaging and laboratory-and-imaging-safety policies and are not evidence of radioactive agents used as therapy; the recorded refusal to print NDPS schedule tables, destruction methods as numbered mandates, or chemotherapeutic doses in this document, those values being hospital-defined at the cupboard, qualified-preparer and register steps; the location where the written guidance is held; induction or briefing records showing staff in locations that hold these classes have been shown the named classes and the absences; the audit sample at step 6 of classes in use matching the service directory and unused classes recorded as absent rather than documented as a bunker that does not exist", "responsible": "Named lead holds the written safe-use guidance, named classes and recorded absences; head of the institution is accountable that unused classes are not invented as facilities; quality or accreditation coordinator audits"}, {"oe_code": "MOM.8.b", "requirement": "Narcotic drugs and psychotropic substances, chemotherapeutic agents and radioactive agents are prescribed by appropriate caregivers.", "steps": "Steps 2, 1, 6", "evidence": "The named appropriate caregivers who may prescribe each class in use, with current professional registration used from human-resource verification and any further hospital authorisation for that class; sample prescriptions showing a person not on that list did not prescribe; the recorded absence for a class not in use; the recorded division that MOM.3/MOM.4 own how the order is written and this step owns who may write it for these classes", "responsible": "Named lead holds the appropriate-caregiver lists; prescribers apply them; human resource function verifies registration; quality or accreditation coordinator audits"}, {"oe_code": "MOM.8.c", "requirement": "Narcotic drugs and psychotropic substances, chemotherapeutic agents and radioactive agents drugs are stored securely.", "steps": "Steps 3, 1, 6", "evidence": "The written secure-storage arrangement for each class in use (cupboard or location, who holds access, how access is recorded), showing storage stricter than ordinary medication storage and able to satisfy the Narcotic Drugs and Psychotropic Substances Act, 1985 for narcotics and psychotropics in use; the recorded absence of a cupboard for a class not in use; the recorded distinction that diagnostic-radiology rooms are not this storage; the audit sample at step 6 of secure storage of classes in use", "responsible": "Named lead holds the secure-storage arrangement; staff who hold access apply it; MOM.2 owns ordinary medication storage; quality or accreditation coordinator audits"}, {"oe_code": "MOM.8.d", "requirement": "Chemotherapy and radioactive agents are prepared properly and safely, and administered by qualified personnel.", "steps": "Steps 4, 1, 6", "evidence": "The named persons who may prepare and who may administer chemotherapy and radioactive agents used as therapy, and where preparation is done, or the recorded absence if the class is not used; sample preparation and administration records against the unique identification number where the class is in use; the recorded application of the infection-prevention policy for PPE at preparation without rewriting it, and of the biomedical-waste policy for cytotoxic waste without restating colours; records showing a chemotherapeutic dose in the operation theatre was still prepared and administered under this step; the audit sample at step 6 of preparation and administration only where the class is in use and only by the named persons", "responsible": "Named lead holds the qualified-person lists where those classes are used; named preparers and administrators apply them; HIC.2 owns PPE method; HIC.3 owns the waste stream; COP.11 owns the surgical act; quality or accreditation coordinator audits"}, {"oe_code": "MOM.8.e", "requirement": "A proper record is kept of the usage, administration and disposal of narcotic drugs and psychotropic substances, chemotherapeutic agents and radioactive agents.", "steps": "Steps 5, 1, 6", "evidence": "The written method for recording usage, administration and disposal of each class in use (register or other record, contents of an entry, who writes it), showing a record that can satisfy the Narcotic Drugs and Psychotropic Substances Act, 1985 for narcotics and psychotropics without a destruction method printed as a numbered mandate; sample entries matching the cupboard; the recorded absence of a register for a class not in use; the recorded distinction that cytotoxic waste colours remain under the biomedical-waste policy while the account of a given dose is this record; the audit sample at step 6 of a usage, administration and disposal record that matches the cupboard", "responsible": "Named lead holds the register method; staff who use, administer or dispose of a class in use write the entry; quality or accreditation coordinator audits"}]$q$::jsonb,
...
1. Two-tier depth standing rule of 2026-08-10 applies. ONE OF FIVE OEs IS TIER 1. Tier 1: MOM.8.a only -- procedure step 1 carries the reasoning (why a copied bunker for an unused class is not safe use, why NDPS still applies if any narcotic or psychotropic is used, why this document accepts the COP.9/COP.10 storage handoff). Tier 2: MOM.8.b (step 2), MOM.8.c (step 3), MOM.8.d (step 4), MOM.8.e (step 5) -- requirement and method without extended rationale. Reviewer to note the shallower treatment of b-e is a DECISION UNDER THE STANDING RULE, not an omission.
...
2. Tier 1 cross-check (2026-08-17) of MOM.8.a against the approved HIC masters and the AAC/COP drafts. Search terms: NDPS, narcotic, psychotropic, chemotherapy, cytotoxic, radioactive, AERB, cupboard, controlled drug.
   ...
   MOM.7 -- NM/ME/ADR capture remains MOM.7 even when the drug is one of these classes.
   COP.11 -- chemo in OT is still MOM.8 preparation/qualified person; surgical act is COP.11. Stated in Scope and step 4.
   AAC.5/AAC.6 -- diagnostic radiology licences and imaging radiation safety stay there; MOM.8 radioactive AGENTS are therapy (I-131 etc.). If none, record absence. Stated in Scope, Policy statement and step 1.
   ...
   HIC.3 -- cytotoxic/chemo waste colours pointed at, not restated, not in P2. Stated in Scope and steps 4-5.
   HIC.2 -- PPE for chemo prep applied, not rewritten. Preparation safety is MOM.8.d. Stated in Scope and step 4.
3. FORWARD REFERENCES: MOM.2 storage; MOM.3/4/5/6 siblings; AAC.5/6 imaging; PRE/HRM/IMS. Each is a deliberate boundary.
4. T2 QUICK CHECK: MOM.8.b vs MOM.3 -- flagged. MOM.8.c vs MOM.2 -- flagged. MOM.8.d vs MOM.6 and HIC.2 -- flagged. MOM.8.e vs MOM.5 and HIC.3 -- flagged. Nothing added to the HIC reconciliation list.

STATUTORY AND EXTERNAL FACTS
5. Narcotic Drugs and Psychotropic Substances Act, 1985 -- named in P2 and References. Cupboard, register and destruction are hospital-defined; this draft does NOT print schedule tables or a numbered destruction method. No section number.
...
8. NO NUMBERS ARE STATED as requirements -- no lock counts, no chemo doses, no destruction recipes, no schedule lists. Every such value is [Hospital to define].
...
13. Step 4's statement that narcotics are not reconstituted as chemotherapy, and that chemo in the OT is still this step, are editorial positions.
...
- PPE donning for chemo prep -- HIC.2.
