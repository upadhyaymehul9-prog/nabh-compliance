-- AAC.2 master policy -- UNAPPROVED DRAFT for review.
...
--
-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 1, printed pages 50-51
...
insert into public.shco_policy_masters (
  ...
  'AAC.2',
  'AAC',
  array['AAC.2.a', 'AAC.2.b', 'AAC.2.c', 'AAC.2.d', 'AAC.2.e', 'AAC.2.f'],
  $q$Registration, Admission and Transfer of Patients$q$,
  $q$This document sets out how {{HOSPITAL_NAME}} registers a patient, decides whether to admit, manages the patient when a bed is not available, prioritises access according to clinical need, and transfers or refers a patient whose needs this hospital cannot meet.
...
- Isolation and transmission-based precautions applied to an admitted patient are governed by the infection-control policies of {{HOSPITAL_NAME}}.$q$,
  $q${{HOSPITAL_NAME}} registers every patient through a defined mechanism and issues a unique identification number at the end of registration. That number identifies the patient in every subsequent record.
...
{{HOSPITAL_NAME}} treats directing a patient to another provider, when this hospital cannot care for them, as correct practice and never as lost business.$q$,
  array[
    $s$1. Registering a patient
...
The registration mechanism, the forms or electronic fields, and the named roles that operate them are held at [Hospital to define — where the registration SOP and forms are held].$s$,
    $s$2. The unique identification number
...
The medical record that this number keys is governed by the information-management policies of {{HOSPITAL_NAME}}. This step owns only the generation of the number at registration and the requirement that it travel with the patient.$s$,
    $s$3. Accepting a patient only when {{HOSPITAL_NAME}} can provide the service
...
Patients and families are told, at admission, where the patient is going and who is responsible for them on arrival. An admitted patient is accompanied to the allocated area; they are not sent to find it.$s$,
    $s$5. Managing patients when beds are not available
...
Life-threatening emergency is the highest category and is not queued. The method does not displace the rule at step 3 that a patient whose needs this hospital cannot meet is stabilised and transferred rather than admitted into a service that does not exist.$s$,
    $s$7. Transfer-in
...
A patient who arrives without prior acceptance is treated as an emergency or walk-in under steps 1, 3 and 6, not as a transfer the hospital is obliged to complete.$s$,
    $s$8. Transfer-out and referral
...
This step owns movement between organisations. Movement of a patient from one unit of {{HOSPITAL_NAME}} to another is governed by the continuity-of-care policy and is not performed under this step.$s$,
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when the service directory changes, when a transfer or a bed-non-availability incident exposes a gap, or when the emergency-care or continuity-of-care policies that this document hands work to are revised.$s$
  ],
  $q$The head of the institution is accountable for {{HOSPITAL_NAME}} accepting no patient it cannot care for, for the existence of a working bed-management mechanism, and for transfer arrangements that do not abandon a patient this hospital cannot admit.
...
All staff are expected to treat a referral or a transfer, when this hospital cannot care for the patient, as correct practice, and to report a registration, a bed wait or a transfer that was improvised rather than performed under this policy.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Access, Assessment and Continuity of Care chapter, standard AAC.2.
