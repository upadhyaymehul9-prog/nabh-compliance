-- PRE.5 master policy -- UNAPPROVED DRAFT for review.
...
insert into public.shco_policy_masters (
  ...
  'PRE.5',
  'PRE',
  array['PRE.5.a', 'PRE.5.b', 'PRE.5.c', 'PRE.5.d'],
  $q$Information on Expected Costs$q$,
  $q$This document sets out how {{HOSPITAL_NAME}} makes patients and families aware of the pricing policy in different settings; how the relevant tariff list is available to patients; how expected costs are explained; and how they are informed of the financial implications when the care plan changes.
...
This document is patient-facing cost information. It is not the billing ledger, the accounts process, or a facility-management tariff file. Those remain ROM/FMS when drafted.$q$,
...
- Informed consent (PRE.3) is not a substitute for cost explanation, and a signed consent is not agreement to an unnamed bill.$q$,
  $q${{HOSPITAL_NAME}} makes the patient and/or family members aware of the pricing policy in out-patient, emergency, ICU (where it exists) and in-patient settings.
...
{{HOSPITAL_NAME}} does not treat a signed consent, or a tariff locked in accounts, as a substitute for that explanation.$q$,
  array[
    $s$1. Pricing policy in different settings
...
This step does not print rupee figures as a NABH mandate.$s$,
    $s$2. Relevant tariff list available to patients
...
Where the State has adopted the Clinical Establishments Act, 2010, the Clinical Establishments (Central Government) Rules, 2012 display-of-rates requirements inform this availability. They are not pasted as a board layout, and they are not applied as if the State had adopted the Act when it has not. Whether this hospital's State has adopted that Act, and how display then meets those rules, are [Hospital to define — whether the Clinical Establishments Act, 2010 has been adopted in this State, and how tariff display meets the 2012 Rules if it has].$s$,
    $s$3. Expected costs explained
...
The Consumer Protection Act, 2019 informs that a consumer of a healthcare service is entitled not to be misled on price. This document does not copy District Commission procedure.$s$,
    $s$4. Financial implications when the care plan changes

Patient and/or family are informed about the financial implications when there is a change in the care plan.

How that information is given when the plan changes, and how it is recorded, are [Hospital to define — how patients and families are informed of the financial implications of a care-plan change, and how that is recorded]. A change that does not change cost is recorded as such if the hospital's method requires it. A change that increases cost and is not told is a defect. Clinical modification of the plan remains AAC.3 / PRE.2.o; this step is the money.$s$,
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a family was billed against a plan they were not told had changed, or a tariff was only in accounts, or when ROM/FMS billing policies or PRE.2 that this document hands work to are revised.$s$
  ],
  $q$The head of the institution is accountable for {{HOSPITAL_NAME}} telling patients and families the pricing policy, making the tariff available, explaining expected costs, and telling them when a care-plan change changes the bill.
...
All staff are expected to treat a hidden tariff and an unexplained increase when the plan changed, as defects, and to report them.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Patient Rights and Education chapter, standard PRE.5.
...
- Internal documents of {{HOSPITAL_NAME}}: the pricing policy; the tariff list and where it is available; the expected-cost explanation method; the care-plan-change financial-implication method; the beliefs-values policy; the consent policy; the assessment policy; and ROM/FMS billing policies when drafted.$q$,
...
Several requirements in this document are statutory rather than advisory — in particular those arising under the Consumer Protection Act, 2019, insofar as a consumer of a healthcare service is entitled not to be misled on price, and the Clinical Establishments Act, 2010 and the Clinical Establishments (Central Government) Rules, 2012, insofar as the State has adopted that Act and those rules require display of rates. Statutory requirements change, and State authorities impose additional or stricter conditions. {{HOSPITAL_NAME}} is responsible for verifying the current text of any rule cited here and the conditions attached to its own authorisations and licences; this document does not constitute legal advice.
...
This document is not issued by, endorsed by, or affiliated with NABH, the World Health Organization, the National Centre for Disease Control, the Food Safety and Standards Authority of India, any Pollution Control Board, or any other body named in it. Wording is original; no text has been reproduced from the standards, rules or guidelines referenced.$q$,
  $q$[{"oe_code": "PRE.5.a", "requirement": "The patient and/or family members are made aware of the pricing policy in different settings (out-patient, emergency, ICU and in-patient).", "steps": "Steps 1, 5", "evidence": "The written pricing policy for each setting this hospital runs, or a recorded absence for a setting it does not (for example ICU); the method of making patients and families aware; sample records of awareness at those settings, including delayed emergency awareness with reason; the audit sample at step 5", "responsible": "Named lead holds the pricing policy; staff at those settings make families aware; quality or accreditation coordinator audits"}, {"oe_code": "PRE.5.b", "requirement": "The relevant tariff list is available to patients.", "steps": "Steps 2, 1, 5", "evidence": "Where the relevant tariff list is available to patients in each setting, showing a route that is not only an accounts-office escort; the recorded decision whether CEA 2010 is adopted in this State and how display meets the 2012 Rules if it is, rather than a copied CEA board in a State that has not adopted the Act; the audit sample at step 5 of a list a patient can actually see", "responsible": "Named lead holds the tariff-availability method; finance produces the list; quality or accreditation coordinator audits"}, {"oe_code": "PRE.5.c", "requirement": "The patient and/or family members are explained about the expected costs.", "steps": "Steps 3, 1, 5", "evidence": "The written explanation method (when, by whom, how recorded); sample explanations against the unique identification number showing a range or package with inclusions/exclusions rather than only 'charges extra'; the recorded statement that a signed consent is not this explanation; the audit sample at step 5", "responsible": "Named staff explain expected costs; named lead holds the method; quality or accreditation coordinator audits"}, {"oe_code": "PRE.5.d", "requirement": "Patient and/or family are informed about the financial implications when there is a change in the care plan.", "steps": "Steps 4, 5", "evidence": "The written method for informing of financial implications when the care plan changes; sample records of plan changes that changed cost being told; the recorded split that AAC.3/PRE.2.o own the clinical modification; the audit sample at step 5", "responsible": "Staff who know the plan changed cost tell the family; clinicians own the clinical change; quality or accreditation coordinator audits"}]$q$::jsonb,
