-- AAC.5 master policy -- UNAPPROVED DRAFT for review.
...
--
-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 1, printed pages 52-53
...
An imaging report that cannot be joined to a registered patient, that arrives after the clinical decision has already been made, or that is issued from a machine whose required quality-assurance test is overdue, is a document that looks like care and is not.$q$,
  $q$This policy applies to every imaging modality {{HOSPITAL_NAME}} actually provides, and to every location from which an imaging study is performed, reported or outsourced. It binds the clinicians who report, the technicians who acquire the images, the person who owns the imaging-licence calendar, the person who notifies a critical imaging finding, and whoever arranges outsourcing. It does not assume that {{HOSPITAL_NAME}} provides every modality: computed tomography, magnetic resonance imaging, interventional radiology and nuclear medicine appear in this document only where the defined scope includes them.
...
- The medical record itself is governed by the information-management policies of {{HOSPITAL_NAME}}. This policy owns the imaging-report content that is written into that record.$q$,
  $q${{HOSPITAL_NAME}} provides imaging services that comply with legal and other requirements attaching to the modalities it actually performs. Licences and authorisations are current. A named person owns the calendar on which they are kept current.
...
A quality-assurance programme for imaging is implemented. It includes reject and repeat analysis, equipment-performance checks, a named lead, periodic peer review of protocols and results, and periodic calibration and maintenance of all equipment. A report is not issued from equipment overdue for a required quality-assurance test.$q$,
  array[
    $s$1. Legal compliance of imaging services
...
This calendar is the imaging-licence calendar. Consent to establish or operate under pollution-control law is not entered on it.$s$,
    $s$2. Scope of imaging commensurate with the services provided
...
When the service directory changes — a specialty added, a service withdrawn — the imaging scope is checked in the same pass. A new clinical service that depends on a study this hospital cannot perform is not opened until the study is provided or outsourced. A withdrawn clinical service does not leave an orphaned imaging claim on the directory or the display.$s$,
    $s$3. Infrastructure and human resources
...
Verification of qualifications sits with the human-resource policies of {{HOSPITAL_NAME}}. Adequacy for the defined scope sits here.$s$,
    $s$4. Standardised results within a defined time
...
The report template and the place it is held are [Hospital to define — where the standardised imaging-report template is held].$s$,
    $s$5. Critical imaging results intimated immediately
...
If the responsible clinician cannot be reached, the finding is escalated along the out-of-hours route rather than left as a message. The patient remains the reason the call is made.$s$,
    $s$6. Outsourcing imaging tests that are not available here
...
A verbal arrangement with a nearby centre, or a centre chosen because it is convenient rather than because its quality-assurance system was examined, is not outsourcing under this policy.$s$,
    $s$7. Quality-assurance programme for imaging
...
Quality assurance of imaging is not the same as a biomedical service contract. A contract that attends when something breaks is breakdown maintenance. This programme is the continuing look at whether the service is producing usable, attributable, timely results from equipment that is fit to produce them.$s$,
    $s$8. Peer review of imaging protocols and results
...
The American College of Radiology RADPEER programme is listed as chapter reference 14 of the AAC chapter and may be used as a method. It is not mandated. A hospital that uses another documented peer-review method, internal or external, with a stated sample, meets this step. A hospital that names RADPEER in a policy and does not sample does not.$s$,
    $s$9. Calibration and maintenance of imaging equipment
...
This step does not replace the hospital-wide equipment programme. It is the imaging-specific rule that programme must satisfy for these machines.$s$,
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a modality is added or withdrawn, when an authorisation is varied, when a critical-result incident exposes a gap, or when the laboratory-and-imaging-safety policy that this document hands safety to is revised.$s$
  ],
  $q$The head of the institution is accountable for {{HOSPITAL_NAME}} providing no imaging service it is not licensed to provide, for the imaging scope matching the services the hospital has defined, and for reports not being issued from equipment overdue for a required quality-assurance test.
...
All staff are expected to treat a missing unique identification number, a critical finding that was not notified, or a report issued from overdue equipment, as a defect, and to report it.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Access, Assessment and Continuity of Care chapter, standard AAC.5.
...
This document is not issued by, endorsed by, or affiliated with NABH, the World Health Organization, the National Centre for Disease Control, the Food Safety and Standards Authority of India, any Pollution Control Board, or any other body named in it. Wording is original; no text has been reproduced from the standards, rules or guidelines referenced.$q$,
  $q$[{"oe_code": "AAC.5.a", "requirement": "Imaging services comply with legal and other requirements", "steps": "Steps 1, 10", "evidence": "The imaging-licence compliance calendar naming the owner; current AERB authorisation for each radiation-generating equipment in use; PC-PNDT registration and statutory records where ultrasonography capable of sex determination is performed; clinical-establishment or nursing-home registration consistent with the imaging scope, where that law applies; the audit sample at step 10 of licences still current", "responsible": "The named calendar owner keeps licences current; head of the institution is accountable that no unlicensed imaging is provided; quality or accreditation coordinator audits"}, {"oe_code": "AAC.5.b", "requirement": "Scope of the imaging services is commensurate to the services provided by the organization", "steps": "Steps 2, 10", "evidence": "The current imaging department scope of services set against the service directory; the record of the match check when the directory last changed; the list of modalities provided here versus outsourced under step 6", "responsible": "Head of imaging authors the scope against the directory; administration maintains the directory under the definition-and-display policy; quality or accreditation coordinator audits"}, {"oe_code": "AAC.5.c", "requirement": "The infrastructure (physical and equipment) and human resources are adequate to provide for its defined scope of services", "steps": "Steps 3, 10", "evidence": "The equipment inventory for the defined imaging scope; the staffing establishment by role with the out-of-hours reporting arrangement; the location of imaging as used", "responsible": "Head of imaging and administration keep space, equipment and staff matched to the scope; human-resource policies verify qualifications; quality or accreditation coordinator audits"}, {"oe_code": "AAC.5.d", "requirement": "Imaging results are available in the standardised manner within a defined time frame", "steps": "Steps 4, 10", "evidence": "The written standardised imaging-report template requiring, on every report including an outsourced report received back, the patient's name and unique identification number matching the registration record, the date and time of the study and of the report, the modality and region examined, the clinical indication taken from the requisition, the findings, the impression or conclusion, and the name and designation of the person who reports; the written categories against which turnaround time is defined, distinguishing urgency rather than applying a single hospital-wide average, and the stated turnaround time for each category measured from completed acquisition (or from receipt of the outsourced report) to the attributable report being available to the requesting clinician; sample reports across categories showing every standard element present and the unique identification number matching registration; sample timed records showing the acquisition (or receipt) time and the release time against the stated turnaround time for that category, including at least one emergency or urgent category where that category exists; records showing a report missing a standard element, or released into a system the requester cannot see, was treated as incomplete rather than as having met the time; the place the current template is held; induction or briefing records showing reporting clinicians have been shown the template and the times; the audit sample at step 10 of reports for standard elements and for turnaround time actually met", "responsible": "Reporting clinicians issue the standardised report within the defined time; technicians complete acquisition so that the clock can start; whoever receives an outsourced report releases it in the same form; quality or accreditation coordinator audits completeness and time"}, {"oe_code": "AAC.5.e", "requirement": "Critical results are intimated immediately to the personnel concerned", "steps": "Steps 5, 10", "evidence": "The written list of imaging findings treated as critical at this hospital, dated and approved, held separately from the laboratory critical-value list, with a recorded statement that the examples used in training are illustrations and not a substitute for the hospital's list; the written notification method naming who notifies, how (a method that produces a person at the other end, not a system release), and the out-of-hours escalation route when the responsible clinician cannot be reached; the instruction that notification is immediate — before the routine report queue — and that read-back of patient identity (name and unique identification number) and of the finding is required; the critical-imaging-result register with unique identification number, finding, date and time of the study, date and time of notification, person who notified, person who received, and that read-back occurred; sample register entries spanning in-hours and out-of-hours, each showing a named recipient rather than a system-release note; records of any finding notified that was not on the list, with the reporting clinician's judgement that it could not wait; briefing records of reporting clinicians and of the staff who may receive a notification; the audit sample at step 10 of critical findings against the list and against register entries that show intimation with read-back rather than filing", "responsible": "Reporting clinicians identify, notify immediately and record read-back; the named notifier and the out-of-hours route operate when the reporter is not the caller; receiving clinicians confirm identity and finding; quality or accreditation coordinator audits the register against the list; laboratory-services policy owns the separate laboratory critical-value pathway"}, {"oe_code": "AAC.5.f", "requirement": "Imaging tests not available in the organization are outsourced to organization(s) based on their quality assurance system", "steps": "Steps 6, 10", "evidence": "The written list of imaging tests this hospital outsources, mapped to the defined imaging scope and to the clinical services that depend on them; the named receiving organisation for each test; the quality-assurance evidence held before the first study was sent — current AERB authorisation where the receiver uses radiation-generating equipment, NABH or other documented healthcare accreditation the hospital has recorded as acceptable, or a quality-assurance system the hospital itself examined (equipment quality-assurance records, reporting arrangements, turnaround times, critical-result pathway) — and the record that a laboratory-accreditation certificate was not treated as sufficient evidence for an imaging centre; the interval at which that evidence is re-examined and the last re-examination; sample requisitions travelling with the outsourced study showing unique identification number, indication and the question asked; sample reports received back issued to the requesting clinician in the standardised manner within the turnaround time for that category, with any critical finding intimated under step 5; the audit sample at step 10 of outsourced work sitting against examined quality-assurance evidence rather than against convenience", "responsible": "Head of imaging decides what is outsourced and on what quality-assurance evidence; whoever arranges outsourcing holds the file and does not send to a centre the evidence does not support; reporting path and critical-result path remain this hospital's after the report returns; quality or accreditation coordinator audits"}, {"oe_code": "AAC.5.g", "requirement": "The quality assurance programme for imaging services is implemented", "steps": "Steps 7, 8, 9, 10", "evidence": "The written imaging quality-assurance programme naming the lead by role, stating that the programme is the continuing look at usable, attributable, timely results from equipment fit to produce them, and distinguishing that look from a biomedical breakdown-maintenance contract; reject and repeat records with unique identification number, modality, reason (patient factors, positioning, exposure or equipment) and the review of those records at the stated interval with the action taken; equipment-performance check records for each modality in the defined scope, with a recorded failure removing the unit from reporting use until restoration under step 9; the peer-review sampling records at step 8; the calibration and required quality-assurance-test records at step 9; the named lead's review notes showing the programme was opened at the stated intervals rather than held as an unused file; briefing records of technicians and reporting clinicians on recording rejects and repeats; the audit sample at step 10 of reject/repeat review actually occurring at the stated interval and of equipment-performance failures that stopped reporting", "responsible": "The named quality-assurance lead runs the programme; technicians record rejects, repeats and performance checks; reporting clinicians and the head of imaging act on findings; biomedical engineering restores equipment under step 9; quality or accreditation coordinator audits implementation rather than the existence of a folder"}, {"oe_code": "AAC.5.h", "requirement": "The programme addresses periodic internal / external peer review of imaging protocols and results using appropriate sampling", "steps": "Steps 8, 10", "evidence": "The written sampling method and interval for peer review of imaging protocols and results, internal or external; sample review records showing protocol appropriateness and report adequacy, with disagreements fed to the quality-assurance lead; where RADPEER is used, the records of that use — RADPEER is optional", "responsible": "The named quality-assurance lead operates the sample; reporting clinicians participate; quality or accreditation coordinator audits that sampling occurred"}, {"oe_code": "AAC.5.i", "requirement": "The programme includes periodic calibration and maintenance of all equipment", "steps": "Steps 9, 10", "evidence": "The written calibration and maintenance plan for every imaging unit in the defined-scope inventory, stating for each class of equipment the interval for calibration, for preventive maintenance, and for the quality-assurance tests the AERB authorisation requires where they apply; the record on each unit of the last such test, the next due date and the result; the rule, evidenced in use, that a due date which has passed without a recorded test takes that unit out of reporting use until the test is done and the unit is restored — sample of a unit taken out of use, or a recorded statement that no unit was overdue in the period, either of which shows the rule is applied rather than written; breakdown-maintenance records; records showing a unit that could not meet the defined scope was treated as a change to capability under the definition-and-display policy until restored; the interface with the hospital-wide equipment programme (inventory and logs held there; imaging-specific AERB tests and the no-report-if-overdue rule held here); the audit sample at step 10 of equipment that was not used to report while overdue", "responsible": "Biomedical engineering performs or coordinates calibration and maintenance and informs imaging when a unit is overdue or restored; the named quality-assurance lead and head of imaging refuse reports from overdue equipment; facility-management policies own the hospital-wide equipment programme; quality or accreditation coordinator audits the due-date rule in use"}]$q$::jsonb,
