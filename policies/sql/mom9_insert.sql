-- MOM.9 master policy -- UNAPPROVED DRAFT for review.
...
--
-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 3, printed page 81
...
insert into public.shco_policy_masters (
  ...
  'MOM.9',
  'MOM',
  array['MOM.9.a', 'MOM.9.b', 'MOM.9.c'],
  $q$Implantable Prosthesis and Medical Devices$q$,
  $q$This document sets out how {{HOSPITAL_NAME}} uses implantable prostheses and medical devices in accordance with laid-down criteria: written guidance that addresses procurement and usage of the implants this hospital actually uses; counselling of the patient and family for usage and precautions; and recording of batch and serial number in the patient's medical records, the master logbook and the discharge summary.
...
MOM.1.e of this chapter points at the same subject. This document is the owning criteria. The multidisciplinary-committee policy of {{HOSPITAL_NAME}} does not rewrite them.$q$,
  $q$This policy applies to every location at {{HOSPITAL_NAME}} in which an implantable prosthesis or an implantable medical device is procured, stored pending use, counselled for, implanted, or recorded: the operation theatre and procedure rooms, the wards that receive the patient afterwards, the stores or pharmacy function that receives the device, and the records function that holds the master logbook and produces the discharge summary. It binds the clinicians who select and implant, the staff who procure and hold stock, the person who counsels the patient and family, and whoever writes batch and serial number into the three places this standard names.
...
- Generation of the unique identification number is governed by the registration and admission policy of {{HOSPITAL_NAME}} (AAC.2). This policy requires that number on the implant record; it does not issue the number.$q$,
  $q${{HOSPITAL_NAME}} uses implantable prostheses and medical devices in accordance with written criteria that address procurement and usage of the types it actually uses. A type that is not a defined service is not implanted. {{HOSPITAL_NAME}} is not a manufacturer of those devices.
...
{{HOSPITAL_NAME}} procures implantable devices from a source licensed under the Medical Devices Rules, 2017, read with the Drugs and Cosmetics Act, 1940. This hospital does not become a manufacturer by implanting.$q$,
  array[
    $s$1. Written guidance addresses procurement and usage of implantable prostheses
...
The written procurement-and-usage guidance is held at [Hospital to define — where the written guidance on procurement and usage of implantable prostheses is held].$s$,
    $s$2. Patient and family are counselled for usage and precautions
...
A device without a serial number as issued is recorded with the identifiers it carries (batch and whatever unique identifier the licensed device supplies). This document does not invent a serial number the manufacturer did not give. It requires that whatever batch and serial (or equivalent unique device identifier) the device carries is written in all three places.$s$,
    $s$4. Records, review and the order of operations
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when an implant type not in the directory was used, a discharge summary omitted the numbers, a device arrived from an unlicensed source, or counselling was treated as surgical consent, or when the discharge, surgical, sterilisation or multidisciplinary-committee policies that this document hands work to are revised.$s$
  ...
  $q$The head of the institution is accountable for {{HOSPITAL_NAME}} implanting only the types it has defined, procuring them from a licensed source, counselling for usage and precautions, and writing batch and serial number in all three places.
...
All staff are expected to treat an undefined implant type, a missing serial in the discharge summary, and counselling used as surgical consent, as defects, and to report them.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Management of Medication chapter, standard MOM.9.
...
- Internal documents of {{HOSPITAL_NAME}}: the written procurement-and-usage guidance; the licensed-source evidence; the counselling record; the implant master logbook; the service directory; the discharge policy; the procedures-and-operation-theatre policy; the sterilisation policy; the multidisciplinary-committee policy (MOM.1.e pointer); and the patient-rights policies.$q$,
