-- MOM.7 master policy -- UNAPPROVED DRAFT for review.
...
--
-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 3, printed pages 79-80
...
insert into public.shco_policy_masters (
  ...
  'MOM.7',
  'MOM',
  array['MOM.7.a', 'MOM.7.b', 'MOM.7.c', 'MOM.7.d', 'MOM.7.e'],
  $q$Monitoring after Medication Administration and Medication-Event Reporting$q$,
  $q$This document sets out how {{HOSPITAL_NAME}} monitors patients after a medicine has been given, changes that medicine where the monitoring shows it should be changed, and captures, reports and analyses near miss, medication error and adverse drug reaction within a time this hospital has specified, with corrective or preventive action from that analysis.
...
This document is not the administration policy. Giving the dose, labelling a prepared syringe, identifying the patient, verifying the order and writing that the dose was given are owned by the administration policy of {{HOSPITAL_NAME}}. This document starts when that dose has been given.$q$,
  $q$This policy applies to every location at {{HOSPITAL_NAME}} in which a medicine is administered and the patient remains under this hospital's care afterwards: in-patient wards, the emergency area, day-care, the operation theatre and recovery insofar as a medicine given there is watched after the dose, intensive and high-dependency areas where they exist, out-patient areas where a medicine is given and the patient is observed before leaving, and any other clinical area this hospital names. It binds the person who gave the dose, the person who watches the patient after it, the clinician who changes therapy on the basis of that watching, and the staff who capture, report and analyse near miss, medication error and adverse drug reaction.
...
- Two identifiers at the point of care are governed by the uniform-care policy of {{HOSPITAL_NAME}} (COP.1). The patient who is watched after a dose is the patient those identifiers named when the dose was given.$q$,
  $q${{HOSPITAL_NAME}} monitors patients after medication administration for the effect of the medicine and for harm from the medicine. A signed administration record is not that monitoring.
...
{{HOSPITAL_NAME}} does not treat a transfusion reaction as an adverse drug reaction under this policy, and does not treat a healthcare-associated infection or an occupational exposure as a medication event under this policy.$q$,
  array[
    $s$1. Patients are monitored after medication administration
...
The written post-medication monitoring guidance is held at [Hospital to define — where the written post-medication monitoring guidance is held].$s$,
    $s$2. Medications are changed where appropriate based on the monitoring
...
Where the monitoring shows harm that is an adverse drug reaction, the medicine is stopped or changed as clinically required and the event is captured under step 3. This step owns the therapeutic change. Step 3 owns the capture.$s$,
    $s$3. Capture of near miss, medication error and adverse drug reaction
...
Staff are expected to capture. The chapter intent is that staff are empowered to report errors. A capture used as punishment, or a capture that only senior staff are allowed to file, is a defect of this step.$s$,
    $s$4. Reporting and analysis within a specified time frame
...
Analysis looks at what happened, why it was possible, and what should change. It uses the captured record, not a recollection. It does not wait for harm as a condition of being analysed: near miss is in the objective element.$s$,
    $s$5. Corrective and/or preventive action based on the analysis
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a medicine was given and not watched, a near miss that staff described was not captured, an analysis missed its specified time frame, or a transfusion reaction was filed here, or when the administration, transfusion, assessment, intensive-care or infection-control policies that this document hands work to are revised.$s$
  ...
  $q$The head of the institution is accountable for {{HOSPITAL_NAME}} monitoring patients after medication administration and for capturing, reporting and analysing near miss, medication error and adverse drug reaction.
...
All staff are expected to treat an unwitnessed dose, an uncaptured near miss, and a transfusion reaction filed as an adverse drug reaction, as defects, and to report them.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Management of Medication chapter, standard MOM.7.
