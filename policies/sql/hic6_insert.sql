-- HIC.6 master policy -- DRAFT for review. Do NOT set status = 'approved' here;
...
insert into public.shco_policy_masters (
  ...
  'HIC.6',
  'HIC',
  array['HIC.6.a', 'HIC.6.b', 'HIC.6.c', 'HIC.6.d', 'HIC.6.e'],
  $q$Sterilisation and Disinfection of Instruments, Equipment and Devices$q$,
  $q$This document sets out how {{HOSPITAL_NAME}} makes a used instrument safe to use on the next patient. It governs the whole of that journey — the treatment an item receives at the moment it is put down, its transport, its cleaning, its inspection, its packaging, the process that disinfects or sterilises it, the conditions it is stored in, the check made before it is used again, and what happens when any of that fails.
...
The other infection control policies of {{HOSPITAL_NAME}} rely on this one. Three of them state in terms that instrument reprocessing is governed here. This document is where that promise is met.$q$,
  $q$This policy applies to every reusable instrument, item of equipment and device used on a patient of {{HOSPITAL_NAME}}, wherever it is processed — in a central sterile supply department, in an operating theatre sub-sterile area, in a ward or clinic treatment room, in the labour room, in dialysis, in the dental or endoscopy suite, or by an external provider under contract.
...
Responsibility for the cycle sits with the person in charge of the processing area of {{HOSPITAL_NAME}}, whose title and reporting line are [Hospital to define]. Individual stages are performed by trained staff who sign for the stage they performed.$s$,
    $s$2. Classifying devices — critical, semi-critical, non-critical
...
The choice of high-level disinfectant, and the selection of agents for instruments generally, is made under this policy at steps 15 and 16. The support services policy governs disinfectants used on the environment and expressly points here for devices; the two sets of agents are not interchangeable.$s$,
    $s$3. The processing area — space and zoning
...
The separation of clean and contaminated flows in the rest of the hospital — the corridors, lifts, trolleys and storage that bring items here and take them away — is governed by the support services policy of {{HOSPITAL_NAME}} and is not restated here.$s$,
    $s$4. The processing area — ventilation, environment and utilities
...
Environmental cleaning of the processing area itself is performed to the support services policy, at a frequency reflecting the zone, and the record is held by the processing area.$s$,
    $s$5. Processing outside the central area
...
Any location that cannot meet these requirements ceases processing and sends its items to the central area. The Infection Control Officer decides this, and the decision is recorded.$s$,
    $s$6. Point-of-use treatment and transport
...
An item received in the processing area with dried soil is returned to the clinical area's attention through the record at step 7, not simply cleaned harder. The failure is at the point of use and is corrected there.$s$,
    $s$7. Receipt and sorting in the decontamination zone
...
Nothing leaves this zone in the direction of the clean side until it has completed cleaning and drying and passed the inspection at step 10.$s$,
    $s$8. Cleaning — manual
...
- an instrument that cannot be cleaned — because a lumen cannot be reached, a joint cannot be opened, or a coating has degraded — is removed from service under step 10 and is not sent forward in the hope that the sterilisation cycle will compensate.$s$,
    $s$9. Cleaning — mechanical, and verification that cleaning worked
...
Where a machine's cycle aborts or its parameters fall outside specification, the entire load is treated as uncleaned, is returned to the beginning, and the event is recorded.$s$,
    $s$10. Inspection, functional check and assembly
...
The person performing inspection and assembly records having done so against the set, by name or identifier. Where an item is found damaged repeatedly, or a set repeatedly incomplete, the pattern is reported to the Committee.$s$,
    $s$11. Packaging
...
Immediate-use steam sterilisation — the short, unwrapped cycle sometimes used for an instrument needed urgently — is not routine practice at {{HOSPITAL_NAME}} and is not used to compensate for insufficient instrument stock, which is the reason it is most often used. Where it is used, it is only for an item that cannot be processed in time by the normal cycle, never for an implant, and it is recorded with the reason, the item, the patient, the cycle parameters and the indicator result, and the record is reviewed by the Infection Control Officer. Repeated use for the same reason is treated as an inventory problem and referred to management. Whether {{HOSPITAL_NAME}} permits it at all, and under what limits, is [Hospital to define].$s$,
    $s$13. Moist heat sterilisation
...
Every cycle is recorded — steriliser identity, cycle number, date, time, load contents, cycle type, the parameters achieved as shown by the machine's own record, the indicator results, and the operator. Release of the load is governed by step 17 and by the monitoring at steps 24 to 30.$s$,
    $s$14. Low-temperature sterilisation
...
Every low-temperature cycle is recorded and released on the same basis as a steam cycle, with the biological indicator appropriate to the process.$s$,
    $s$15. High-level disinfection of semi-critical devices
...
Staff performing high-level disinfection work with ventilation adequate to the agent and with the protective equipment the product requires; exposure and health surveillance are governed by the occupational health policy.$s$,
    $s$16. Non-critical items and lower-level disinfection
...
Agents used on the patient environment itself — floors, walls, furniture and fittings — are governed by the support services policy and not by this one.$s$,
    $s$17. Unloading, cooling and release of the load
...
A load not released is quarantined in a designated area, physically separated and clearly marked, pending the decision at step 30.$s$,
    $s$18. Sterile storage and shelf life
...
Storage areas are cleaned to the schedule in the support services policy and are checked at a stated frequency for expired, damaged or unlabelled packs, with the check recorded — [Hospital to define].$s$,
    $s$19. Issue, transport to the point of use, and the check before use
...
Responsibility for the check sits with the person opening the pack, and it is not delegated to the processing department, which cannot see the pack at the moment it matters.$s$,
    $s$20. The records that evidence the cycle
...
Where the true cost of the default is unsustainable for a particular device — which is a legitimate concern and not one this policy dismisses — the route is a protocol under step 22 or a change of procurement to a reusable equivalent, decided in the open. It is not an informal local practice.$s$,
    $s$22. If a single-use device is reprocessed — what the written guidance must contain
...
Patients are not exposed to a reprocessed single-use device without the hospital having made this determination. Whether patient consent is separately sought is a decision for {{HOSPITAL_NAME}} on the advice of the Committee, and the position adopted is [Hospital to define].$s$,
    $s$23. Devices never reprocessed
...
The decision to withdraw an item under this step is made by any member of the processing staff or any clinician, and does not require prior authorisation. It is reported to the Infection Control Officer, who records it. A decision to withdraw is never reversed by a person other than the Infection Control Officer, and never on operational grounds alone.$s$,
    $s$24. The monitoring and validation schedule
...
Results are not merely filed. They are reviewed by the person in charge of processing at a stated frequency, trended, and reported to the Committee with the failures, the actions taken and the outcomes.$s$,
    $s$25. Physical and mechanical monitoring
...
Machine records are retained with the load record under step 20 for the period stated there.$s$,
    $s$26. Chemical indicators
...
An internal indicator failure discovered at the point of use is reported immediately to the processing area, because it may affect the whole load, and it is a trigger for step 31.$s$,
    $s$27. Biological indicators
...
A positive biological indicator, with a valid negative control, is a sterilisation failure and triggers step 30 and the recall at steps 31 to 37 immediately. It is not repeated first in the hope of a different answer; a repeat test may be run in parallel with the recall, never in place of starting it.$s$,
    $s$28. Air removal and leak testing
...
A machine that is due requalification and has not had it is taken out of service for patient items. Continuing to use it is a decision only the head of the institution may take, on the Infection Control Officer's written advice, with the compensating measures and the review date recorded.$s$,
    $s$30. When a test fails — the immediate decision
...
Quarantined items are reprocessed in full from the beginning of the cycle once the machine is fit, or are discarded where reprocessing is not appropriate. They are never released on the basis that the failure was probably not real.$s$,
    $s$31. What counts as a breakdown in the sterilisation system
...
Record the timing. The time the failure was identified, the time each of the above was completed, and the interval between them are recorded. The interval from identification to containment is the number that says whether this procedure works, and it is reported with the outbreak or incident report at step 36.$s$,
    $s$33. Determining the scope of the recall
...
Where the scope includes implants, the scope is treated as extending to every patient who received one, regardless of how long ago, and the assessment at step 34 applies to each.$s$,
    $s$34. Executing the recall and assessing exposed patients
...
Statutory or regulatory reporting, and reporting to a manufacturer where a device, indicator or sterilant is implicated, is made where required, and the report and its date are recorded.$s$,
    $s$35. Reprocessing recalled items and return to service
...
Enhanced monitoring. For a period following return to service, stated by the Infection Control Officer and recorded, monitoring is increased above the routine schedule — typically more frequent biological monitoring and closer review of physical records — and the results are reviewed by the Officer rather than only by the processing area. The period and the enhanced regime are recorded, and the return to routine monitoring is a decision that is also recorded.$s$,
    $s$36. The recall report, investigation and corrective action
...
The lessons are carried into the written guidance, the training programme and the monitoring schedule. A recall that changes no document and no practice has not been learned from.$s$,
    $s$37. Mock recalls
...
The exercise also tests the human elements: whether the out-of-hours contact route reaches someone, whether the person in charge of processing knows their standing authority to quarantine without waiting for approval, and whether departments know how to respond to a stop-issue instruction.$s$,
    $s$38. Outsourced processing
...
The contract manager of {{HOSPITAL_NAME}} is responsible for these obligations being in the contract and being enforced; the Infection Control Officer is responsible for the technical adequacy of the arrangement.$s$,
    $s$39. Loaner, consignment and trial instrument sets
...
The pattern of late arrivals is trended and reported to the Committee, because it is a supplier and scheduling problem that presents as an infection control one.$s$,
    $s$40. Records retained under this policy
...
This policy is reviewed at the interval {{HOSPITAL_NAME}} sets, and in any event after any recall, any change of steriliser or process, any change to the standard in force, and any change in the services provided — [Hospital to define — state the review interval].$s$
  ],
...
All staff are responsible for treating an item of unverified sterility as non-sterile, and for reporting immediately any suspicion that the sterilisation system has failed — a duty that belongs to every member of staff of every grade and is exercised directly.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Hospital Infection Control chapter, standard HIC.6.
...
- Internal documents of {{HOSPITAL_NAME}}: infection prevention and control programme policy, infection prevention and control practices policy, infection prevention and control in support services policy, prevention of healthcare associated infections and staff occupational health policy, infection prevention and control surveillance policy, occupational health policy, records retention policy, and the procurement and contract management procedures.$q$,
