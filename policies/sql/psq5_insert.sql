-- PSQ.5 master policy -- UNAPPROVED DRAFT for review.
...
--
-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 6 PSQ, printed page 104.
...
This document is the hospital-wide incident system. Specialty capture (medication events, transfusion reactions, unit QA findings, complaints) remains those documents; those events still enter this system when they are incidents.$q$,
  $q$This policy applies to every area of {{HOSPITAL_NAME}} in which an incident, near miss, adverse event or sentinel event can occur, and to everyone who reports, receives, analyses, acts or informs.
...
- This chapter is PSQ, not CQI. Do not code this system as a CQI incident module.$q$,
  $q${{HOSPITAL_NAME}} implements an incident management system.
...
{{HOSPITAL_NAME}} does not treat a specialty log (medication, transfusion, complaints) as a substitute for this system, and does not treat a 2nd Edition CQI incident file as this 3rd Edition standard.$q$,
  array[
    $s$1. Incident management system
...
This document does not print a named software product or a mandatory 24-hour clock as a NABH mandate. The time frame for reporting is this hospital's defined method.$s$,
    $s$2. Mechanism to identify sentinel events
...
A sentinel event enters the incident system at step 1 and is analysed at step 3. This step is identification.$s$,
    $s$3. Analysis of incidents

The organisation has an established process for analysis of incidents.

How incidents are analysed (including a deeper look for sentinel events), who analyses, and how the analysis is recorded, are [Hospital to define — the established process for analysis of incidents, including any distinct path for sentinel events]. Charles et al. (chapter reference 2), RCA2 (chapter reference 30) and ASQ on RCA (chapter reference 49) are frameworks. This document does not print a named RCA template as a NABH mandate. Proactive analysis before harm remains PSQ.1.c.$s$,
    $s$4. CAPA based on analysis
...
How CAPA is decided, implemented and checked, is [Hospital to define — how corrective and preventive actions from incident analysis are taken and followed through]. MOM.7.e remains CAPA from medication-event analysis as a medication process; when the event also sat in this system, CAPA is coordinated, not duplicated as two unrelated actions, and not omitted because "MOM.7 already did it".$s$,
    $s$5. Informing stakeholders of near miss, adverse event or sentinel event
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a sentinel event was not identified, or a medication error never entered this system, or when MOM.7, COP.10, COP.12, PRE.6 or PSQ.1 that this document hands work to are revised.$s$
  ],
  $q$The head of the institution is accountable for {{HOSPITAL_NAME}} implementing the incident management system and identifying sentinel events.
...
All staff are expected to treat an unused register, a copied sentinel list for services this hospital does not run, and a specialty log offered as the whole system, as defects, and to report them.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Chapter 6 PSQ, standard PSQ.5. This is not the 2nd Edition CQI chapter.
...
- Internal documents of {{HOSPITAL_NAME}}: the incident system; the sentinel-event definition; analysis and CAPA records; stakeholder-information method; MOM.7; COP.5, COP.6, COP.10, COP.11, COP.12; HIC.4/HIC.5; PRE.6; PSQ.1.$q$,
...
This document is not issued by, endorsed by, or affiliated with NABH, the World Health Organization, the National Centre for Disease Control, the Food Safety and Standards Authority of India, any Pollution Control Board, or any other body named in it. Wording is original; no text has been reproduced from the standards, rules or guidelines referenced.$q$,
  $q$[{"oe_code": "PSQ.5.a", "requirement": "The organisation implements an incident management system.", "steps": "Steps 1, 6", "evidence": "The written incident management system (what counts as an incident including near miss and adverse event as this hospital uses those terms, how reported, who receives on a route that is not only the person involved, how recorded) showing implementation on the wards rather than a quality-office register unused except at assessment; sample incidents against the unique identification number; the recorded method by which MOM.7 medication events, COP.5 transfusion reactions, COP.10.h anaesthesia events, HIC.4 needlesticks and PRE.6 complaint-described harm also enter this system when they meet the definition, without those specialty documents being replaced; the recorded use of WHO 2020 incident-reporting guidance and AHRQ Reporting Patient Safety Events (chapter references 26 and 31) as frameworks not a named software mandate; the recorded refusal to print a mandatory 24-hour NABH clock or a 2nd Edition CQI incident module as this system; induction or briefing of staff who report; the location of the system so staff can use it; the audit sample at step 6 of incidents that actually entered from the wards", "responsible": "Named lead holds the system; staff report; specialty leads ensure dual entry when required; quality or accreditation coordinator audits"}, {"oe_code": "PSQ.5.b", "requirement": "The organisation has a mechanism to identify sentinel events.", "steps": "Steps 2, 1, 6", "evidence": "The written sentinel-event definition matching services this hospital actually runs, with recorded absences for classes the service directory does not provide rather than a copied tertiary list; the identification method at the time of the event (who may declare, how recorded) rather than only a quarterly retrospective stamp; sample identifications entering the incident system; the recorded refusal to treat every complaint as sentinel or to leave the term undefined; the location of the definition; induction or briefing of staff who may recognise a sentinel event; the audit sample at step 6 of identification against this hospital's definition", "responsible": "Named lead holds the definition and mechanism; staff who recognise a sentinel event apply it; quality or accreditation coordinator audits"}, {"oe_code": "PSQ.5.c", "requirement": "The organisation has an established process for analysis of incidents.", "steps": "Steps 3, 2, 6", "evidence": "The written analysis process including any distinct path for sentinel events; sample analyses; the recorded split that PSQ.1.c is proactive analysis before harm; RCA2/Charles/ASQ used as frameworks not mandated templates; the audit sample at step 6", "responsible": "Named analysers; named lead holds the process; quality or accreditation coordinator audits"}, {"oe_code": "PSQ.5.d", "requirement": "Corrective and preventive actions are taken based on the findings of such analysis.", "steps": "Steps 4, 3, 6", "evidence": "Sample CAPA from incident analysis followed through; the recorded coordination with MOM.7.e when the event was also a medication event, neither omitted nor duplicated as unrelated actions; the audit sample at step 6", "responsible": "Named CAPA owners; MOM.7.e remains medication CAPA; quality or accreditation coordinator audits"}, {"oe_code": "PSQ.5.e", "requirement": "The organization shall have a process for informing various stakeholders in case of a near miss / adverse event / sentinel event.", "steps": "Steps 5, 2, 6", "evidence": "The written informing process naming stakeholders as this hospital defines them; sample records of informing; the recorded splits that PRE.6 is complaint redressal and ROM.4.c will own governance external-reporting duty; the audit sample at step 6", "responsible": "Named informer; PRE.6/ROM.4 remain those documents; quality or accreditation coordinator audits"}]$q$::jsonb,
...
The values the hospital must supply: incident system; sentinel-event mechanism; analysis process; CAPA method; stakeholder-informing process; named lead; audit interval; review interval; intranet location; and any additional local abbreviation.$q$,
  '1.0',
