-- MOM.1 master policy -- UNAPPROVED DRAFT for review.
...
--
-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 3, printed page 76
...
- The medical record itself is governed by the information-management policies of {{HOSPITAL_NAME}} (IMS, not yet drafted). This policy owns the formulary, committee, acquisition and after-hours content written into working records.$q$,
  $q${{HOSPITAL_NAME}} maintains a list of medications appropriate to the patients it treats and to the healthcare services it has defined. The list is developed, updated and implemented. A copied formulary that does not match this hospital's services is not that list.
...
{{HOSPITAL_NAME}} requires clinicians to adhere to the current formulary. A personal preference that bypasses the list without the non-formulary procedure is a defect.$q$,
  array[
    $s$1. Formulary list developed, updated and implemented
...
This step does not write storage, prescription method, or after-hours obtainment; those are later steps and sibling policies.$s$,
    $s$2. Pharmacy services and medication usage through a multidisciplinary committee
...
Medicines used for procedural sedation and for anaesthesia are selected as a clinical act under those clinical policies. This committee still owns that those medicines, if stocked, appear on the formulary or are acquired under step 3. Storage of any controlled drug among them is MOM.8, not this minute.$s$,
    $s$3. Acquisition of formulary medications and of medications not listed in the formulary
...
Each acquisition that concerns a named patient is recorded against the unique identification number. Stock acquisition of formulary items is recorded in the pharmacy or stores record the hospital uses. The record shows which route was used.$s$,
    $s$4. Obtaining medication when the pharmacy is closed
...
The after-hours procedure is held with the written guidance at step 2 and is known to the night and holiday staff who would use it. A procedure that lives only in the quality office is not a procedure to obtain medication when the pharmacy is closed.$s$,
    $s$5. Implantable prosthesis and medical devices — pointer to the implant policy
...
This committee does not approve an implant as if it were a non-formulary tablet. An implant is not added to the medicine formulary as a substitute for MOM.9.$s$,
    $s$6. Clinician adherence to the current formulary
...
A clinician who repeatedly bypasses the list is a committee matter at step 2, not only a personal preference.$s$,
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when an unlicensed acquisition, an after-hours obtainment that was not recorded, a formulary that does not match the directory, or a revision of the storage, prescription, transfusion, implant or infection-control policies that this document hands work to, exposes a gap.$s$
  ...
  $q$The head of the institution is accountable for {{HOSPITAL_NAME}} having a formulary that matches defined services, a multidisciplinary committee that actually guides pharmacy services, licensed acquisition, and a usable after-hours obtainment method.
...
All staff are expected to treat an unlicensed acquisition, an unrecorded after-hours taking, and a personal non-formulary habit, as defects, and to report them.$q$,
  $q$- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Management of Medication chapter, standard MOM.1.
...
Several requirements in this document are statutory rather than advisory — in particular those arising under the Pharmacy Act, 1948, insofar as it governs pharmacy services and the registered pharmacist, and the Drugs and Cosmetics Act, 1940 and the Drugs and Cosmetics Rules, insofar as they govern the acquisition and sale of medicines. Statutory requirements change, and State authorities impose additional or stricter conditions. {{HOSPITAL_NAME}} is responsible for verifying the current text of any rule cited here and the conditions attached to its own authorisations and licences; this document does not constitute legal advice.
