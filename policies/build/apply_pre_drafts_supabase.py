# -*- coding: utf-8 -*-
"""Insert PRE.1–PRE.6 master-policy drafts into live Supabase. Never approves.

These drafts are UNAPPROVED. Do not run --insert until the owner confirms the
write. When that write is confirmed, insert all six rows, leave status = 'draft'
...
Usage:
  python3 policies/build/apply_pre_drafts_supabase.py --insert
  python3 policies/build/apply_pre_drafts_supabase.py --verify-only
  python3 policies/build/apply_pre_drafts_supabase.py --dry-run
"""
import argparse
import os
import sys
...
CODES = [f"PRE.{n}" for n in range(1, 7)]
_POLICIES = _HERE.parent


def draft_path_for(code):
    n = code.split(".", 1)[1]
    return _POLICIES / "drafts" / f"pre{n}_draft.json"


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--insert", action="store_true",
                    help="POST each missing PRE.1–PRE.6 row (status stays draft)")
    ap.add_argument("--verify-only", action="store_true",
                    help="only verify live rows against local drafts")
    ap.add_argument("--dry-run", action="store_true",
                    help="no network: validate all 6 drafts and print the plan")
    args = ap.parse_args()
    ...
    if args.dry_run:
        print("\nDRY RUN -- no network calls. Plan for each of PRE.1–PRE.6:")
