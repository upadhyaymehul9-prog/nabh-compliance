...
The {eng} holds the current list and last inspection. A missing device in a care area that is in use is escalated the same shift.""",
        ...
        "FMS.1.b": f"""{HOSPITAL} has facilities for the differently-abled (at a minimum a wheelchair-accessible entrance and a toilet, as regulatory requirement and this hospital's building allow).

The {eng} keeps the current access list. A step-only entrance with no documented alternative is not this element.""",
...
The {eng} owns the round calendar. A month without a completed round is a gap under this CORE element.""",
...
The {eng} holds the dated assessment. Work started with no assessment is incomplete under this element.""",
...
The {eng} walks the signs {quarterly}. This is a CORE element.""",
...
The {eng} files what was applied. This Excellence element is evidenced on actual building work, not by a sentence alone.""",

        "FMS.3.c": f"""The organisation conducts electrical safety audits of the facility at least {yearly} to reduce risk to people and property and to prevent fire from short-circuiting. National Electrical Code of India 2023 is a reference. Thermal imaging may be used.

The {eng} holds the last audit and actions.""",

        "FMS.3.d": f"""There is a written procedure for identifying and disposing of material not in use (non-functioning items, excess stock, general scrap). Condemnation records sit with the {eng}. This is an asterisked element.""",
...
The {eng} holds the calibration schedule. Portal wording “Utility equipment, are” is kept as printed.""",
...
A measuring device past its calibration due date is a stop-work trigger (section 6).""",
        ...
        "FMS.5.f": f"""Written guidance supports medical equipment replacement and disposal. Condemnation is systematic and recorded. This is an asterisked element.""",
...
A live leak or a silenced required alarm is a stop-work trigger (section 6). This is a CORE element.""",
        ...
        "FMS.6.c": f"""There is an operational, inspection, testing and maintenance plan for piped medical gas, compressed air and vacuum, following the manufacturer. Compressed-air purity is checked at a terminal outlet at least {yearly}, at least one terminal in the operating theatre and one in intensive care if those areas exist.
...
No required alternate source is a stop-work trigger (section 6). This is a CORE element.""",
        ...
        "FMS.6.e": f"""The organisation regularly tests these alternate sources. Results are documented. Default interval {monthly} unless the manufacturer or the written plan states a shorter one.""",
...
The {eng} holds the plan and last service evidence. This is an asterisked element.""",
    }
