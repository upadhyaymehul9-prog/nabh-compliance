# -*- coding: utf-8 -*-
"""Parse official portal IPC chapter OCR into inventory JSON.

Authoritative source: policies/source/hco6_ipc_ocr.txt (pdftotext of portal
Standards PDF pages 122–129). Does not use the Guidebook for counts/levels/stars.
"""
...
OCR = ROOT / "policies/source/hco6_ipc_ocr.txt"
OUT = ROOT / "policies/source/hco6_ipc_inventory.json"

LIGATURE_MAP = str.maketrans({"\uf001": "fi", "\uf002": "fl", "\ufb01": "fi", "\ufb02": "fl"})
LETTER_RE = re.compile(
    r"^\s+(CORE|Commitment|Achievement|Excellence)\s+([a-lA-L])\.\s+(.*)$"
)
STD_RE = re.compile(r"^\s+IPC\.(\d+)\.\s*(.*)$")
...
# IPC.8 title reconstruction: two-column bleed from IPC.6.h on printed p.117.
IPC8_TITLE = (
    "The organisation takes action to prevent or reduce healthcare "
...
def parse() -> dict:
    ...
    for line in body.splitlines():
        ...
        if mstd and (in_title or stripped.startswith("IPC.")):
            flush_oe()
            n = mstd.group(1)
            ...
            inv[n] = current
            in_title = True
            # Drop two-column bleed ("of infections. *") on the IPC.8 code line.
            if rest and not is_footer(rest) and "of infections" not in rest.lower():
        ...
        if in_title:
            if re.match(r"^\s*Objective Elements\s*$", line):
                flush_title()
                continue
            # Skip OE-letter bleed (IPC.6.h leaking into the IPC.8 heading).
            if LETTER_RE.match(line):
                continue
            if stripped and not is_footer(stripped) and not stripped.startswith("IPC."):
                if "of infections" in stripped.lower() and current and current["standard_code"] == "IPC.8":
                    continue
...
def main() -> int:
    ...
    for n in sorted(inv, key=int):
        ...
        print(f"IPC.{n}: {d['count']} [{letters}] {d['title'][:80]}")
        for o in d["oes"]:
            star = "*" if o["star"] else " "
            print(f"  {o['letter']}{star} {o['level']:12} {o['text'][:90]}")
    print(f"TOTAL {total} CORE {cores} Comm {comm} Ach {ach} Exc {exc}")
    print("asterisks", stars)
    assert len(inv) == 8, inv.keys()
    assert total == 49, total
    assert cores == 13 and comm == 33 and ach == 3 and exc == 0, (cores, comm, ach, exc)
