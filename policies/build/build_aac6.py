# -*- coding: utf-8 -*-
"""Builds the AAC.6 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): AAC.6.a
  Tier 2 (lighter pass):   AAC.6.b, AAC.6.c, AAC.6.d, AAC.6.e

ONLY AAC.6.a is asterisked. The whole of b-e is lean.

Official source: NABH Standards for Small Healthcare Organisations, 3rd Edition
(August 2022), Chapter 1, standard AAC.6 and OEs AAC.6.a-e, read from the official
standards PDF (downloaded 2026-08-17 from the NABH website's Explore NABH Standards
page), printed page 53, PDF page index 59.
...
committed scripts/shco_oe_asterisks.json on all 408 entries) and the AAC.6 page
read directly. Only AAC.6.a carries the asterisk.
"""
...
POLICY_TITLE = "Laboratory and Imaging Safety"
...
- The hospital-wide hazardous-materials programme, where one exists under facility management, owns building-level chemical stores. This policy owns chemical hygiene at the laboratory bench."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} implements a laboratory-safety programme. The programme is written, led, used on working days, and reviewed when something goes wrong and when the calendar says so.
...
Imaging signage is displayed prominently in every location it belongs. A sign in a drawer is not displayed."""

PROCEDURE_STEPS = [
"""1. The laboratory-safety programme — what it covers and why
...
The written programme is held at [Hospital to define — where the laboratory-safety programme is held], and the named lead is [Hospital to define — the named lead of the laboratory-safety programme].""",

"""2. Implementing the laboratory-safety programme
...
Outsourced laboratory work performed on another organisation's premises is not this hospital's laboratory. Outsourced work performed by visiting staff in this hospital's laboratory is.""",

"""3. Laboratory-safety incidents and review
...
The reason incidents sit inside the documented-evidence anchor rather than as an appendix: a programme that has never had an incident may be excellent or may be unreporting. The register, the review and the walk at step 2 are how {{HOSPITAL_NAME}} can tell which.""",

"""4. Laboratory personnel — training and safety measures
...
Staff who only collect specimens and do not work at the laboratory bench are trained under the infection-prevention policy for that collection work; they are trained under this step only for the laboratory tasks they actually perform.""",

"""5. Screening patients for safety and risk before imaging
...
A study is not begun until the checklist for that modality is complete. A positive screen is acted on — delayed, modified, or not performed — and the action is recorded against the unique identification number. Screening is performed by [Hospital to define — who performs pre-imaging safety/risk screening].""",

"""6. Radiation-safety devices, monitoring and training
...
ALARA is the principle — as low as reasonably achievable — not a figure. Technique, collimation, and not repeating a study without a reason are how the principle is applied. A numeric dose constraint copied from an old circular is not this step.""",
...
Licences behind the activity the signs warn about remain on the imaging-licence calendar under the imaging-services policy of {{HOSPITAL_NAME}}.""",
...
RESPONSIBILITY = """The head of the institution is accountable for {{HOSPITAL_NAME}} running a laboratory-safety programme rather than holding a file, and for radiation work being performed only with the monitoring, devices and signage the AERB authorisation and this policy require.
...
All staff are expected to treat an unscreened imaging study, a monitoring device left in a drawer, a missing radiation sign, or laboratory work done without the protective equipment the task requires, as a defect, and to report it."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Access, Assessment and Continuity of Care chapter, standard AAC.6.
...
ABBREVIATIONS = """Abbreviations already defined in the HIC.1 to HIC.6 master policies are not repeated here. A reader using this document on its own should refer to those policies for the shared glossary, including NABH, SHCO, OE, BMW and PPE.
...
STATUTE_CLAUSE = (
    "the Bio-Medical Waste Management Rules, 2016, the Atomic Energy Act, 1962 and the "
    "Atomic Energy (Radiation Protection) Rules together with the authorisations issued by "
    "the Atomic Energy Regulatory Board, and, where ultrasonography is performed, the "
...
OE_MAPPING = [
    ...
    },
    {
        ...
        "steps": "Steps 4, 8",
        "evidence": "Training records before unsupervised bench work and at the stated interval, covering biosafety, chemical hygiene, spill response, bench-sharps, laboratory fire and microbiological-waste pre-treatment, with return demonstration of the spill method and of the protective equipment the person's work uses; records that protective equipment, the spill kit, reagent safety information and a biosafety cabinet where required were provided at the point of work and not as a personal purchase",
        "responsible": "The named laboratory-safety lead trains and records competence; administration provides the measures at the bench; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "AAC.6.c",
        "requirement": "Patients are appropriately screened for safety / risk before imaging",
        "steps": "Steps 5, 8",
        "evidence": "The written pre-imaging checklist by modality actually provided (pregnancy where ionising radiation to abdomen or pelvis applies; contrast history and renal risk where contrast is used; metal and implant contraindications where MRI is provided — MRI not assumed); sample checklists completed before the study, against the unique identification number, with the action taken on a positive screen; the named role that screens",
        "responsible": "The named screening role completes the checklist before the study; imaging personnel do not begin an unscreened study; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "AAC.6.d",
        ...
        "steps": "Steps 6, 8",
        "evidence": "Personnel-monitoring issue, return and dose records as the AERB authorisation requires, at the stated interval, devices not shared; lead aprons, thyroid shields and other protocol devices available, intact and used, with cracked devices withdrawn; training records before unsupervised radiation work and at the stated interval; a recorded statement that numeric dose limits are those the AERB authorisation requires and are not restated in this policy",
        "responsible": "The person who issues monitoring devices keeps the records; imaging personnel wear them and use intact protective devices; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "AAC.6.e",
        "requirement": "Imaging signage is prominently displayed in all appropriate locations",
        "steps": "Steps 7, 8",
        "evidence": "The current list of imaging-signage locations and the signs in place — radiation trefoil, restricted-area notice, pregnancy caution, and the PC-PNDT notice where ultrasonography is performed; the named role that maintains them; a check that this list is not the service display and not isolation signage",
        "responsible": "The named signage role keeps signs visible; imaging-services policy owns the licences the signs warn about; definition-and-display policy owns service boards; infection-control policies own isolation cards; quality or accreditation coordinator audits",
    },
]
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 1, printed page 53
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="aac6_draft.json",
        sql_name="aac6_insert.sql",
    )
