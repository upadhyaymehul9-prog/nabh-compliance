# -*- coding: utf-8 -*-
"""Insert FMS.1–FMS.5 master-policy drafts into live Supabase. Never approves.
...
This chapter is FMS (Facility Management and Safety) in the SHCO 3rd Edition,
Chapter 8. Codes are FMS.1–FMS.5.
...
Usage:
  python3 policies/build/apply_fms_drafts_supabase.py --insert
  python3 policies/build/apply_fms_drafts_supabase.py --verify-only
  python3 policies/build/apply_fms_drafts_supabase.py --dry-run
"""
import argparse
import os
import sys
...
def draft_path_for(code):
    n = code.split(".", 1)[1]
    return _POLICIES / "drafts" / f"fms{n}_draft.json"


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--insert", action="store_true",
                    help="POST each missing FMS.1–FMS.5 row (status stays draft)")
    ...
    if args.dry_run:
        print("\nDRY RUN -- no network calls. Plan for each of FMS.1–FMS.5:")
        ...
        print("Codes are FMS.1–FMS.5 (3rd Edition Chapter 8).")
        return 0
