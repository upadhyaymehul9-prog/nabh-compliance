# -*- coding: utf-8 -*-
"""Builds the ROM.4 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): ROM.4.a, ROM.4.c
  Tier 2 (lighter pass):   ROM.4.b, ROM.4.d, ROM.4.e

TWO of five OEs are asterisked. ROM.4.d is Core and still unasterisked — Core
is not the tier trigger.

Official source: NABH SHCO 3rd Edition PDF, Chapter 7, ROM.4, printed page 111
(PDF page index 117). Intent printed page 109: leaders ensure that
patient-safety and risk-management issues are an integral part of patient care
and hospital management.

Asterisks verified 2026-08-17: ROM.4.a and ROM.4.c carry the asterisk.

No named Act of Parliament is a numbered ROM chapter reference. ISO 31000:2018
(chapter reference 38) is a framework, not a certified-system mandate.
PSQ.1.c, COP.12 and PSQ.5 already forwarded organisation-wide risk and
governance reporting here — this draft accepts those handoffs and does not
absorb their methods.
"""
...
TIER1_OES = ["ROM.4.a", "ROM.4.c"]

POLICY_TITLE = "Patient Safety and Risk Management as Integral to Care and Hospital Management"
...
This document is management's duty that those systems exist and are integral. It is not the patient-safety programme (PSQ.1), not the incident system (PSQ.5), not bedside vulnerable-patient programmes (COP.12), and not FMS facility-risk rounds."""

SCOPE = """This policy applies to management at {{HOSPITAL_NAME}}, to the people who operate organisation-wide risk management, to those who report system and process failures internally and externally, and to those who contract and monitor outsourced services.
...
- Indian Contract Act, 1872 is not a numbered ROM chapter reference and is not in paragraph 2. Outsourcing agreements are accreditation method with service parameters; the hospital's own counsel owns contract law."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} requires management to ensure proactive risk management across the organisation.
...
{{HOSPITAL_NAME}} does not treat a quality-office risk register unused on the wards, or a handshake with no service parameters, as that duty."""

PROCEDURE_STEPS = [
"""1. Proactive risk management across the organisation
...
"""3. Systems for internal and external reporting of system and process failures
...
"""5. Monitoring quality of outsourced services and improvement
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a process failure was never reported, or when PSQ.1, PSQ.5, COP.12, ROM.3 or FMS that this document hands work to are revised.""",
]

RESPONSIBILITY = """The head of the institution is accountable that management ensures the duties in this document.
...
All staff are expected to treat a quality-office risk file unused on the wards, an incident that never entered PSQ.5, and an outsourced service with no service parameters, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Chapter 7 ROM, standard ROM.4.
...
- Internal documents of {{HOSPITAL_NAME}}: the organisation-wide risk system; failure-reporting systems; outsourcing agreements; PSQ.1, PSQ.4, PSQ.5; COP.12; ROM.1, ROM.3; AAC.1."""
...
OE_MAPPING = [
    {
        ...
        "steps": "Steps 1, 6",
        "evidence": "The written organisation-wide proactive risk system showing management ownership across patient care and hospital management rather than a quality-office file unused on the wards; sample analyses that produced an improvement before harm; the recorded splits that PSQ.1.c owns quality-system patient-safety analysis, COP.12 owns bedside vulnerable/falls/PU/VTE tools, FMS (when drafted) owns facility rounds, and PSQ.5 owns after-the-fact incidents; the recorded use of ISO 31000:2018, Alam and Kaya (chapter references 38, 1 and 26) as frameworks not a certified-system or named-FMEA mandate; induction or briefing of named risk leads; the location of the system; the audit sample at step 6 of management-owned proactive work rather than one workshop",
        "responsible": "Head of the institution is accountable; named risk lead operates the system; PSQ.1.c/COP.12/PSQ.5/FMS remain those documents; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "ROM.4.b",
        ...
        "steps": "Steps 2, 1, 6",
        "evidence": "The recorded method of integration and sample minutes or plans showing QI (PSQ.1), risk (this document) and strategy (ROM.3.a) talking to each other rather than three unrelated files; the audit sample at step 6",
        "responsible": "Head of the institution ensures integration; PSQ.1 and ROM.3.a remain those documents; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "ROM.4.c",
        ...
        "steps": "Steps 3, 6",
        "evidence": "The written internal and external reporting systems for system and process failures showing implementation (sample reports that left the organisation or reached governance) rather than a sentence in this policy; the recorded split that PSQ.5 is the incident SOP those clinical incidents use, PRE.6 is complaint redressal, MOM.7 is medication-event capture, and HIC.4/HIC.5 remain needlestick and HAI case-finding; the recorded refusal to print a mandatory 24-hour NABH clock or a universal named regulator; induction or briefing of staff who report failures; the location of the systems; the audit sample at step 6 of reports that actually travelled internally and, where defined, externally",
        "responsible": "Head of the institution ensures the systems are implemented; named reporters use them; PSQ.5/PRE.6/MOM.7/HIC remain those documents; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "ROM.4.d",
        ...
        "steps": "Steps 4, 6",
        "evidence": "The list of outsourced services against AAC.1 and the documented agreement for each including service parameters; recorded absences where nothing is outsourced; the recorded refusal to import Indian Contract Act 1872 as a NABH mandate; the audit sample at step 6",
        "responsible": "Head of the institution ensures agreements exist; named contracting lead holds them; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "ROM.4.e",
        "requirement": "Management monitors the quality of the outsourced services and improvements are made as required.",
        "steps": "Steps 5, 4, 6",
        "evidence": "Sample monitoring against the service parameters at step 4 and recorded improvements; the recorded split that HIC.6/AAC.4/AAC.5 own method where the work is CSSD, laboratory or imaging; the audit sample at step 6",
        "responsible": "Named monitor; owning clinical/IPC documents remain those methods; quality or accreditation coordinator audits",
    },
]
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="rom4_draft.json",
        sql_name="rom4_insert.sql",
    )
