# -*- coding: utf-8 -*-
"""Builds the FMS.4 master policy draft: JSON for review + SQL for later insert.
...
FMS chapter calibration (owner 2026-08-18): technical T2 (safe handling;
alternate sources and tests) carries HTM/ISO/NFPA/BCGA substance. Asterisked
OEs still get full Tier 1.

Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): FMS.4.a, FMS.4.d
  Tier 2 (lighter pass):   FMS.4.b, FMS.4.c
  b and c are Core and unasterisked, drafted at technical T2 density.

Official source: NABH SHCO 3rd Edition PDF, Chapter 8. FMS.4 header and
OEs a-b printed page 117 (PDF index 123); OEs c-d printed page 118 (PDF
index 124). Summary on printed page 115 ends with a period; the OE-page
header after "FMS.4." has no period after "compressed air". FMS.4.b and
FMS.4.c have no terminal period in the book — preserved in mapping.

Asterisks verified 2026-08-18: FMS.4.a and FMS.4.d carry the asterisk.

PDF md5 39e3bc86d73d651b9cfef283bbf018a9.

No named Act of Parliament is a numbered FMS chapter reference for gases.
HTM medical-gas pipeline (chapter reference 6), ISO 10524-1/2/3 (14-16),
NFPA (12, 17), BCGA (2), BS EN 12021 (3), BOC (23) and Sarangi et al. (19)
are frameworks. PESO / Gas Cylinder Rules / Explosives Act are not numbered
FMS refs; they stay on ROM.2.c's applicable-legislation register if they
apply, and are flagged in master-policy-todos.md — not forced into P2.
P2 is accreditation-only.
"""
...
CHAPTER = "FMS"
OE_CODES = [
    "FMS.4.a", "FMS.4.b", "FMS.4.c", "FMS.4.d",
]
TIER1_OES = ["FMS.4.a", "FMS.4.d"]
...
This document is the gas, vacuum and compressed-air programme. It is not FMS.1 electrical backup (though MGPS plant may be an essential circuit there). It is not FMS.3 equipment PPM of a flowmeter inventoried as a device. It is not FMS.5 fire plan, though a gas leak is a non-fire emergency FMS.5 must name. It is not HIC.3 BMW. It is not the billing ledger."""
...
- UK DH HTM Medical Gas Pipeline Systems (chapter reference 6), NFPA medical gas / cylinder storage (chapter references 12 and 17), BCGA (2), BS EN 12021 (3), BOC handling (23) and Sarangi et al. (19) are frameworks, not pasted protocols and not a mandate to hold a UK HTM certificate."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} uses written guidance that governs procurement, handling, storage, distribution, usage and replenishment of medical gases.
...
{{HOSPITAL_NAME}} does not treat a copied HTM manual for a cylinder-only hospital, mixed full and empty cylinders, or an untested manifold reserve, as that duty."""

PROCEDURE_STEPS = [
"""1. Written guidance: procurement, handling, storage, distribution, usage and replenishment
...
Which gases this hospital actually uses (oxygen as a minimum if any medical gas is used; nitrous oxide, medical air, carbon dioxide, vacuum, compressed air only if in AAC.1), who may procure from which licensed source, how a delivery is accepted (identity, pressure, pin-index or DISS, expiry/batch as the supplier provides), how storage, distribution, use and replenishment are each governed, are [Hospital to define — written guidance governing procurement, handling, storage, distribution, usage and replenishment of the medical gases this hospital actually uses]. A gas the directory does not use is a recorded absence.""",

"""2. Safe handling, storage, distribution and use
...
The failure mode is a "full" rack that contains empties, a cylinder used as a doorstop, a grease-on-oxygen regulator, or a theatre that opens a new cylinder without a second person to check the gas identity. How staff handle, store, distribute and use, including the check that the gas at the workstation is the gas intended, are [Hospital to define — safe handling, storage, distribution and use of medical gases, including full/empty segregation and identity check at use]. PESO / Gas Cylinder Rules, if they apply, live on ROM.2.c; they are not restated as a NABH protocol here.""",

"""3. Alternate sources and tests at a predefined frequency
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a wrong-gas or empty-reserve event occurred, or when FMS.1, FMS.2, FMS.3, FMS.5 or AAC.1 that this document hands work to are revised.""",
]

RESPONSIBILITY = """The head of the institution is accountable that medical gases, vacuum and compressed air are managed as this document requires.
...
All staff are expected to treat mixed full and empty cylinders, an untested reserve, a forced connector, and a piped terminal that is unlabelled, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Chapter 8 FMS, standard FMS.4.
...
- Internal documents of {{HOSPITAL_NAME}}: medical-gas written guidance; cylinder-store checks; backup-test records; MGPS file or recorded absence; FMS.1, FMS.2, FMS.3, FMS.5; AAC.1; ROM.2.c."""

DISTRIBUTION = """Controlled master copy: office of the head of the institution, {{HOSPITAL_NAME}}, with the named gas-plant or engineering lead and the quality or accreditation coordinator.
...
OE_MAPPING = [
    {
        ...
        "steps": "Steps 1, 5",
        "evidence": "The written guidance covering procurement, handling, storage, distribution, usage and replenishment of the gases this hospital actually uses, with sample records of a delivery accepted, a store, a distribution and a replenishment that followed it, rather than a supplier brochure or a BOC pocket guide offered as the hospital SOP; recorded absences for gases not in AAC.1; the recorded use of BCGA, BOC, NFPA cylinder storage and ISO 10524 as frameworks not pasted protocols; induction of staff who procure or change cylinders; the location of the guidance; the audit sample at step 5 of all six acts present rather than store-only guidance",
        "responsible": "Named gas-plant lead holds the written guidance; AAC.1 remains the directory; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.4.b",
        "requirement": "Medical gases are handled, stored, distributed and used in a safe manner",
        "steps": "Steps 2, 5",
        "evidence": "Store checks showing full/empty segregation, chained upright cylinders, matched regulators (ISO 10524 framework) and identity check at use rather than a cylinder used as a doorstop; PESO flagged to ROM.2.c not P2; the audit sample at step 5",
        "responsible": "Named gas-plant lead with clinical users at the point of use; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.4.c",
        ...
        "steps": "Steps 3, 5",
        "evidence": "Named alternate sources for each gas/vacuum/air actually used; functioning-test records (flow from the reserve, vacuum that aspirates, alarms that annunciate) at the predefined frequency rather than a paper changeover; recorded absences where a gas is not used; split from FMS.1.e plant electricity; the audit sample at step 5",
        "responsible": "Named gas-plant lead runs functioning tests; FMS.1.e remains electrical backup; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.4.d",
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 8 FMS, printed pages 117-118
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="fms4_draft.json",
        sql_name="fms4_insert.sql",
    )
