# -*- coding: utf-8 -*-
"""Parse official portal PSQ chapter OCR into inventory JSON.

Authoritative source: policies/source/hco6_psq_ocr.txt (pdftotext of portal
Standards PDF pages 130–142). Does not use the Guidebook for counts/levels/stars.
"""
...
OCR = ROOT / "policies/source/hco6_psq_ocr.txt"
OUT = ROOT / "policies/source/hco6_psq_inventory.json"

LIGATURE_MAP = str.maketrans({"\uf001": "fi", "\uf002": "fl", "\ufb01": "fi", "\ufb02": "fl"})
LETTER_RE = re.compile(
    r"^\s+(CORE|Commitment|Achievement|Excellence)\s+([a-lA-L])\.\s+(.*)$"
)
STD_RE = re.compile(r"^\s+PSQ\.(\d+)\.\s*(.*)$")
...
def parse() -> dict:
    ...
    for line in body.splitlines():
        ...
        if mstd and (in_title or stripped.startswith("PSQ.")):
            flush_oe()
            n = mstd.group(1)
        ...
        if in_title:
            ...
            if LETTER_RE.match(line):
                continue
            if stripped and not is_footer(stripped) and not stripped.startswith("PSQ."):
...
def main() -> int:
    ...
    for n in sorted(inv, key=int):
        ...
        print(f"PSQ.{n}: {d['count']} [{letters}] {d['title']}")
        for o in d["oes"]:
            star = "*" if o["star"] else " "
    ...
    assert cores == 8 and comm == 28 and ach == 7 and exc == 3, (cores, comm, ach, exc)
