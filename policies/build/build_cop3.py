# -*- coding: utf-8 -*-
"""Builds the COP.3 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  ...
  Tier 2 (lighter pass):   COP.3.a, COP.3.b, COP.3.c, COP.3.d.
...
(August 2022), Chapter 2 Care of Patients, standard COP.3 and OEs COP.3.a-d,
read from the official standards PDF (downloaded 2026-08-17 from the NABH
website's Explore NABH Standards page), printed pages 62-63, PDF page index 68-69.
...
committed scripts/shco_oe_asterisks.json on all 408 entries) and the COP.3 pages
read directly. ZERO OEs of COP.3 carry the asterisk.
"""
...
POLICY_TITLE = "Cardio-Pulmonary Resuscitation"
...
PURPOSE = """This document sets out how {{HOSPITAL_NAME}} provides cardio-pulmonary resuscitation uniformly: services available to patients at all times; assigned roles complied with during an event and the events recorded; equipment and medications for resuscitation available in the areas where they are needed; and a multidisciplinary committee that analyses every resuscitation and takes corrective and preventive action.

A crash that is answered by whoever is nearest, with a trolley that is incomplete, and with no record that can be reviewed, is not a resuscitation service. This document is the process that prevents that."""

SCOPE = """This policy applies to every location in {{HOSPITAL_NAME}} in which a patient may require cardio-pulmonary resuscitation: the emergency area, every in-patient ward, day-care, the operation theatre and recovery, intensive or high-dependency areas where they exist, diagnostic and procedural areas, and any other clinical location the hospital names. It binds the staff who respond, the staff who record the event, the staff who check and restore the kit, and the multidisciplinary committee that reviews every resuscitation.
...
- The medical record itself is governed by the information-management policies of {{HOSPITAL_NAME}}. This policy owns the resuscitation-event record that is written into it."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} makes resuscitation services available to patients at all times. A service that exists only during ordinary hours is not this service.
...
{{HOSPITAL_NAME}} adopts an American Heart Association cardio-pulmonary resuscitation framework as the clinical framework it uses. The edition and the algorithm in force are this hospital's choice. This document does not print a numbered algorithm and does not state joule doses or drug doses."""

PROCEDURE_STEPS = [
"""1. Resuscitation services available at all times
...
"""2. Assigned roles during resuscitation and the record of events
...
A resuscitation without a record is an event this hospital cannot review at step 4.""",

"""3. Equipment and medications in various areas
...
The framework this hospital adopts for resuscitation practice is an American Heart Association framework; the edition and the algorithm in force are [Hospital to define — the American Heart Association edition and the resuscitation algorithm in force]. The algorithm is not printed in this policy.""",

"""4. Multidisciplinary post-event analysis and corrective action
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a failed call, an incomplete kit or an unreviewed event exposes a gap, or when the assessment, emergency-care, sterilisation or infection-control policies that this document hands work to are revised.""",
]

RESPONSIBILITY = """The head of the institution is accountable for {{HOSPITAL_NAME}} having a resuscitation service at all times, for kits that are actually present in the named areas, and for every event being analysed.
...
All staff are expected to start the call when resuscitation is required, and to report a call that was not answered, a kit that was incomplete, or an event that was not recorded."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Care of Patients chapter, standard COP.3.
...
ABBREVIATIONS = """Abbreviations already defined in the HIC.1 to HIC.6 master policies are not repeated here. A reader using this document on its own should refer to those policies for the shared glossary, including NABH, SHCO, OE, PPE, WHO and SOP.
...
STATUTE_CLAUSE = (
    "the professional-practice obligations under the National Medical Commission Act, 2019 "
    "and the Indian Nursing Council Act, 1947 insofar as registered practitioners must "
    "maintain resuscitation competence"
)
...
OE_MAPPING = [
    {
        ...
        "steps": "Steps 1, 5",
        "evidence": "The written response arrangement naming who responds, how they are summoned, and the arrangement when the ordinary team is already occupied, including nights, weekends and holidays; roster or equivalent showing a service outside ordinary hours; the audit sample at step 5 of a service that can be shown to exist outside ordinary hours",
        "responsible": "Named responders attend when summoned; head of the institution is accountable that the service exists at all times; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "COP.3.b",
    ...
    },
    {
        ...
        "steps": "Steps 3, 5",
        "evidence": "The written list of areas that hold a kit, the contents, and the completeness-check method; kit-check records for those areas; records of used reusable items sent for reprocessing under HIC.6 and the kit restored to ready rather than a used item returned to the trolley; the American Heart Association edition and algorithm in force, not printed as a numbered protocol in this policy; the audit sample at step 5 of kit checks and restoration after use",
        "responsible": "Staff in each named area check and restore the kit; HIC.6 owns reprocessing of reusable items; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "COP.3.d",
        ...
        "steps": "Steps 4, 5",
        "evidence": "The written committee composition, meeting interval and location of minutes; sample analyses covering events that restored circulation and events that did not; assigned corrective and preventive actions with dates and closure; the audit sample at step 5 of every event having a committee analysis with actions",
        "responsible": "The multidisciplinary committee analyses every event and assigns actions; the person assigned an action closes it; quality or accreditation coordinator audits",
    },
]
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 2, printed pages 62-63
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="cop3_draft.json",
        sql_name="cop3_insert.sql",
    )
