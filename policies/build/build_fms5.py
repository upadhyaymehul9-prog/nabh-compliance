# -*- coding: utf-8 -*-
"""Builds the FMS.5 master policy draft: JSON for review + SQL for later insert.
...
FMS chapter calibration (owner 2026-08-18): technical T2 (exit plan; drills;
continuity) carries NBC/NDMA substance. Asterisked OEs still get full Tier 1.

Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): FMS.5.a, FMS.5.d
  Tier 2 (lighter pass):   FMS.5.b, FMS.5.c, FMS.5.e
  c states "at least twice a year" from the OE.

Official source: NABH SHCO 3rd Edition PDF, Chapter 8, FMS.5 printed page 118
(PDF index 124). Summary on printed page 115 uses American "organization";
the OE-page header uses "The organisation has plans…". FMS.5.b and FMS.5.c
have no terminal period on b in the book (c has a period). Mapping preserves
the book's b without a terminal period.

Asterisks verified 2026-08-18: FMS.5.a and FMS.5.d carry the asterisk.

PDF md5 39e3bc86d73d651b9cfef283bbf018a9.

NBC 2016 (chapter reference 4) is named in P2 insofar as the local fire
authority applied it (detection, abatement, containment, occupancy). NDMA
Hospital Safety Guidelines 2016 (chapter reference 9) is a non-fire/disaster
framework, not an Act. IS 2190 is NBC-pointed extinguisher practice, not an
extra P2 statute. No invented sprinkler/bed-count/occupancy-subdivision
mandate for every SHCO.
"""
...
CHAPTER = "FMS"
OE_CODES = [
    "FMS.5.a", "FMS.5.b", "FMS.5.c", "FMS.5.d", "FMS.5.e",
]
TIER1_OES = ["FMS.5.a", "FMS.5.d"]
...
{{HOSPITAL_NAME}} does not treat a fire NOC in a drawer, an extinguisher that was never inspected, or a drill that never left the quality office, as that duty."""

PROCEDURE_STEPS = [
"""1. Plans and provisions for early detection, abatement and containment
...
How the exit plan is documented, where it is displayed, and how a change in a fire door or a ward move updates the display, are [Hospital to define — the documented and displayed exit plan for fire and non-fire emergencies]. FMS.1.b drawings are the controlled set this display must match.""",

"""3. Mock drills at least twice a year
...
"""4. Maintenance plan for fire-related equipment and infrastructure
...
How each essential service continues or is paused, the named receiving arrangement if diversion is the plan, how medical-gas and electrical backups (FMS.4.c / FMS.1.e) are used in that hour, and who declares continuity versus full evacuation, are [Hospital to define — the service continuity plan in case of fire and non-fire emergencies]. AAC.1 unused services are not given a continuity annex.""",
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a drill failed or a fire-door closer was found disconnected, or when FMS.1, FMS.2, FMS.4, AAC.6 or ROM.4 that this document hands work to are revised.""",
]

RESPONSIBILITY = """The head of the institution is accountable that fire and non-fire emergency plans exist and are provisioned as this document requires.
...
All staff are expected to treat a blocked exit, a discharged extinguisher left in place, a fire door wedged open, and a year with fewer than two drills, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Chapter 8 FMS, standard FMS.5.
...
- Internal documents of {{HOSPITAL_NAME}}: fire plan; non-fire plans; exit display; drill file; fire-equipment maintenance; service continuity plan; FMS.1, FMS.2, FMS.4; AAC.6; ROM.4; drawings at FMS.1.b."""

DISTRIBUTION = """Controlled master copy: office of the head of the institution, {{HOSPITAL_NAME}}, with the named fire or facilities lead and the quality or accreditation coordinator.
...
OE_MAPPING = [
    {
        ...
        "steps": "Steps 1, 6",
        "evidence": "The fire plan covering detection, abatement and containment with provisions that match the occupancy the local fire authority actually required rather than a municipal NOC in a drawer or a copied sprinkler mandate for every SHCO; non-fire plans for events this building faces using NDMA Hospital Safety 2016 (chapter reference 9) as framework rather than a photocopied CBRN annex; the recorded use of NBC 2016 (chapter reference 4) as applied, not as a pasted code; the recorded splits that AAC.6 lab fire points at this plan, FMS.4 owns gas-handling SOP, COP.8/COP.12 remain care method for dependent patients, and ROM.4.a remains management risk duty; induction of fire wardens; the location of the plans; the audit sample at step 6 of a provision that exists in the building (detector, extinguisher, door closer) not only on paper",
        "responsible": "Named fire or facilities lead holds the plans and provisions; AAC.6/FMS.4/ROM.4 remain those documents; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.5.b",
        "requirement": "The organisation has a documented and displayed exit plan in case of fire and non-fire emergencies",
        "steps": "Steps 2, 6",
        "evidence": "The documented exit plan and inspection that it is displayed on occupied floors, matches FMS.1.b drawings, and is not AAC.6.e radiation signs or FMS.1.c wayfinding counted twice; the audit sample at step 6",
        "responsible": "Named fire lead; FMS.1.c and AAC.6.e remain those signs; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.5.c",
        "requirement": "Mock drills are held at least twice a year.",
        "steps": "Steps 3, 6",
        "evidence": "Drill records meeting at least twice a year covering fire and a named non-fire, including a dependent-patient move and a night or weekend sample across the year, rather than two tabletop minutes in the quality office; HRM.3 flagged as future training method; PSQ.5 dual entry if harm occurred; the audit sample at step 6",
        "responsible": "Named fire lead schedules drills; HRM.3 when drafted trains to this plan; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.5.d",
        "requirement": "There is a maintenance plan for fire-related equipment and infrastructure.",
        "steps": "Steps 4, 6",
        "evidence": "The maintenance plan and job cards for detectors, alarms, extinguishers, hose reels, fire doors, emergency lighting and any pump or sprinkler this occupancy actually has, showing last inspection and a failed item isolated with a compensating provision, rather than a vendor contract with no job card or a fire NOC treated as proof the pump still starts; the recorded use of IS 2190 as NBC-pointed extinguisher practice with hospital-defined local interval, not a NABH monthly-extinguisher mandate stacked on FMS.2.d; the recorded split that FMS.3 biomedical PPM is not detector maintenance and FMS.2.d may find a failure this plan must close; induction of the maintainer; the location of the plan; the audit sample at step 6 of an extinguisher gauge and a door closer that were actually inspected",
        "responsible": "Named fire or facilities lead holds fire-equipment maintenance; FMS.2.d and FMS.3 remain those programmes; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.5.e",
        "requirement": "The organisation has a service continuity plan in case of fire and non-fire emergencies.",
        "steps": "Steps 5, 6",
        "evidence": "The continuity plan showing how AAC.1 essential services continue, pause or divert with a named receiving arrangement and use of FMS.1.e/FMS.4.c backups, rather than an unnamed 'nearby hospital'; the audit sample at step 6",
        "responsible": "Named fire lead with clinical heads; AAC.1/AAC.2/AAC.7 remain those routes; quality or accreditation coordinator audits",
    },
]
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 8 FMS, printed page 118
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="fms5_draft.json",
        sql_name="fms5_insert.sql",
    )
