# -*- coding: utf-8 -*-
"""Shared emit + numbering checks for MOM v2 (adoptable-policy shape).

Used by build_mom1_v2.py … build_mom9_v2.py. Does not write SQL.
"""
...
def build_markdown(draft: dict) -> str:
    ...
    lines = [
        ...
        f"**{HOSPITAL}**",
        "",
        f"*{draft.get('subtitle') or 'Medication management in day-to-day care.'}*",
        "",
        "## Document control",
        "",
...
Issue number: {D('01')}
Version: {version} (MOM v2 draft — not an approved master)
Date created: {BLANK}
