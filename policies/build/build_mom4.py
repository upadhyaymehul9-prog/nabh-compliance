# -*- coding: utf-8 -*-
"""Builds the MOM.4 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): MOM.4.a only
  Tier 2 (lighter pass):   MOM.4.b, MOM.4.c, MOM.4.d

ONE of four OEs is asterisked. The draft builds a deep block for a.
b-d are lean.

Official source: NABH Standards for Small Healthcare Organisations, 3rd Edition
(August 2022), Chapter 3, standard MOM.4 and OEs MOM.4.a-d, read from the
official standards PDF (downloaded 2026-08-17 from the NABH website's Explore
NABH Standards page), printed page 78, PDF page index 84.

The OE-page header uses the book's grammar: "Medications orders are written in
a uniform manner." (Chapter summary printed page 75, PDF page index 81, uses
"Medication orders". This draft quotes the OE-page header as the standard
header for MOM.4.)
...
committed scripts/shco_oe_asterisks.json on all 408 entries) and the MOM.4
page read directly. MOM.4.a carries the asterisk; MOM.4.b-d are unasterisked.
"""
...
CHAPTER = "MOM"
OE_CODES = [
    "MOM.4.a", "MOM.4.b", "MOM.4.c", "MOM.4.d",
]
TIER1_OES = ["MOM.4.a"]

POLICY_TITLE = "Uniform Medication Orders"
...
- The two identifiers used at the point of care are governed by the uniform-care policy of {{HOSPITAL_NAME}} (COP.1). Name and unique identification number on the order location are identity on the artefact; they are not a substitute for the administration-time identity check under MOM.6.c."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} requires that only authorised personnel write medication orders. An order written by a person who is not authorised is not an order of this hospital and is not acted on.
...
{{HOSPITAL_NAME}} treats a verbal order, once written, as a medication order under this document."""

PROCEDURE_STEPS = [
"""1. Only authorised personnel write medication orders
...
An order found to have been written by a person who was not authorised is not acted on. It is returned for a valid order, and the event is treated as a defect, not as a paperwork inconvenience.""",

"""2. Uniform location, with the patient's name and unique identification number
...
The medical record itself remains under the information-management policies. This step owns only that the medication order has one home in that record, and that the home identifies the patient.""",

"""3. Orders are legible, dated, timed and signed
...
Countersignature of a transcribed verbal order follows the verbal-order process of the safe-and-rational-prescribing policy; the written line still has a time, a date and the writer's identification under this step.""",

"""4. Required content of a medication order
...
A change of dose, route or frequency is a new order under this step: dated, timed, signed, in the uniform location, written by an authorised person. A cancelled order is marked so that it cannot be read as current.""",
...
RESPONSIBILITY = """The head of the institution is accountable for {{HOSPITAL_NAME}} accepting medication orders only from authorised personnel, only in the uniform location, and only when they are legible, dated, timed, signed and complete.
...
All staff are expected to treat an unauthorised writer, an order outside the uniform location, an illegible or unsigned order that was acted on, and a missing content element, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Chapter 3 Management of Medication, standard MOM.4.
...
STATUTE_CLAUSE = (
    ...
    "write a medication order as a medical practitioner, the Indian Nursing "
    "Council Act, 1947 insofar as hospital rules permit a registered nurse to "
    "write a medication order within that professional scope, and the Drugs "
    "and Cosmetics Act, 1940 and the rules under it insofar as a prescription "
    "or medication order is a legal document for the sale or supply of "
    "Schedule H and Schedule H1 medicines"
)
...
OE_MAPPING = [
    ...
    },
    {
        ...
        "steps": "Steps 4, 5",
        "evidence": "Sample orders in the uniform location showing name of medicine, route, strength and frequency or time of administration; records of an incomplete order not dispensed and not administered until completed; records of a change of dose, route or frequency entered as a new dated, timed and signed order; the recorded division that the prescribing policy owns prescription minimums as a prescribing standard and this step owns the written artefact",
        "responsible": "Persons who write orders include the four content elements; pharmacy and administering staff refuse an incomplete order; quality or accreditation coordinator audits",
    },
]
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 3, printed page 78
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="mom4_draft.json",
        sql_name="mom4_insert.sql",
    )
