# -*- coding: utf-8 -*-
"""Insert COP.1–COP.13 master-policy drafts into live Supabase. Never approves.

Owner confirmation for this write (2026-08-17): insert all thirteen rows, leave
...
Usage:
  python3 policies/build/apply_cop_drafts_supabase.py --insert
  python3 policies/build/apply_cop_drafts_supabase.py --verify-only
  python3 policies/build/apply_cop_drafts_supabase.py --dry-run
"""
import argparse
import os
import sys
...
CODES = [f"COP.{n}" for n in range(1, 14)]
_POLICIES = _HERE.parent


def draft_path_for(code):
    n = code.split(".", 1)[1]
    return _POLICIES / "drafts" / f"cop{n}_draft.json"


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--insert", action="store_true",
                    help="POST each missing COP.1–COP.13 row (status stays draft)")
    ap.add_argument("--verify-only", action="store_true",
                    help="only verify live rows against local drafts")
    ap.add_argument("--dry-run", action="store_true",
                    help="no network: validate all 13 drafts and print the plan")
    args = ap.parse_args()
    ...
    if args.dry_run:
        print("\nDRY RUN -- no network calls. Plan for each of COP.1–COP.13:")
