# -*- coding: utf-8 -*-
"""Builds the COP.4 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): COP.4.a (Core), COP.4.d (Excellence)
  Tier 2 (lighter pass):   COP.4.b, COP.4.c

COP.4.a is Core AND asterisked, so it is Tier 1. Core without an asterisk would
be Tier 2 (see COP.1.a). The asterisk, not the Core label, allocates depth.

Official source: NABH Standards for Small Healthcare Organisations, 3rd Edition
(August 2022), Chapter 2 Care of Patients, standard COP.4 and OEs COP.4.a-d,
read from the official standards PDF (downloaded 2026-08-17 from the NABH
website's Explore NABH Standards page), printed page 63, PDF page index 69.
...
committed scripts/shco_oe_asterisks.json on all 408 entries) and the COP.4 page
read directly. COP.4.a and COP.4.d carry the asterisk; COP.4.b and COP.4.c are
unasterisked.
"""
...
PURPOSE = """This document sets out how {{HOSPITAL_NAME}} provides nursing care in consonance with clinical protocols: nursing care aligned and integrated with overall patient care and documented in the patient record; assignment of patient care according to current good clinical and nursing practice; nurses provided with the equipment that care requires; and nursing clinical practice guidelines that reflect current standards of practice and are actually used.

Nursing that is done and not written, assigned by habit rather than by the patient's need, or guided by a medical protocol that never tells a nurse what to observe, is not the care this standard asks for. This document is the process that makes nursing visible in the record, assigned, equipped and guided."""
...
- Planned maintenance of equipment may be performed under the facility-management policies of {{HOSPITAL_NAME}} (FMS, not yet drafted). This policy owns that the equipment nurses need is present and usable at the point of care."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} aligns and integrates nursing care with overall patient care, and documents that nursing care in the patient record. Care that was given and not written is not available to the next nurse, the treating clinician, or the record.
...
{{HOSPITAL_NAME}} develops and implements nursing clinical practice guidelines that reflect current standards of practice. A guideline that exists as a file and is not used is not implemented."""

PROCEDURE_STEPS = [
"""1. Nursing care aligned, integrated and documented in the patient record
...
The unique identification number on every nursing entry is the number issued at registration (AAC.2.b). This step does not generate it.""",

"""2. Assignment of patient care
...
The assignment is recorded. A patient without a named assigned nurse for the current period is a break in nursing care.""",

"""3. Equipment for safe and efficient nursing
...
This document does not state a required number of devices per bed.""",

"""4. Nursing clinical practice guidelines
...
A new or revised guideline is issued to every setting in which that nursing care is given, and the previous version is withdrawn on the same date — the same control the uniform-care policy uses for medical protocols, applied here to nursing guidelines.""",
...
RESPONSIBILITY = """The head of the institution is accountable for {{HOSPITAL_NAME}} documenting nursing care in the patient record, for assignment that follows current good practice, for equipment that is actually present, and for nursing clinical practice guidelines that are used.
...
All staff are expected to treat a patient whose nursing care was not written, a period with no named assigned nurse, and a guideline that is not the version in use, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Care of Patients chapter, standard COP.4.
...
STATUTE_CLAUSE = (
    "the Indian Nursing Council Act, 1947 and State Nursing Council registration, insofar as "
    "who may be assigned as a nurse and as the professional-practice context of nursing "
    "clinical practice guidelines"
)
...
OE_MAPPING = [
    ...
    },
    {
        ...
        "steps": "Steps 3, 5",
        "evidence": "The written nursing-equipment list by area and the method for reporting absence or failure; sample reports of missing or failed equipment and the action taken; records showing reusable items counted as available only after reprocessing under HIC.6; the audit sample at step 5 of equipment-absence reports that were acted on",
        "responsible": "Nursing administration and the nurse in charge keep equipment present; HIC.6 owns reprocessing; facility-management policies may perform planned maintenance; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "COP.4.d",
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 2, printed page 63
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="cop4_draft.json",
        sql_name="cop4_insert.sql",
    )
