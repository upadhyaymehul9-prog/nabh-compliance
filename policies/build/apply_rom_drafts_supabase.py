# -*- coding: utf-8 -*-
"""Insert ROM.1–ROM.4 master-policy drafts into live Supabase. Never approves.

These drafts are UNAPPROVED. Do not run --insert until the owner confirms the
write. When that write is confirmed, insert all four rows, leave status = 'draft'
on every one, do not approve, do not generate test docs.

This chapter is ROM (Responsibilities of Management) in the SHCO 3rd Edition,
Chapter 7. Codes are ROM.1–ROM.4.
...
Usage:
  python3 policies/build/apply_rom_drafts_supabase.py --insert
  python3 policies/build/apply_rom_drafts_supabase.py --verify-only
  python3 policies/build/apply_rom_drafts_supabase.py --dry-run
"""
import argparse
import os
import sys
...
CODES = [f"ROM.{n}" for n in range(1, 5)]
_POLICIES = _HERE.parent


def draft_path_for(code):
    n = code.split(".", 1)[1]
    return _POLICIES / "drafts" / f"rom{n}_draft.json"


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--insert", action="store_true",
                    help="POST each missing ROM.1–ROM.4 row (status stays draft)")
    ap.add_argument("--verify-only", action="store_true",
                    help="only verify live rows against local drafts")
    ap.add_argument("--dry-run", action="store_true",
                    help="no network: validate all 4 drafts and print the plan")
    args = ap.parse_args()
    ...
    if args.dry_run:
        print("\nDRY RUN -- no network calls. Plan for each of ROM.1–ROM.4:")
        ...
        print("Codes are ROM.1–ROM.4 (3rd Edition Chapter 7).")
        return 0
