# -*- coding: utf-8 -*-
"""Parse official portal PRE chapter OCR into inventory JSON.

Authoritative source: policies/source/hco6_pre_ocr.txt (pdftotext of portal
Standards PDF pages 113–121). Does not use the Guidebook for counts/levels/stars.
"""
...
OCR = ROOT / "policies/source/hco6_pre_ocr.txt"
OUT = ROOT / "policies/source/hco6_pre_inventory.json"

LIGATURE_MAP = str.maketrans({"\uf001": "fi", "\uf002": "fl", "\ufb01": "fi", "\ufb02": "fl"})
LEVELS = ("CORE", "Commitment", "Achievement", "Excellence")
LETTER_RE = re.compile(
    r"^\s+(CORE|Commitment|Achievement|Excellence)\s+([a-lA-L])\.\s+(.*)$"
)
STD_RE = re.compile(r"^\s+PRE\.(\d+)\.\s*(.*)$")
...
def parse() -> dict:
    ...
    # Body starts at "Standards and Objective Elements"
    ...
    for line in body.splitlines():
        ...
        if mstd and (in_title or stripped.startswith("PRE.")):
            flush_oe()
            n = mstd.group(1)
        ...
        if in_title:
            if re.match(r"^\s*Objective Elements\s*$", line):
                flush_title()
                continue
            if stripped and not is_footer(stripped) and not stripped.startswith("PRE."):
...
def main() -> int:
    ...
    for n in sorted(inv, key=int):
        ...
        print(f"PRE.{n}: {d['count']} [{letters}] {d['title'][:70]}")
        for o in d["oes"]:
            star = "*" if o["star"] else " "
    ...
    assert cores == 12 and comm == 32 and ach == 7 and exc == 1, (cores, comm, ach, exc)
