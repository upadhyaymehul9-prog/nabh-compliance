# -*- coding: utf-8 -*-
"""Builds the AAC.1 master policy draft: JSON for review + SQL for the Supabase SQL Editor.

Column types confirmed against the live schema (2026-08-10), plus the version /
revision_history infrastructure added by migration
supabase/migrations/20260812_shco_policy_masters_version_revision_history.sql:
  oe_codes         text[]
  procedure_steps  text[]
  oe_mapping       jsonb
  version          text        (semantic version as text; default '1.0')
  revision_history jsonb       (array of {version, date, description}; dates DD-MM-YYYY)

THIS IS THE SEVENTH MASTER POLICY AND THE FIRST DRAFTED UNDER THE TWO-TIER DEPTH
STANDING RULE (scripts/master-policy-todos.md, added 2026-08-10). Tier is decided
by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): AAC.1.c only -- the sole asterisked OE of the standard.
  Tier 2 (lighter pass):   AAC.1.a, AAC.1.b, AAC.1.d.
Depth of content per OE follows the tier. Structure does not: full control box,
OE cross-reference table, abbreviations with the HIC-glossary back-pointer, the
numbered sections, the hash-checked disclaimer, status = 'draft', the five
optional sections left unset, and policy_placeholder_audit.py wired in from this
first build.

Official source: NABH Standards for Small Healthcare Organisations, 3rd Edition
(August 2022), Chapter 1 Access, Assessment and Continuity of Care, standard AAC.1
and OEs AAC.1.a-d, read from the official standards PDF (downloaded 2026-08-17 from
the NABH website's Explore NABH Standards page), printed page 50, PDF page index 56.

Levels, verified against the PDF and against the repo extraction: a Commitment,
b Commitment, c Commitment, d Commitment. ONE OE CARRIES THE ASTERISK -- AAC.1.c
("Scope of healthcare services of each department is defined. *"). Verified three
ways on 2026-08-17: (1) scripts/asterisk_extract.py re-run against the freshly
downloaded PDF -- self-validation passed, 408 OEs, 132 asterisks, and its output
matched the committed scripts/shco_oe_asterisks.json on every one of the 408
entries; (2) the AAC.1 page read directly by eye from the extracted page text;
(3) scripts/shco_oe_asterisks.json was itself verified byte-for-byte against the
live shco_full_oes on 2026-08-13 (132 true / 276 false / 408 total on both sides).
AAC.1 was not among the 14 mismatches of the 2026-08-10 audit.

version and revision_history are populated in the INSERT from the start -- the
first master policy to do so. HIC.1-6 predate the migration and were backfilled.

The five optional sections (definitions, training_competency, resources_required,
monitoring_audit, exceptions) are deliberately left unset, matching HIC.1-6.
"""
import json
import re
...
POLICY_TITLE = "Definition and Display of Healthcare Services"
...
PURPOSE = """This document sets out how {{HOSPITAL_NAME}} decides which healthcare services it provides, satisfies itself that each of them can actually be delivered, writes down what every department does and does not do, and tells the public.
...
- Isolation and infection control signage is governed by the infection control policies of {{HOSPITAL_NAME}} and has nothing to do with the display governed here."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} defines its healthcare services in writing, approves the definition at the level of the head of the institution, keeps it consistent with the registrations and licences the hospital holds, and grounds it in an assessment of the needs of the community it serves.
...
{{HOSPITAL_NAME}} expects every member of staff to know the scope of their own department, and treats directing a patient to another provider, when this hospital cannot care for them, as correct practice and never as lost business."""

# ---------------------------------------------------------------------------
# Procedure. Tier per OE:
#   Step 1        AAC.1.a  Tier 2
#   Step 2        AAC.1.b  Tier 2
#   Steps 3-5     AAC.1.c  Tier 1  (the asterisked documented-evidence anchor)
#   Step 6        AAC.1.d  Tier 2
#   Step 7        cross-cutting change control (all four OEs)
# ---------------------------------------------------------------------------
PROCEDURE_STEPS = [
"""1. Defining the healthcare services of {{HOSPITAL_NAME}}
...
The directory is reviewed at [Hospital to define — review interval for the service directory], and immediately upon any service being added, suspended or withdrawn under step 7.""",

"""2. Resourcing every defined service
...
The resourcing behind each service is confirmed at every review of the directory, against the current staffing and equipment rather than against what was true when the service was introduced.""",

"""3. The scope of services of each department — what it states
...
The commonest defect in this document is genericness — a scope copied between departments, or downloaded, that describes a department the hospital does not have. The test applied at {{HOSPITAL_NAME}} is specificity: a reader who knows the hospital should be able to tell from the scope alone which department it describes, and a reader who knows the department should find nothing in the scope it does not actually do. Each department's scope is held at [Hospital to define — where each department's scope of services is held and how the set is indexed].""",

"""4. The scope of services — the boundary matters more than the list
...
When a department's real capability moves — a specialist leaves, a machine fails, a dependency named in the scope becomes unavailable — the boundary has moved with it whether or not the paper is updated, and the paper is updated so that staff are applying the real boundary and not last year's. The route for that update is step 7, and the department head's duty to trigger it is stated there.""",

"""5. The scope of services — approval, alignment, review and communication
...
Responsibility for keeping every display current sits with [Hospital to define — the role responsible for the display], who corrects it within the timescale at step 7 whenever the directory changes.""",

"""7. Changing the services — the order of operations
...
Every change, temporary or permanent, updates all four aligned documents at step 5 or records why one is unaffected. The change and its date are recorded, and the head of the institution is informed of every suspension and withdrawal."""
]

RESPONSIBILITY = """The head of the institution approves the service directory and every department scope of services, provides the personnel, equipment and arrangements that resourcing a defined service requires, is informed of every suspension and withdrawal, and is accountable for {{HOSPITAL_NAME}} holding out no service it cannot deliver.
...
All staff are expected to know the scope of their own department, to direct a patient whose needs fall outside it along the referral route the scope names, and to report a display or directory statement they know to be no longer true."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Access, Assessment and Continuity of Care chapter, standard AAC.1.
...
# Verbatim from the approved HIC.3 / HIC.4 / HIC.5 / HIC.6 master policies -- do not edit.
# The master template boilerplate is shared across the set, so any change belongs in a
# deliberate pass over all of them, not in this file. Verified below against the shared
# block by hash (same md5 as the live HIC.5 row, LF-normalised).
...
# 2026-08-10. HIC.3, HIC.4, HIC.5 and HIC.6 all carry this identical value; AAC.1 makes five.
SHARED_DISCLAIMER_MD5_LF = "ae331bb0cb2ca6428d4d1e0800e51e60"

OE_MAPPING = [
    {
        "oe_code": "AAC.1.a",
        "requirement": "The healthcare services provided are defined and are in consonance with the needs of the community",
        "steps": "Steps 1, 7",
        "evidence": "The service directory of {{HOSPITAL_NAME}}, approved by the head of the institution, dated and version-controlled, listing every service with its out-patient, in-patient and emergency availability; the community needs assessment with its date and the catchment, disease-profile, other-provider and demand information it relied on; the record of directory reviews at the stated interval and on every change; evidence of consistency between the directory and the hospital's registrations and licences",
        "responsible": "Head of the institution approves the directory; the administration maintains it and holds the community needs assessment; quality or accreditation coordinator audits the review cycle",
    },
    {
        "oe_code": "AAC.1.b",
        "requirement": "Each defined healthcare service has diagnostic and treatment services with suitably qualified personnel providing out-patient, in-patient and emergency cover",
        "steps": "Steps 2, 7",
        "evidence": "Per defined service, the personnel behind it with their professional council registrations and qualification verification under the human resource procedures; the diagnostic and treatment arrangement for each service, in-house or documented external; the written emergency cover arrangement per service — resident cover or the on-call roster with its response arrangement; visiting-consultant terms and the stated arrangement outside visiting hours; the recorded limitation and referral route for any service offered on a narrower basis; the resourcing confirmation made at each directory review",
        "responsible": "Head of the institution for resourcing; heads of departments for rosters and cover in their service; human resource function for registration and qualification verification",
    },
    {
        "oe_code": "AAC.1.c",
        "requirement": "The scope of healthcare services of each department is defined",
        "steps": "Steps 3-5, 7",
        "evidence": "The written scope of services of every department, clinical and diagnostic, each authored by the head of that department and approved by the head of the institution, dated, version-controlled and signed by both; each scope stating the services and procedures performed, the patient populations accepted with the ages and categories not accepted, the acuity manageable and the point beyond it, the hours of each service and the out-of-hours arrangement, the dependencies relied on and where each is met from, the conditions stabilised and referred with the referral destination, and the person in charge; the specific exclusions written to the precision step 4 requires, each pointing to a response rather than ending in a refusal; the four-way alignment check record — directory against scopes, scopes against registrations and licences, scopes against current personnel and equipment, scopes against the display — with mismatches found, the document corrected and the correction recorded; review records at the stated interval and on every trigger event, showing the trigger honoured when a specialist left, equipment changed or a dependency moved; induction records showing each department's staff received its scope; the current scope available within the department; and the full set or consolidated summary held at reception, the front office and the emergency area",
        "responsible": "Heads of departments author and keep current the scope of their own department and trigger its review the moment capability changes; head of the institution approves every scope; the administration performs and records the four-way alignment check; reception, front office and emergency hold the set and answer the public from it; quality or accreditation coordinator audits currency at the review interval",
    },
    {
        "oe_code": "AAC.1.d",
        "requirement": "The defined healthcare services are prominently displayed",
        "steps": "Steps 6, 7",
        "evidence": "The display itself at the main entrance, reception and the emergency entrance and at any further stated location, in the stated languages, legible and maintained; the displayed content matching the service directory exactly, with round-the-clock services identified; the match check between display and directory performed with the alignment check at step 5; consistency of printed material and the online presence with the display; the record of prompt correction of every display on each suspension, withdrawal or addition, with dates; and the statutory displays attaching to specific services in place where those services are provided",
        "responsible": "The role named at step 6 maintains every display and corrects it at once on any change; the administration controls the displayed content against the directory; heads of departments report capability changes that the display must follow",
    },
]
...
# renderer does not emit those headings, matching HIC.1-6.
...
with open(DRAFTS / "aac1_draft.json", "w", encoding="utf-8", newline="\n") as f:
...
sql = f"""-- AAC.1 master policy -- DRAFT for review. Do NOT set status = 'approved' here;
...
with open(SQL_OUT / "aac1_insert.sql", "w", encoding="utf-8", newline="\n") as f:
    f.write(sql)
...
print("mapping covers all 4 OEs:", sorted(m["oe_code"] for m in OE_MAPPING) == sorted(OE_CODES))
print("every mapping row has evidence + responsible:",
      all(m.get("evidence") and m.get("responsible") for m in OE_MAPPING))

# The renderer splits evidence on ';' into Required Records rows -- no empty records.
for m in OE_MAPPING:
...
# Tier discipline: AAC.1.c is the documented-evidence anchor and must carry the
# deepest evidence column of the four -- a later edit that inverts this has broken
# the two-tier allocation and should fail loudly.
_ev = {m["oe_code"]: len(m["evidence"]) for m in OE_MAPPING}
assert max(_ev, key=_ev.get) == "AAC.1.c", f"evidence depth anchor is not AAC.1.c: {_ev}"
print("Tier 1 evidence anchor is AAC.1.c:", _ev)

# version and revision_history -- populated from the start, in the formats the
# renderer and the migration expect.
assert re.fullmatch(r"\d+\.\d+", VERSION), f"version not semantic text: {VERSION}"
assert isinstance(REVISION_HISTORY, list) and REVISION_HISTORY, "revision_history empty"
...
assert REVISION_HISTORY[-1]["version"] == VERSION, "version does not match latest revision entry"
print("version + revision_history populated and well-formed:", VERSION, REVISION_HISTORY)

# Placeholder integrity: template tokens must be double-braced. A single-braced
# {HOSPITAL_NAME} is the str.format() collapse that reached the local HIC.2 draft
# (see scripts/master-policy-todos.md, 2026-08-13) -- never use str.format() here.
_all_text = " ".join([PURPOSE, SCOPE, POLICY_STATEMENT, RESPONSIBILITY, REFERENCES,
...
# The disclaimer must stay byte-identical to the shared approved block, ignoring
# line endings.
import hashlib
_d = hashlib.md5(DISCLAIMER.replace("\r", "").encode("utf-8")).hexdigest()
assert _d == SHARED_DISCLAIMER_MD5_LF, f"disclaimer drifted from the shared HIC.3-6 block: {_d}"
print("disclaimer verbatim identical to the shared HIC.3-6 block (LF-normalised md5):", _d)

from policy_placeholder_audit import audit
_exact, _variant, _total, _problems = audit(draft)
assert not _problems, _problems

# The checklist states the placeholder inventory in prose. Assert the stated
# figures against the audit so the prose cannot go stale -- the HIC.3 "38 vs 40"
# error was exactly this figure drifting from the truth.
...
print("wrote aac1_draft.json and aac1_insert.sql")
