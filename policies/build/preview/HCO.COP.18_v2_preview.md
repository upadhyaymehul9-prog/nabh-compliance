# Rehabilitation Services
**Preview Hospital**

*HCO Full Accreditation, 6th Edition — COP.18.*

## Document control

Document number: «HCO/COP/POL/18»
Issue number: «01»
Version: 2.0 (HCO Full v2 draft)
Date created: «________»
Date of implementation: «________»
Review due: «one year from implementation»

Prepared by (designation): «Rehabilitation In-Charge»    Name: «________»    Signature: «________»
Reviewed by (designation): «Quality Coordinator»    Name: «________»    Signature: «________»
Approved by (designation): «Medical Superintendent»    Name: «________»    Signature: «________»

Amendment sheet (add a line for each change after issue)

Sr | Section | Change | Reason | Prepared | Approved
1. |  |  |  |  | 

## Statement of intent

Rehabilitation services are provided to the patients in a safe, collaborative and consistent manner.

## 1. Purpose

This policy says how Preview Hospital meets NABH Hospitals 6th Edition standard COP.18: Rehabilitation services are provided to the patients in a safe, collaborative and consistent manner.

It covers objective elements COP.18.a–g (7 elements).

This policy owns COP.18. Related AAC, PRE, IPC/HIC, HRM and MOM duties stay with those policies — cross-reference only.

Words marked «like this» are defaults. A blank marked «________» must be filled before issue.

## 2. Scope

This policy applies to staff who deliver or oversee the care described in COP.18 at Preview Hospital, including the «Rehabilitation In-Charge», treating doctors, nurses and the Quality Coordinator.

It covers 7 objective elements (COP.18.a, COP.18.b, COP.18.c, COP.18.d, COP.18.e, COP.18.f, COP.18.g).

Boundaries: do not copy SHCO COP wording. Do not overwrite AAC policies. Spell out abbreviations on first use in training materials.

## 3. Policy standards

Preview Hospital implements COP.18 so that rehabilitation services are provided to the patients in a safe, collaborative and consistent manner..

Staff follow written guidance, keep the records listed in the OE table, and escalate when stop-work triggers fire (if this policy includes a stop-work section).

## 4. Non-negotiable rules

1. Do not skip: Scope of the rehabilitation services at a minimum is commensurate to the services provided by the organisat...
2. Do not skip: Rehabilitation services are provided in a consistent manner.
3. Do not skip: Care providers collaboratively plan rehabilitation services.
4. Do not skip: There are adequate space and equipment to provide rehabilitation.
5. Do not skip: Care is guided by functional assessment and periodic re-assessments which are done and documented.
6. Do not skip: Care is provided adhering to infection prevention and control and safety practices.
7. Do not skip: Care pathways are developed, implemented, and reviewed periodically.
8. Staff who see a COP.18 rule broken report it the same shift to the «department in-charge» or the «Medical Superintendent».

## 5. What we do

### 5.1 Scope of the rehabilitation services at a minimum is commensurate to ..

Scope of the rehabilitation services at a minimum is commensurate to the services provided by the organisation.

Staff follow the written guidance and record the action in the medical record or the department register named for this element.

### 5.2 Rehabilitation services are provided in a consistent manner

Rehabilitation services are provided in a consistent manner.

Staff follow the written guidance and record the action in the medical record or the department register named for this element.

### 5.3 Care providers collaboratively plan rehabilitation services

Care providers collaboratively plan rehabilitation services.

Staff follow the written guidance and record the action in the medical record or the department register named for this element.

### 5.4 There are adequate space and equipment to provide rehabilitation

There are adequate space and equipment to provide rehabilitation.

Staff follow the written guidance and record the action in the medical record or the department register named for this element.

### 5.5 Care is guided by functional assessment and periodic re-assessments w..

Care is guided by functional assessment and periodic re-assessments which are done and documented.

Staff follow the written guidance and record the action in the medical record or the department register named for this element.

### 5.6 Care is provided adhering to infection prevention and control and saf..

Care is provided adhering to infection prevention and control and safety practices.

Staff follow the written guidance and record the action in the medical record or the department register named for this element.

### 5.7 Care pathways are developed, implemented, and reviewed periodically

Care pathways are developed, implemented, and reviewed periodically.

Staff follow the written guidance and record the action in the medical record or the department register named for this element.

## 6. Governance and responsibility

Medical Superintendent
- Accountable that COP.18 is resourced and followed.

Rehabilitation In-Charge
- Owns day-to-day implementation and records for this standard.

Treating doctors / nurses / technicians (as applicable)
- Deliver care as written; escalate stop-work triggers without delay.

Quality Coordinator
- Audits this policy «quarterly»; holds training acknowledgements.

## 7. Quality monitoring (RCA → CAPA)

The Quality Coordinator audits this policy «quarterly».

What is monitored each quarter:

- Sample of records for each OE COP.18.a–g.
- Asterisked elements (none) have document evidence as required.
- CORE elements (none) show no critical gaps in the sample.
- Stop-work events (if any) are logged with outcome.

This policy is reviewed «annually», and sooner after a related adverse event.

## 8. Training and staff acknowledgement

Staff covered by this policy are trained at induction and «once a year» after that. Training covers the What-we-do steps, non-negotiables and stop-work (if present).

Staff acknowledgement

I have read this Rehabilitation Services policy of Preview Hospital. I will follow the processes described.

Name: ___________________________    Designation: ___________________________

Department / floor: ____________________    Date: ____________

Signature: ___________________________

(One row per staff member. The Quality Coordinator holds signed acknowledgements with the induction record.)

## 9. References

- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Accreditation Standards for Hospitals, 6th Edition (January 2025) — Care of Patients, standard COP.18.
- NABH Guidebook to Accreditation Standards for Hospitals, 6th Edition — COP.18 interpretations (source PDF md5 2c4489ee98de4ae9b49cba168ea9f42a).
- Internal documents of Preview Hospital: written guidance and registers named for COP.18.

## 10. Distribution

Medical Superintendent; Rehabilitation In-Charge; Quality Coordinator; department clinical staff covered by COP.18

## 11. Abbreviations

COP — Care of Patients (NABH Hospitals chapter)
NABH — National Accreditation Board for Hospitals and Healthcare Providers
OE — Objective Element
CORE — Core objective element (NABH)
RCA — Root Cause Analysis
CAPA — Corrective and Preventive Action

## 12. Traceability to NABH HCO Full Accreditation 6th Edition COP.18

This table is an index. It is not how the policy is organised.

| OE | Requirement | Where this policy addresses it | Responsible |
| --- | --- | --- | --- |
| COP.18.a | Scope of the rehabilitation services at a minimum is commensurate to the services provided by the organisation. | Section 3; 5.1 | Rehabilitation In-Charge |
| COP.18.b | Rehabilitation services are provided in a consistent manner. | Section 3; 5.2 | Rehabilitation In-Charge |
| COP.18.c | Care providers collaboratively plan rehabilitation services. | Section 3; 5.3 | Rehabilitation In-Charge |
| COP.18.d | There are adequate space and equipment to provide rehabilitation. | Section 3; 5.4 | Rehabilitation In-Charge |
| COP.18.e | Care is guided by functional assessment and periodic re-assessments which are done and documented. | Section 3; 5.5 | Rehabilitation In-Charge |
| COP.18.f | Care is provided adhering to infection prevention and control and safety practices. | Section 3; 5.6 | Rehabilitation In-Charge |
| COP.18.g | Care pathways are developed, implemented, and reviewed periodically. | Section 3; 5.7 | Rehabilitation In-Charge |

## 13. Required Records / Evidence Checklist

Records the hospital holds under this policy, listed by objective element.

### COP.18.a — Scope of the rehabilitation services at a minimum is commensurate to the services provided by the organisation.

- Records showing COP.18.a was followed for sampled patients/cases.
- Written guidance / protocol referenced for this element (where required).
- Audit sample notes for COP.18.a reviewed «quarterly».

### COP.18.b — Rehabilitation services are provided in a consistent manner.

- Records showing COP.18.b was followed for sampled patients/cases.
- Written guidance / protocol referenced for this element (where required).
- Audit sample notes for COP.18.b reviewed «quarterly».

### COP.18.c — Care providers collaboratively plan rehabilitation services.

- Records showing COP.18.c was followed for sampled patients/cases.
- Written guidance / protocol referenced for this element (where required).
- Audit sample notes for COP.18.c reviewed «quarterly».

### COP.18.d — There are adequate space and equipment to provide rehabilitation.

- Records showing COP.18.d was followed for sampled patients/cases.
- Written guidance / protocol referenced for this element (where required).
- Audit sample notes for COP.18.d reviewed «quarterly».

### COP.18.e — Care is guided by functional assessment and periodic re-assessments which are done and documented.

- Records showing COP.18.e was followed for sampled patients/cases.
- Written guidance / protocol referenced for this element (where required).
- Audit sample notes for COP.18.e reviewed «quarterly».

### COP.18.f — Care is provided adhering to infection prevention and control and safety practices.

- Records showing COP.18.f was followed for sampled patients/cases.
- Written guidance / protocol referenced for this element (where required).
- Audit sample notes for COP.18.f reviewed «quarterly».

### COP.18.g — Care pathways are developed, implemented, and reviewed periodically.

- Records showing COP.18.g was followed for sampled patients/cases.
- Written guidance / protocol referenced for this element (where required).
- Audit sample notes for COP.18.g reviewed «quarterly».

## Disclaimer

This document is a template prepared for the guidance of Preview Hospital and must be reviewed, adapted and formally approved by Preview Hospital before use. Every entry marked [Hospital to define] must be replaced with the hospital's own decision; a document issued with those markers left in place is not an approved policy.

The requirements in this document are accreditation requirements of the NABH Full Accreditation Standards for Hospitals, 6th Edition, not duties under a named Act of Parliament. This policy does not import the Consumer Protection Act, 2019, the Clinical Establishments Act, 2010, or the Mental Healthcare Act, 2017 as a checklist. Statutory duties that arise under other documents of Preview Hospital remain those documents. Preview Hospital is responsible for verifying any statutory duty that applies to it; this document does not constitute legal advice.

The clinical and technical content reflects recognised national and international guidance current at the date of preparation. Preview Hospital remains responsible for verifying that it is current and consistent with the edition of the accreditation standard against which it is being assessed.

This document is not issued by, endorsed by, or affiliated with NABH, the World Health Organization, the National Centre for Disease Control, the Food Safety and Standards Authority of India, any Pollution Control Board, or any other body named in it. Wording is original; no text has been reproduced from the standards, rules or guidelines referenced.
