# -*- coding: utf-8 -*-
"""Builds the COP.1 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): COP.1.d, COP.1.e
  Tier 2 (lighter pass):   COP.1.a (Core), COP.1.b, COP.1.c (Achievement)

COP.1.a is Core (assessed at every visit) but not asterisked. Core is not a
substitute for the asterisk when allocating depth.

Official source: NABH Standards for Small Healthcare Organisations, 3rd Edition
(August 2022), Chapter 2 Care of Patients, standard COP.1 and OEs COP.1.a-e,
read from the official standards PDF (downloaded 2026-08-17 from the NABH
website's Explore NABH Standards page), printed page 61, PDF page index 67.
...
committed scripts/shco_oe_asterisks.json on all 408 entries) and the COP.1 page
read directly. COP.1.a, COP.1.b and COP.1.c are unasterisked; COP.1.d and
COP.1.e carry the asterisk.
"""
...
TIER1_OES = ["COP.1.d", "COP.1.e"]

POLICY_TITLE = "Uniform Care Across Settings and Telemedicine"
...
PURPOSE = """This document sets out how {{HOSPITAL_NAME}} gives the same standard of care for a given clinical condition wherever that care is provided, and how a telemedicine facility, if one is provided, is run from written guidance rather than from informal remote advice.
...
POLICY_STATEMENT = """{{HOSPITAL_NAME}} identifies every patient, before an act of care, by a uniform process that uses at least two identifiers. A bed number, a room number or a tray position is not an identifier.
...
{{HOSPITAL_NAME}} provides a telemedicine facility, if it provides one, safely and securely based on written guidance consistent with the Telemedicine Practice Guidelines, 2020. If it does not provide a telemedicine facility, that decision is written, and informal remote advice is not treated as one."""

PROCEDURE_STEPS = [
"""1. Two identifiers at the point of care
...
This is Core: it will be assessed at every visit. It is not asterisked. The depth of this step is the requirement and the method.""",

"""2. Care in consonance with applicable laws and regulations
...
When a service is added, suspended or withdrawn under the definition-and-display policy, the applicable-law list is reviewed for that service.""",

"""3. Evidence-based clinical practice guidelines and clinical protocols
...
This step adopts the protocols. Step 4 requires that, once adopted, they are applied the same way in every setting where that care is given.""",

"""4. Uniform care for a given condition across settings
...
How staff in each setting are shown the current version, and how a contradiction between settings is reported, are [Hospital to define — how staff in each setting are shown the current protocol version, and how a contradiction between settings is reported].""",

"""5. Telemedicine facility based on written guidance
...
Registration of a telemedicine patient, and generation of the unique identification number, remain AAC.2. This step requires that a telemedicine episode is a registered episode. It does not invent a second identity system.""",
...
All staff are expected to treat a misidentification, a second protocol for the same condition in another setting, and unofficial remote advice as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Care of Patients chapter, standard COP.1.
...
STATUTE_CLAUSE = (
    "the Telemedicine Practice Guidelines, 2020 issued by the Ministry of Health and Family "
    "Welfare and adopted by the National Medical Commission, insofar as a telemedicine "
...
OE_MAPPING = [
    {
        ...
        "steps": "Steps 1, 6",
        "evidence": "The written two-identifier process naming the pair in use; sample point-of-care records showing both identifiers confirmed before medication, a procedure or specimen collection rather than a bed or room number; records distinguishable from AAC.2 generation of the unique identification number; the audit sample at step 6 of identifiers used at the point of care",
        "responsible": "Clinical and nursing staff confirm two identifiers before an act of care; registration and admission policy owns generation of the unique identification number; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "COP.1.b",
        "requirement": "Care shall be provided in consonance with applicable laws and regulations",
        "steps": "Steps 2, 6",
        "evidence": "The written list of laws and regulations this hospital treats as applicable to the care it provides, aligned to the current service directory rather than inherited from another chapter; the record of review when a service is added, suspended or withdrawn; the location where the list is held; the audit sample at step 6 of the list against the directory",
        "responsible": "Administration maintains the applicable-law list against the service directory; head of the institution is accountable that care stays inside those laws; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "COP.1.c",
        ...
        "steps": "Steps 3, 4, 6",
        "evidence": "The written list of evidence-based clinical protocols in current use, the conditions they cover, dated and version-controlled; records showing a withdrawn protocol removed from points of use; distinction from nursing clinical practice guidelines owned by COP.4; the location where the protocols are held; the audit sample at step 6 of adopted protocols being the version in use",
        "responsible": "Clinicians and heads of department adopt and keep current the protocol list; nursing-care policy owns nursing CPGs; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "COP.1.d",
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 2, printed page 61
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="cop1_draft.json",
        sql_name="cop1_insert.sql",
    )
