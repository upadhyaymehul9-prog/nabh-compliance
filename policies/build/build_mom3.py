# -*- coding: utf-8 -*-
"""Builds the MOM.3 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): MOM.3.a, MOM.3.b, MOM.3.e
  Tier 2 (lighter pass):   MOM.3.c, MOM.3.d, MOM.3.f, MOM.3.g, MOM.3.h

THREE of eight OEs are asterisked. The draft builds deep blocks for a, b and e.
c, d, f, g and h are lean.

Official source: NABH Standards for Small Healthcare Organisations, 3rd Edition
(August 2022), Chapter 3 Management of Medication, standard MOM.3 and OEs
MOM.3.a-h, read from the official standards PDF (downloaded 2026-08-17 from the
NABH website's Explore NABH Standards page), printed page 77, PDF page index 83.

THIS is the real MOM.3 header. MOM.5's OE-page header wrongly copies this
wording; do not confuse the two standards.
...
committed scripts/shco_oe_asterisks.json on all 408 entries) and the MOM.3 page
read directly. MOM.3.a, MOM.3.b and MOM.3.e carry the asterisk; MOM.3.c, MOM.3.d,
MOM.3.f, MOM.3.g and MOM.3.h are unasterisked.
"""
...
TIER1_OES = ["MOM.3.a", "MOM.3.b", "MOM.3.e"]

POLICY_TITLE = "Safe and Rational Prescription of Medications"
...
Blood and blood components are not prescribed here as ward-stock medicines; the clinical transfusion method is the transfusion policy. Pain titration as a clinical loop is the pain policy; prescribing the analgesic is this document."""

SCOPE = """This policy applies to every clinician at {{HOSPITAL_NAME}} who prescribes a medicine, every location in which a prescription or medication order is written or given verbally, and the staff who record allergies, who receive verbal orders, who audit prescriptions, and who reconcile medicines at a transition. It binds employed practitioners and any visiting practitioner who prescribes under this hospital's unique identification number.
...
- Counselling and education about medicines are governed by PRE (not yet drafted). The medical record itself is governed by IMS (not yet drafted)."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} prescribes medications in consonance with good practices for rational prescription. A medicine added because it is familiar, or a combination that has no recorded indication, is not that practice.
...
{{HOSPITAL_NAME}} reconciles medications at transition points of patient care."""

PROCEDURE_STEPS = [
"""1. Rational prescription in consonance with good practices
...
The written rational-prescription guidance is held at [Hospital to define — where the written rational-prescription guidance is held].""",

"""2. Determined minimum requirements of a prescription
...
Counselling the patient about an allergy remains PRE when drafted. This step owns the ascertain-before-prescribe act.""",

"""4. Mechanism to assist the clinician in prescribing an appropriate medication
...
Assistance is not a substitute for the prescriber's decision at step 1, and it is not the MOM.4 order-writing layout.""",

"""5. Verbal orders implemented so that medication management remains safe
...
The written verbal-order method is held at [Hospital to define — where the written verbal-order method is held], including in the areas that would use it. A method that lives only in the quality office is not implementation.""",

"""6. Audit of medication orders and prescriptions
...
The audit record is dated and retrievable. An audit that is planned and not done is not an audit.""",

"""7. Corrective and preventive action based on the audit
...
The multidisciplinary medication committee under the pharmacy-committee policy is an available forum; this hospital may name another. This step owns that the prescription audit leads to action, not only to a percentage.""",

"""8. Reconciliation of medications at transition points of patient care
...
RESPONSIBILITY = """The head of the institution is accountable for {{HOSPITAL_NAME}} prescribing rationally, for a determined minimum that is actually used, for verbal orders that become written orders, and for reconciliation at named transitions.
...
All staff are expected to treat an incomplete prescription that was guessed, a verbal order that was never written, a known allergy that was not looked at, and a transition that copied the last list without comparing it, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Management of Medication chapter, standard MOM.3.
...
STATUTE_CLAUSE = (
    "the Drugs and Cosmetics Act, 1940 and the Drugs and Cosmetics Rules, insofar as "
    "Schedule H and Schedule H1 medicines are prescribed, and the National Medical "
    "Commission Act, 2019, insofar as it governs who may prescribe"
)
...
OE_MAPPING = [
    {
        "oe_code": "MOM.3.a",
        "requirement": "Medication prescription is in consonance with good practices and guidelines for rational prescription of medications.",
        "steps": "Steps 1, 6, 9",
        "evidence": "The written good-practice method this hospital uses for rational prescription, held at the place of prescribing rather than as a WHO poster in a seminar room, and showing that rational is not equated with formulary adherence alone (MOM.1.f owns formulary adherence; a listed medicine can still be the wrong medicine); the named guidelines actually used, with WHO promoting rational use, NLEM 2018, WHO EML and WHO drug-use indicators (chapter references 9, 19, 21, 23) recorded as frameworks that may inform and not as pasted protocols, antibiotic tables or mandated durations; sample prescriptions against the unique identification number showing a recorded indication or a recorded clinical reason rather than a combination no one can explain; the named roles who may prescribe, with current registration under the National Medical Commission Act, 2019 and State Medical Council used from human-resource verification, and Schedule H / H1 medicines prescribed under the Drugs and Cosmetics Act, 1940 rather than sold as general items; records of prescription from the current formulary or through the MOM.1 non-formulary procedure; records distinguishing this prescription of an analgesic from COP.13 titration, this prescription of a sedative or anaesthetic from COP.9 / COP.10 clinical acts, and this rational-use look from MOM.8's 'appropriate caregiver' rule for narcotic and psychotropic agents; the location of the written guidance; induction or briefing records of prescribing clinicians; the audit sample at steps 6 and 9 of rational prescription against this hospital's method rather than familiarity",
        "responsible": "Prescribing clinicians apply the good-practice method; named person holds the guidance; human resource function verifies registration; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "MOM.3.b",
        "requirement": "The organisation adheres to the determined minimum requirements of a prescription.",
        "steps": "Steps 2, 6, 9",
        "evidence": "The written determined minimum requirements of a prescription as a clinical document, held at the place of prescribing, including the hospital's chosen elements (which may include indication, known allergies, patient identity and other determined items as well as name, route, strength and frequency) and showing the recorded split that MOM.4 owns how the order appears in the record (authorised writers, uniform location, unique identification number on that location, legibility, date, time, signature, and those four fields as order content) so that this OE is not written as a duplicate that makes MOM.4 redundant; the recorded statement that NCCMERP prescription-writing recommendations (chapter references 25 and 26) and ISMP error-prone abbreviations (chapter reference 13) are frameworks the hospital may use, not a numeric rule and not a mandated banned-abbreviation table; the written method for returning an incomplete prescription to the prescriber rather than guessing it into a dose, and who may not complete it by inference; sample out-patient prescriptions and in-patient medication orders showing the minimum met for each form, or returned rather than inferred; induction or briefing records of prescribers and of staff who would otherwise complete by inference; the audit sample at steps 6 and 9 of prescriptions that meet the determined minimum rather than guessed completions, with MOM.4 layout defects handed to MOM.4",
        "responsible": "Prescribing clinicians meet the minimum; staff who receive a prescription return an incomplete one rather than infer; named person holds the written minimum; MOM.4 owns order appearance in the record when drafted; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "MOM.3.c",
        "requirement": "Drug allergies and previous adverse drug reactions are ascertained before prescribing.",
        "steps": "Steps 3, 9",
        "evidence": "The written method for ascertaining allergy and previous adverse reaction, and where the finding is recorded so the prescriber sees it at the moment of prescribing; sample records showing a look before the medicine (including recorded none or unknown) rather than only an AAC.3 assessment checkbox; records of a known allergy stopping the prescription unless a documented clinical reason was recorded; the audit sample at step 9",
        "responsible": "Prescribing clinicians ascertain before choosing the medicine; AAC.3 may collect the fact; PRE owns counselling when drafted; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "MOM.3.d",
        "requirement": "The organisation has a mechanism to assist the clinician in prescribing an appropriate medication.",
        "steps": "Steps 4, 1, 9",
        "evidence": "The written mechanism (formulary at the point of prescribing, standard treatment guidelines, pharmacy enquiry route, electronic prompt if used, or other local method) and records that clinicians can use it at the time they prescribe; the recorded statement that no named software is mandated; the audit sample at step 9",
        "responsible": "Named person holds the mechanism; prescribing clinicians use it; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "MOM.3.e",
        "requirement": "Implementation of verbal orders ensures safe medication management practices.",
        "steps": "Steps 5, 2, 3, 9",
        "evidence": "The written verbal-order method stating when a verbal order is permitted (defined situations such as an emergency in which the prescriber cannot at that moment write, not whenever it is quicker), who may give it (a person who may prescribe at step 1), who may receive it, and how it is recorded (medicine, the dose elements the determined minimum requires, unique identification number, giver, receiver, time) without a mandated number of minutes for later signature presented as a NABH requirement; the written method for how the verbal order becomes a written or signed order and who is responsible for that, showing that an unwritten shout is not an order under this document; the recorded expectation of read-back unless a defined alternative that still catches a mis-heard name or dose is written, with NCCMERP verbal-order recommendations (chapter reference 27) used as a framework not as a numeric sign-off rule; sample verbal-order records against the unique identification number showing permission, read-back, recording and later writing rather than a corridor instruction that never became a record; records of allergy ascertaining at step 3 insofar as the emergency allowed, or a recorded reason why it could not and completion afterwards; the recorded division that a verbal NDPS order, if ever permitted, still uses MOM.8's register rather than a spoken exception to that cupboard, and that look-alike names take care informed by the MOM.2 high-risk list without rewriting storage; the location of the method in the areas that would use it rather than only in the quality office; induction or briefing records of staff who give or receive verbal orders; the audit sample at step 9 of verbal orders that were permitted, read back and then written",
        "responsible": "Prescribers give verbal orders only when permitted; receivers record, read back and do not administer an unwritten shout; named person holds the method; MOM.8 owns any NDPS register; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "MOM.3.f",
        "requirement": "Audit of medication orders and prescriptions is carried out to check for safe and rational prescription.",
        "steps": "Steps 6, 9",
        "evidence": "The written audit method including what is looked at, the sample, the interval and who performs it; sample audit records covering rational prescription, completeness, allergy, verbal orders and formulary adherence; the audit sample at step 9 of audits that were done rather than only planned",
        "responsible": "Named auditor performs the audit; quality or accreditation coordinator reviews that it ran",
    },
    {
        "oe_code": "MOM.3.g",
        "requirement": "Corrective and/or preventive action is taken based on the audit, where appropriate.",
        "steps": "Steps 7, 6, 9",
        "evidence": "The written method for assigning, dating and closing actions and the named forum; sample actions closed; records of an audit with no defect stating that finding rather than inventing an action; the audit sample at step 9",
        "responsible": "Named forum assigns actions; the person assigned an action closes it; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "MOM.3.h",
        "requirement": "Reconciliation of medications occurs at transition points of patient care.",
        "steps": "Steps 8, 9",
        "evidence": "The named transition points and the written method; who performs reconciliation at each; sample records at admission, internal transfer and discharge showing medicines continued, stopped or changed rather than a copied list; the recorded split that AAC.7 owns the move and AAC.8 owns the discharge summary; the recorded statement that WHO High 5s, WHO transition-of-care and AHRQ reconciliation (chapter references 16, 17, 29) are frameworks not a mandated tool; the audit sample at step 9",
        "responsible": "Named persons reconcile at each transition; AAC.7 owns the move; AAC.8 owns the summary; quality or accreditation coordinator audits",
    },
]
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 3, printed page 77
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="mom3_draft.json",
        sql_name="mom3_insert.sql",
    )
