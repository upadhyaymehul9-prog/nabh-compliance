# -*- coding: utf-8 -*-
"""Builds the FMS.1 master policy draft: JSON for review + SQL for later insert.
...
FMS chapter calibration (owner 2026-08-18): two-tier still applies, but
non-asterisked OEs that are genuine technical processes — not administrative
acts — carry real substance (named codes, failure modes, test method).
Asterisked OEs still get full Tier 1. Density is HIC.4/HIC.6-grade technical
content, not filler. Evidence-column length still requires Tier 1 longer than
Tier 2.

Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): FMS.1.f
  Tier 2 (lighter pass):   FMS.1.a, FMS.1.b, FMS.1.c, FMS.1.d, FMS.1.e
  FMS.1.d and FMS.1.e are Core/Commitment and unasterisked, but they are
  technical (potable water/electricity availability; backup sources and
  loaded testing) and are drafted at technical T2 density.

Official source: NABH Standards for Small Healthcare Organisations, 3rd
Edition (August 2022), Chapter 8 Facility Management and Safety, standard
FMS.1 and OEs FMS.1.a-f, printed page 116, PDF page index 122. Chapter
intent and summary: printed page 115, PDF page index 121. References:
printed pages 119-120, PDF indices 125-126. Chapter 8 occupies printed
pages 115-120 (PDF indices 121-126). HRM starts printed page 121.

Asterisks verified 2026-08-18 against that page and
scripts/shco_oe_asterisks.json. Only FMS.1.f carries the asterisk.
...
CHAPTER = "FMS"
OE_CODES = [
    "FMS.1.a", "FMS.1.b", "FMS.1.c", "FMS.1.d", "FMS.1.e", "FMS.1.f",
]
TIER1_OES = ["FMS.1.f"]
...
This document is planned operation of the facility and its utilities, and the energy/environment initiatives. It is not FMS.2 safety devices, monthly rounds, electrical audits, unused-material disposal or hazardous materials. It is not FMS.3 medical-equipment PPM. It is not FMS.4 medical gases. It is not FMS.5 fire and non-fire emergency plans. It is not the billing ledger."""

SCOPE = """This policy applies to the physical facilities of {{HOSPITAL_NAME}}, to the people who plan space, hold drawings, post wayfinding, operate potable-water and electrical systems and their backups, and to those who run energy-efficiency and environment-friendly initiatives.
...
- PSQ.2 owns managerial indicators as quality tools. A utility-failure rate may be supplied from these records; PSQ.2 owns the indicator set."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} requires that facilities and space provisions are appropriate to the scope of services.
...
{{HOSPITAL_NAME}} does not treat a DG set that is cranked but never takes essential-circuit load, a drawing that does not match the built floor, or a green-hospital poster with no measured initiative, as that duty."""

PROCEDURE_STEPS = [
"""1. Facilities and space appropriate to the scope of services
...
How appropriateness is judged (against AAC.1 and, as a framework, IPHS 2022 chapter reference 18 and NBC 2016 occupancy as the local authority applied it), who records a mismatch, and what happens when a service is added or withdrawn, are [Hospital to define — how facilities and space are shown to be appropriate to the defined scope of services]. A service the directory does not provide is a recorded absence.""",

"""2. As-built and updated drawings
...
Which drawings are held (architectural, structural, electrical single-line, plumbing/water, fire/detection as FMS.5 will use them, medical-gas as FMS.4 will use them if piped), where the controlled set lives, who updates it after a change, and which statutory or occupancy condition requires which sheet, are [Hospital to define — the as-built and updated drawings maintained as statutory and occupancy requirements require]. FMS.5 uses fire drawings; this step holds the controlled set. FMS.4 uses MGPS drawings if a piped system exists.""",

"""3. Internal and external sign postings
...
Languages, scripts, and how a visitor who cannot read the local majority language is still directed, plus who inspects faded or contradictory signs, are [Hospital to define — internal and external sign postings in a manner understood by patients, families and the community].""",

"""4. Round-the-clock potable water and electricity
...
Who watches availability (including night and holiday), how an interruption is recorded, and the restoration target this hospital sets, are [Hospital to define — how round-the-clock potable water and electricity availability is watched, recorded and restored]. This step does not invent a NABH minutes-to-restore clock.""",

"""5. Alternate electricity and water, and testing that they actually work
...
A utility failure that becomes a declared emergency is handed to FMS.5; this step is the test before that night.""",

"""6. Energy-efficient and environment-friendly initiatives
...
Which initiatives are in force this year, the named owner, the baseline and the review of result, and what is out of scope as HIC.3 BMW or as a certification this hospital does not hold, are [Hospital to define — the energy-efficient and environment-friendly initiatives, with baseline and result, and the named owner]. An unused rooftop solar array, or an energy audit that produced no action, is not this OE.""",
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a backup test failed under load, or when AAC.1, AAC.6, FMS.2, FMS.4, FMS.5 or HIC.3 that this document hands work to are revised.""",
]

RESPONSIBILITY = """The head of the institution is accountable that facilities operate as this document requires.
...
All staff are expected to treat a dark essential circuit, a dry clinical tap, a drawing that does not match the floor, and a green poster with no initiative, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Chapter 8 FMS, standard FMS.1.
...
- Internal documents of {{HOSPITAL_NAME}}: AAC.1 service directory; as-built drawing set; wayfinding; potable-water and electrical records; backup-test records; energy-initiative file; FMS.2, FMS.3, FMS.4, FMS.5; HIC.3; AAC.6."""
...
OE_MAPPING = [
    {
        ...
        "steps": "Steps 1, 7",
        "evidence": "The recorded match of built space to the AAC.1 directory rather than a copied floor plate; recorded absences for unused services; IPHS 2022 and NBC 2016 used as frameworks not bed-count mandates; the audit sample at step 7",
        "responsible": "Named facilities lead holds the space record; AAC.1 remains the directory; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.1.b",
        "requirement": "As-built and updated drawings are maintained as per statutory requirements.",
        "steps": "Steps 2, 7",
        "evidence": "The controlled as-built set the occupancy actually required, with an update log after a building or major-service change, rather than a brochure plan; NBC 2016 as the local-authority framework; FMS.4/FMS.5 use of those sheets recorded as users not second masters; the audit sample at step 7",
        "responsible": "Named facilities lead holds the controlled set; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.1.c",
        ...
        "steps": "Steps 3, 7",
        "evidence": "Wayfinding in the languages this hospital has defined, inspected for faded or contradictory signs; the recorded split that AAC.6.e owns radiation/PC-PNDT signs and FMS.5.b owns emergency exit display; the audit sample at step 7",
        "responsible": "Named facilities lead holds wayfinding; AAC.6.e and FMS.5.b remain those documents; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.1.d",
        "requirement": "Potable water and electricity are available round the clock.",
        "steps": "Steps 4, 7",
        "evidence": "Availability records covering night and holiday, not only a morning check; essential-circuit list actually energised; WHO GDWQ 4th edition quality-testing record as framework not a NABH interval; dialysis-water method or recorded absence vs AAC.1; the audit sample at step 7 of a tap and a circuit that worked after hours",
        "responsible": "Named engineering lead watches availability; HIC.3 kitchen water use remains HIC.3; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.1.e",
        ...
        "steps": "Steps 5, 4, 7",
        "evidence": "Named alternate electricity and water sources; loaded essential-circuit test records at the predefined frequency rather than a no-load crank; tank-to-tap or pump auto-start records rather than a sealed tanker MoU; IS 732/IS 3043 named as NBC-pointed frameworks not extra P2 statutes; the audit sample at step 7",
        "responsible": "Named engineering lead runs the tests; FMS.5 receives a declared emergency; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.1.f",
        "requirement": "The organisation takes initiatives towards an energy-efficient and environment friendly hospital.",
        "steps": "Steps 6, 7",
        "evidence": "The written energy-efficient and environment-friendly initiative file showing named owner, baseline and result for initiatives this hospital actually runs this year rather than a green-hospital poster, a single LED retrofit offered as the whole OE, an unused rooftop array, or an energy audit that produced no action; the recorded use of Dhillon 2015 (chapter reference 7) as a green-hospital framework not a named rating mandate and not authority to cut clinical ventilation below what the services require; the recorded split that FMS.1.d/e remain availability and backup testing and that this OE is measured initiative, and that HIC.3 remains BMW colour, transport and SPCB so environment-friendly hospital is not a second biomedical-waste SOP; induction or briefing of the named owner; the location of the file; the audit sample at step 7 of a baseline that changed a result rather than a photograph",
        "responsible": "Head of the institution is accountable that initiatives exist; named owner holds the file with baseline and result; HIC.3 remains BMW; quality or accreditation coordinator audits",
    },
]
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 8 FMS, printed page 116
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="fms1_draft.json",
        sql_name="fms1_insert.sql",
    )
