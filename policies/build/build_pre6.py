# -*- coding: utf-8 -*-
"""Builds the PRE.6 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): PRE.6.c
  Tier 2 (lighter pass):   PRE.6.a, PRE.6.b, PRE.6.d

ONE of four OEs is asterisked. The draft builds a deep block for c.
a, b and d are lean.

Official source: NABH Standards for Small Healthcare Organisations, 3rd Edition
(August 2022), Chapter 4 Patient Rights and Education, standard PRE.6 and OEs
PRE.6.a-d, printed page 89, PDF page index 95. Chapter intent printed page 85,
PDF page index 91: patients are educated about the mechanisms available for
addressing grievances. Summary header confirmed from the PDF (do not assume):
"The organisation has a mechanism to capture patient's feedback and to redress
complaints."

Asterisks verified 2026-08-17 against that PDF and scripts/shco_oe_asterisks.json.
PRE.6.c carries the asterisk; PRE.6.a, PRE.6.b and PRE.6.d are unasterisked.

The chapter bibliography's source for complaints is Reader, Gillespie and
Roberts 2014 (chapter reference 17). Consumer Protection Act, 2019 is used as
a framework for a consumer grievance mechanism, not as copied District
...
CHAPTER = "PRE"
OE_CODES = [
    "PRE.6.a", "PRE.6.b", "PRE.6.c", "PRE.6.d",
]
TIER1_OES = ["PRE.6.c"]
...
This document is feedback and complaint redressal. A complaint that is also a rights violation is received here and is also a PRE.1.d report; PRE.1.e owns leadership review of violations. This document does not replace that review."""

SCOPE = """This policy applies to every point at which a patient or family at {{HOSPITAL_NAME}} can give feedback or lodge a complaint: out-patient, emergency, in-patient, day-care, and after discharge insofar as this hospital accepts post-discharge feedback. It binds the staff who collect feedback, who receive complaints, who redress them, who make patients aware of the procedure, and who review and analyse within the defined time frame.
...
- This document does not copy District Commission, State Commission or National Commission procedure under the Consumer Protection Act, 2019. A patient who chooses a statutory consumer remedy is not blocked by this hospital mechanism, and this hospital mechanism is not that Commission."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} has a mechanism to capture feedback from patients, which includes patient satisfaction.
...
This document does not print a named ombudsman title, a rupee ex-gratia table, or a statutory Commission form as a NABH mandate.""",

"""4. Review and analysis within a defined time frame, and CAPA
...
This document does not print a number of days as a NABH mandate.""",
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a complaint could be lodged only with the person complained of, or a box was never opened, or when PRE.1 or PRE.2 that this document hands work to are revised.""",
]

RESPONSIBILITY = """The head of the institution is accountable for {{HOSPITAL_NAME}} capturing feedback and experience, redressing complaints as per the defined mechanism, making patients and families aware of that procedure, and analysing within the defined time frame.
...
All staff are expected to treat an unusable complaint route and an unopened feedback store as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Patient Rights and Education chapter, standard PRE.6.
...
- Internal documents of {{HOSPITAL_NAME}}: the feedback mechanism; the experience mechanism; the complaint-redressal mechanism; the awareness method; analysis and CAPA records; the rights and violation-report policies; the expected-costs policy; and the information-management policies."""
...
STATUTE_CLAUSE = (
    ...
    "may seek a grievance remedy, and not as copied District Commission procedure "
    "or as a bar to a statutory consumer complaint"
)
...
OE_MAPPING = [
    {
        ...
        "steps": "Steps 1, 5",
        "evidence": "The written feedback mechanism including how satisfaction is captured; sample feedback records; the recorded split that a dissatisfied comment asking for redressal is also a complaint at step 3; the audit sample at step 5",
        "responsible": "Named lead holds the feedback mechanism; staff who collect feedback apply it; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "PRE.6.b",
        "requirement": "The organisation has a mechanism to capture the patient experience.",
        "steps": "Steps 2, 5",
        "evidence": "The written experience-capture method as distinct from a satisfaction score; sample experience records rather than a named survey printed as a NABH mandate; the audit sample at step 5",
        "responsible": "Named lead holds the experience mechanism; staff who capture experience apply it; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "PRE.6.c",
    ...
    },
    {
        ...
        "steps": "Steps 4, 3, 5",
        "evidence": "The written defined time frame, who reviews, and how CAPA is recorded where appropriate; sample reviews dated within that frame with action where appropriate; the recorded split that PRE.1.e analyses rights violations as violations; the audit sample at step 5",
        "responsible": "Named reviewer analyses within the time frame; PRE.1.e owns violation CAPA; quality or accreditation coordinator audits",
    },
]
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="pre6_draft.json",
        sql_name="pre6_insert.sql",
    )
