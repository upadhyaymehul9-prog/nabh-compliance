# -*- coding: utf-8 -*-
"""Builds the FMS.3 master policy draft: JSON for review + SQL for later insert.
...
FMS chapter calibration (owner 2026-08-18): technical T2 (calibration,
downtime) carries BEMMP/CDSCO substance. Asterisked OEs still get full Tier 1.

Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): FMS.3.c, FMS.3.f
  Tier 2 (lighter pass):   FMS.3.a, FMS.3.b, FMS.3.d, FMS.3.e, FMS.3.g
  d (calibration) is technical T2.

Official source: NABH SHCO 3rd Edition PDF, Chapter 8, FMS.3 printed page 117
(PDF index 123). Header on the OE page: "The organisation has a programme for
medical and support service equipment management." Summary on printed page 115
uses "medical equipment and support services management."

Asterisks verified 2026-08-18: FMS.3.c and FMS.3.f carry the asterisk.

PDF md5 39e3bc86d73d651b9cfef283bbf018a9.

NHM BEMMP (chapter reference 10) is the biomedical-equipment framework — not
the Clinical Establishments Act and not Central Electricity Authority.
CDSCO Medical Devices page is chapter reference 8; Medical Devices Rules,
2017 are the instrument that page implements and are named because FMS.3.f
engages adverse events, hazard notices and recalls.
"""
...
This document is the hospital-wide equipment programme. It is not AAC.4.h's rule that a laboratory result is not issued from an overdue calibrator. It is not AAC.5.i's imaging AERB QA and no-report-if-overdue. It is not HIC.6 steriliser validation as a reprocessing act. It is not COP.3 crash-cart contents. It is not FMS.1 plant utilities (DG, pumps as building plant). It is not the billing ledger."""

SCOPE = """This policy applies to medical equipment and support-service equipment of {{HOSPITAL_NAME}}, whether owned, leased, loaned, consigned or outsourced, and to the people who plan, inventory, operate, maintain, inspect, calibrate, recall and record downtime for that equipment.
...
- CDSCO Medical Devices and Diagnostics (chapter reference 8) and the Medical Devices Rules, 2017, read with the Drugs and Cosmetics Act, 1940, govern regulated devices, adverse-event reporting and recalls insofar as they apply to devices this hospital uses. They do not make this hospital a manufacturer."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} plans medical and support-service equipment in accordance with its services and strategic plan.
...
{{HOSPITAL_NAME}} does not treat a calibration sticker without a record, a recall letter that never reached the user, or AAC.4.h/AAC.5.i specialty QA offered as this whole programme, as that duty."""

PROCEDURE_STEPS = [
"""1. Planning equipment against services and the strategic plan
...
What is on the inventory, what a log contains, and how a loaner or outsourced analyser is still listed, are [Hospital to define — the medical and support-service equipment inventory and the logs maintained]. An item in use that is not on the inventory is a defect.""",

"""3. Operational and maintenance (preventive and breakdown) plan implemented
...
Which items require calibration versus inspection-only, the interval, the traceable standard or vendor, and the rule that an overdue measuring device is withdrawn until passed, are [Hospital to define — periodic inspection and calibration: which items, interval, traceable method, and withdrawal-until-passed]. BEMMP criticality informs interval; it is not a NABH universal calendar. This step does not invent a SHCO-wide six-month calibration mandate.""",

"""5. Qualified and trained personnel operate and maintain
...
Which roles may operate which class, which roles may maintain which class, and how training is recorded, are [Hospital to define — who is qualified and trained to operate and to maintain each class of medical and support-service equipment].""",

"""6. Adverse events, hazard notices and recalls
...
Which items are critical, how reporting-to-restoration is timed, and how a corrective action is recorded, are [Hospital to define — critical-equipment list and downtime monitoring from report to inspection and corrective action]. PSQ.2 may use the rate; this document owns the clock. A diverted service remains AAC.1/AAC.2; this clock still runs.""",
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a recall was missed, or when AAC.4, AAC.5, AAC.6, HIC.6, FMS.1, FMS.2, FMS.4, COP.3 or MOM.9 that this document hands work to are revised.""",
]

RESPONSIBILITY = """The head of the institution is accountable that the medical and support-service equipment programme runs as this document requires.
...
All staff are expected to treat an uninventoried device in use, an overdue calibrator still in service, and a recall that did not reach the ward, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Chapter 8 FMS, standard FMS.3.
...
- Internal documents of {{HOSPITAL_NAME}}: equipment plan; inventory; PPM and breakdown plan; calibration; recall file; downtime log; AAC.1, AAC.4, AAC.5; HIC.6; COP.3; FMS.1, FMS.2, FMS.4; MOM.9; ROM.3; PSQ.5."""

DISTRIBUTION = """Controlled master copy: office of the head of the institution, {{HOSPITAL_NAME}}, with the named biomedical or engineering lead and the quality or accreditation coordinator.
...
OE_MAPPING = [
    {
        ...
        "steps": "Steps 1, 8",
        "evidence": "The equipment plan matched to AAC.1 and ROM.3.a; recorded absences for unused services; BEMMP/IPHS as frameworks not mandates; the audit sample at step 8",
        "responsible": "Named biomedical lead holds the plan; AAC.1 and ROM.3.a remain those documents; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.3.b",
        "requirement": "Medical equipment and support service equipment are inventoried, and proper logs are maintained as required.",
        "steps": "Steps 2, 8",
        "evidence": "The inventory including loaners/outsourced items and the logs (acceptance, PPM, breakdown, calibration due); split from COP.3 crash-cart checklists and AAC.4/AAC.5 specialty registers; the audit sample at step 8",
        "responsible": "Named biomedical lead holds the inventory; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.3.c",
        ...
        "steps": "Steps 3, 8",
        "evidence": "The written operational and maintenance (preventive and breakdown) plan showing criticality-based PPM tasks, who attends, how a user reports a breakdown, and sample job cards that show last month's PPM and last week's breakdown actually happened rather than a binder of manufacturer PDFs or a sticker with no job card; the recorded use of NHM BEMMP (chapter reference 10) as a criticality/PPM framework not a named NHM-contract mandate and not Clinical Establishments Act or Central Electricity Authority guidance; the recorded splits that HIC.6 remains steriliser process validation, AAC.4.h/AAC.5.i remain no-report-from-overdue-calibrator, and FMS.1.e remains DG/UPS plant tests; induction of users who report breakdowns; the location of the plan; the audit sample at step 8 of a critical item whose PPM was done when due",
        "responsible": "Named biomedical lead implements PPM and breakdown; HIC.6/AAC.4/AAC.5/FMS.1 remain those methods; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.3.d",
        "requirement": "Medical and support service equipment are periodically inspected and calibrated for their proper functioning.",
        "steps": "Steps 4, 8",
        "evidence": "Inspection and calibration records with withdrawal-until-passed for overdue measuring devices; AAC.4.h/AAC.5.i still owning no-report rules; BEMMP informing interval not a NABH universal calendar; the audit sample at step 8",
        "responsible": "Named biomedical lead; laboratory/imaging no-report rules remain AAC.4/AAC.5; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.3.e",
        "requirement": "Qualified and trained personnel operate and maintain medical and support service equipment.",
        "steps": "Steps 5, 8",
        "evidence": "Role-to-class training records for operators and maintainers; HRM flagged as future credentialing file; the audit sample at step 8",
        "responsible": "Named biomedical lead with department heads; HRM when drafted holds credentialing method; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "FMS.3.f",
    ...
    },
    {
        ...
        "steps": "Steps 7, 3, 8",
        "evidence": "Critical-equipment list and downtime logs from user report to inspection and corrective action rather than from engineer arrival; PSQ.2 may use the rate; the audit sample at step 8",
        "responsible": "Named biomedical lead times downtime; PSQ.2 remains the indicator set; quality or accreditation coordinator audits",
    },
]
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 8 FMS, printed page 117
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="fms3_draft.json",
        sql_name="fms3_insert.sql",
    )
