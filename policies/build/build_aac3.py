# -*- coding: utf-8 -*-
"""Builds the AAC.3 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): AAC.3.a
  Tier 2 (lighter pass):   AAC.3.b, AAC.3.c, AAC.3.d, AAC.3.e

The whole standard is mostly Tier 2 except AAC.3.a. AAC.3.c is Core but not
asterisked; Core is not a substitute for the asterisk when allocating depth.

Official source: NABH Standards for Small Healthcare Organisations, 3rd Edition
(August 2022), Chapter 1, standard AAC.3 and OEs AAC.3.a-e, read from the official
standards PDF (downloaded 2026-08-17 from the NABH website's Explore NABH Standards
page), printed page 51, PDF page index 57.
...
committed scripts/shco_oe_asterisks.json on all 408 entries) and the AAC.3 page
read directly. Only AAC.3.a carries the asterisk.
"""
...
POLICY_TITLE = "Initial Assessment, Reassessment and Early Warning"
...
PURPOSE = """This document sets out how {{HOSPITAL_NAME}} performs the initial assessment of every patient it has accepted, writes the care plan that assessment produces, reassesses the patient at defined intervals, tells the out-patient the next follow-up, and recognises early warning signs of deterioration so that intervention starts without waiting for a later planned review.
...
- The temperature and vital signs record is a shared data point with the infection-surveillance policy of {{HOSPITAL_NAME}}, not a shared process. This policy owns the taking of those observations for detecting clinical deterioration and starting prompt intervention — the method, the trigger, who is called, and that the call is made now. The infection-surveillance policy reads the same record later, as one source of active case-finding, looking for new fever after a defined period from admission, and, where invasive ventilation is provided, uses recorded temperature among the criteria of the ventilator-associated event definition. It does not take the observations, does not operate the early-warning method, and does not escalate for the patient's immediate care. A fever that triggers escalation under this policy is a clinical event. Whether that same fever later counts as a surveillance case is decided by the surveillance case definitions, by a different person, for a different purpose. An escalated deterioration is not an infection report; a surveillance case is not an early-warning call. Neither process substitutes for the other."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} performs an initial assessment of every out-patient, day-care patient, in-patient and emergency patient it has accepted. Each setting has its own written minimum dataset. A form written for one setting is not used as the assessment in another.
...
{{HOSPITAL_NAME}} lays down guidelines and operates a process to identify early warning signs of change or deterioration in clinical condition and to start prompt intervention. The method is written; it is not left to whoever happens to be on the ward."""

PROCEDURE_STEPS = [
"""1. Initial assessment of out-patients and day-care patients
...
The out-patient and day-care assessment forms or electronic templates, and the named roles that complete them, are held at [Hospital to define — where the out-patient and day-care assessment forms are held].""",

"""2. Initial assessment of in-patients and emergency patients
...
The in-patient and emergency assessment forms or electronic templates, and the named roles that complete them, are held at [Hospital to define — where the in-patient and emergency assessment forms are held].""",

"""3. Documented care plan
...
Where the assessment at steps 1 or 2 changes — a day-care patient admitted, an emergency patient moved to a ward — the care plan is written or revised for the new setting rather than left as the previous setting's next action.""",
...
Discharge planning that follows from reassessment is handed to the discharge policy of {{HOSPITAL_NAME}} when that document is in force; this step owns the clinical decision that discharge is now the plan.""",

"""5. Out-patient follow-up
...
This step is the appointment and the record of it. Education about the condition, and consent for a later procedure, remain under the patient-rights policies of {{HOSPITAL_NAME}}.""",

"""6. Early warning signs of change or deterioration
...
Temperature and the other vital signs used here are observations for clinical deterioration. The infection-surveillance policy of {{HOSPITAL_NAME}} later reads the same record as a case-finding source — new fever after a defined period from admission — and, where invasive ventilation is provided, as a criterion in the ventilator-associated event definition. That reading is not this process: it does not take the observations, does not make the call, and does not start the intervention. The division is stated in Scope.

A trigger without a recorded intervention is a process that did not run.""",
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a missed assessment, a deterioration that was not escalated, or a revision of the emergency-care or registration-and-admission policies that this document hands work to, exposes a gap."""
]

RESPONSIBILITY = """The head of the institution is accountable for {{HOSPITAL_NAME}} assessing every accepted patient in the setting where care starts, for a care plan existing in the record, and for a written method that identifies deterioration and starts intervention.
...
All staff are expected to treat a missing initial assessment, a care plan that was never written, and an early-warning trigger that was not escalated, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Access, Assessment and Continuity of Care chapter, standard AAC.3.
...
STATUTE_CLAUSE = (
    ...
    "home registration law where the 2010 Act is not in force — insofar as they require the "
    "organisation to maintain records of the assessment and care of patients"
)
...
OE_MAPPING = [
    ...
    },
    {
        ...
        "steps": "Steps 4, 7",
        "evidence": "The written patient categories and the reassessment interval for each; sample records showing reassessment notes at those intervals, stating response to treatment and the decision on further treatment or discharge; the audit sample at step 7 of reassessment completeness",
        "responsible": "Clinicians reassess and record the decision; nursing staff record the observations the reassessment uses; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "AAC.3.d",
        "requirement": "Out-patients are informed of their next follow-up, where appropriate",
        "steps": "Steps 5, 7",
        "evidence": "The written rule for which out-patients require a next follow-up; sample out-patient records showing the next follow-up told to the patient and recorded, or recorded as not required; the audit sample at step 7 of out-patient follow-up entries",
        "responsible": "Out-patient clinicians inform and record; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "AAC.3.e",
        ...
        "steps": "Steps 6, 7",
        "evidence": "The written guidelines naming the method used to identify early warning signs and the escalation route, including who is called; sample records of a trigger, the person who identified it, the intervention started and the time; briefing records of the staff who observe patients; the audit sample at step 7 of early-warning events that show an intervention",
        "responsible": "Nursing and observing staff apply the method and escalate; clinicians respond; emergency-care, critical-care and resuscitation policies own the intervention they already govern; quality or accreditation coordinator audits",
    },
]
...
2. Tier 1 cross-check (2026-08-17) of AAC.3.a against all six approved HIC masters and the approved AAC.1 master, plus the unapproved AAC.2 draft whose step 4 already hands this work over. Files: policies/drafts/hic1_draft.json through hic6_draft.json, aac1_draft.json and aac2_draft.json. Search terms: initial assessment, care plan, reassess, follow-up, early warning, deteriorat, vital signs.
   ...
   HIC.1-6: no subject-matter overlap on the initial clinical assessment of the accepted patient (AAC.3.a). HIC.6's "assessing exposed patients" is sterilisation-recall exposure assessment, not this OE. HIC.5 vs AAC.3.e is a shared DATA POINT, checked in full below even though e is Tier 2, because HIC.5 is approved and live. Nothing in AAC.3.a requires a reconciliation edit to an approved document.
...
4. AAC.3.e vs APPROVED HIC.5 -- FULL CHECK, not the standing-rule T2 one-liner, because HIC.5 is live. Files read: policies/build/build_hic5.py steps 4 and 8, Scope, and the HIC.5 evidence for HIC.5.a; this file's step 6.
   Shared data point: the temperature and vital signs record (the observation chart). HIC.5 step 4 lists "the temperature and vital signs record, and nursing handover, for new fever after a defined period from admission" as one of seven active case-finding SOURCES the Infection Control Nurse reads on a working day. HIC.5 step 8 also uses recorded temperature (with white cell count and antimicrobial start) as a criterion inside the NHSN ventilator-associated event framework, where invasive ventilation is provided. HIC.5 does not take the observations, does not define an early-warning method or score, does not name who is called when a patient deteriorates, and does not start clinical intervention. Its reader is the ICN; its question is whether a surveillance case definition is met; its output is a line list and a rate.
   AAC.3.e / step 6 owns the taking of those observations for detecting clinical deterioration and starting prompt intervention. Its reader is the staff at the bedside; its question is whether this patient needs a clinician now; its output is a call and an intervention in the record.
   Not a contradiction: two processes, one chart. An escalated deterioration is not an infection report; a surveillance case is not an early-warning call. HIC.5 is approved and is NOT reopened; the division is stated in this draft's Scope and restated at step 6, the same pattern AAC.2 used toward this document. No text change required in HIC.5. Not added to the reconciliation list.
   Remaining T2 quick checks (not a full audit): AAC.3.b care-plan content vs IMS record structure -- IMS owns structure, this owns content. AAC.3.c reassessment vs COP.4 nursing care -- interval observations may later sit with nursing; this document owns the clinical reassessment that decides treatment or discharge. AAC.3.d follow-up vs PRE education -- this owns the appointment; PRE owns education. AAC.3.e vs COP emergency/critical-care/CPR -- this owns recognition; those own the protocols.
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 1, printed page 51
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="aac3_draft.json",
        sql_name="aac3_insert.sql",
    )
