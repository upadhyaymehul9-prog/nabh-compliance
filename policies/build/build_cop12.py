# -*- coding: utf-8 -*-
"""Builds the COP.12 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): COP.12.a, COP.12.f
  Tier 2 (lighter pass):   COP.12.b, COP.12.c (Core), COP.12.d (Core),
                           COP.12.e (Core)

Official source: NABH Standards for Small Healthcare Organisations, 3rd Edition
(August 2022), Chapter 2, standard COP.12 and OEs COP.12.a-f, read from the
official standards PDF (downloaded 2026-08-17 from the NABH website's Explore
NABH Standards page), printed page 68, PDF page index 74.
...
committed scripts/shco_oe_asterisks.json on all 408 entries) and the COP.12
page read directly. COP.12.a and COP.12.f carry the asterisk; COP.12.b, c, d
and e are unasterisked.
"""
...
CHAPTER = "COP"
OE_CODES = [
    "COP.12.a", "COP.12.b", "COP.12.c",
    "COP.12.d", "COP.12.e", "COP.12.f",
]
TIER1_OES = ["COP.12.a", "COP.12.f"]

POLICY_TITLE = "Identification and Management of Patients at Higher Risk of Morbidity and Mortality"
...
- Rehabilitation, pain and nutrition are governed by the pain, rehabilitation and nutrition policy of {{HOSPITAL_NAME}} (COP.13). Mobilisation as a thrombosis or pressure-ulcer measure is coordinated with that document; it is not rewritten here."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} identifies patients it treats as vulnerable and manages their care as a defined programme, not as an informal extra kindness.
...
PROCEDURE_STEPS = [
"""1. Identifying and managing vulnerable patients
...
This step does not rewrite paediatric abduction measures, intensive-care admission, or facility locks.""",

"""2. Safe and secure care-environment for the vulnerable patient
...
A vulnerable patient is not left in a public corridor, an unlocked bathroom, or an unattended trolley as a holding pattern.""",

"""3. Patients at risk of fall
...
A fall that occurs is recorded, the patient is assessed, and the identification and measures are reviewed. How a fall is reported is [Hospital to define — how a fall is recorded and reported]. This step does not set a numeric fall-rate target.""",

"""4. Patients at risk of pressure ulcers
...
The ulcer, if present, is described in the record (location, stage or the hospital's chosen description method) and reassessed. The description method is [Hospital to define — how a pressure ulcer is described and reassessed in the record]. No staging system is mandated here.""",

"""5. Patients at risk of deep vein thrombosis
...
Contraindications to a chosen measure are recorded rather than silently skipped. This step does not print a dosing table.""",

"""6. Patients who need restraints
...
Devices used, and where they are kept, are [Hospital to define — the restraint devices used and where they are kept]. Devices are not stored in a way that makes informal use the path of least resistance.""",
...
RESPONSIBILITY = """The head of the institution is accountable for {{HOSPITAL_NAME}} identifying and managing patients at higher risk of morbidity and mortality, and for restraint that is last-resort, documented and lawful.
...
All staff are expected to treat an unidentified vulnerable patient who came to harm, a fall that was not recorded, and a restraint without an order or a clock, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Care of Patients chapter, standard COP.12.
...
ABBREVIATIONS = """Abbreviations already defined in the HIC.1 to HIC.6 master policies are not repeated here. A reader using this document on its own should refer to those policies for the shared glossary, including NABH, SHCO, OE, ICU, SOP and PPE.
...
STATUTE_CLAUSE = (
    "the Mental Healthcare Act, 2017, insofar as restraint or seclusion of a person "
    "with mental illness is used"
)
...
OE_MAPPING = [
    {
        ...
        "steps": "Steps 1, 7",
        "evidence": "The written categories of patient this hospital treats as vulnerable, dated, matching the services the hospital actually provides rather than a generic downloaded list, and naming the person who keeps the list current; the identification method (flag, field, band or other) and sample records showing identification at first clinical contact and on change of condition, against the unique identification number, producing a flag that is read rather than a checkbox that is never used; management measures for each category beyond the later risk programmes, with sample records showing the flag changed observations, accompaniment or environment rather than remaining a label; review records that flags are actually applied, including enquiry when a period of admissions shows nobody flagged or when everybody is flagged; induction or briefing records showing ward staff have been shown the categories and the method; the distinction recorded that the assessment policy may collect the facts while this programme owns the flag and the plan; the audit sample at step 7 of flags that match the written categories and that have a management measure",
        "responsible": "Named person at step 1 keeps the category list and reviews application; treating clinicians and nurses identify and manage; quality or accreditation coordinator audits; head of the institution is accountable for the programme",
    },
    {
        "oe_code": "COP.12.b",
        "requirement": "The organization provides for a safe and secure environment for the vulnerable patient.",
        "steps": "Steps 2, 7",
        "evidence": "The written care-environment measures for vulnerable patients, including call method, accompaniment when leaving the unit, and how a missing vulnerable patient is noticed and who is called; sample records of those measures applied; building security left to facility policies rather than specified here",
        "responsible": "Nursing staff apply the care-environment measures; facility function owns building security; named person at step 1 reviews that the measures exist",
    },
    {
        "oe_code": "COP.12.c",
        "requirement": "The organization identifies and manages patients who are at risk of fall.",
        "steps": "Steps 3, 7",
        "evidence": "The written falls-risk identification method and sample identifications against the unique identification number, repeated on change of condition; the measures applied once at risk; fall reports with assessment after a fall and review of measures; no mandated numeric score or rate target",
        "responsible": "Nurses identify and apply measures; treating clinicians review medicines that increase fall risk; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "COP.12.d",
        "requirement": "The organization identifies and manages patients who are at risk of developing / worsening of pressure ulcers.",
        "steps": "Steps 4, 7",
        "evidence": "The written pressure-ulcer risk identification method, including patients who already have an ulcer; measures for prevention of developing or worsening, including skin inspection and repositioning as this hospital defined them; ulcer description and reassessment in the record",
        "responsible": "Nurses identify, inspect, reposition and record; treating clinicians manage existing ulcers; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "COP.12.e",
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 2, printed page 68
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="cop12_draft.json",
        sql_name="cop12_insert.sql",
    )
