# -*- coding: utf-8 -*-
"""Insert AAC.2–AAC.8 master-policy drafts into live Supabase. Never approves.

Owner confirmation for this write (2026-08-17): insert all seven rows, leave
status = 'draft' on every one, do not approve, do not generate test docs. The
owner will review the live drafts together and approve individually later.

Uses the same field-by-field hash verification as apply_aac1_supabase.py
(every INSERT column except status, deep-equal for arrays/jsonb, LF/CRLF
report). After each insert, asserts the live row's status is still 'draft'.
...
Usage:
  python3 policies/build/apply_aac_drafts_supabase.py --insert
  python3 policies/build/apply_aac_drafts_supabase.py --verify-only
  python3 policies/build/apply_aac_drafts_supabase.py --dry-run
"""
import argparse
import os
import sys
...
# Reuse AAC.1's PostgREST helpers and the exact verify loop so the per-field
# output is comparable line-for-line with the AAC.1 insert log.
...
CODES = ["AAC.2", "AAC.3", "AAC.4", "AAC.5", "AAC.6", "AAC.7", "AAC.8"]
_POLICIES = _HERE.parent


def draft_path_for(code):
    n = code.split(".", 1)[1]
    return _POLICIES / "drafts" / f"aac{n}_draft.json"


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--insert", action="store_true",
                    help="POST each missing AAC.2–AAC.8 row (status stays draft)")
    ap.add_argument("--verify-only", action="store_true",
                    help="only verify live rows against local drafts")
    ap.add_argument("--dry-run", action="store_true",
                    help="no network: validate all 7 drafts and print the plan")
    args = ap.parse_args()
    ...
    for code in CODES:
        ...
        print(f"draft OK: {path.name}, {len(draft['procedure_steps'])} steps, "
              ...
              f"status={draft['status']!r}")

    if args.dry_run:
        print("\nDRY RUN -- no network calls. Plan for each of AAC.2–AAC.8:")
    ...
    for code, path, draft in drafts:
        ...
        if not ok:
            ...
            print(f"verification content matched but live status is {live_status!r} "
                  f"-- expected 'draft'. This script did not approve it.")
        else:
