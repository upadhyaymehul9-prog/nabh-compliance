# -*- coding: utf-8 -*-
"""Builds the ROM.1 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): ROM.1.a, ROM.1.b, ROM.1.e
  Tier 2 (lighter pass):   ROM.1.c, ROM.1.d

THREE of five OEs are asterisked. The draft builds deep blocks for a, b and e.
c and d are lean.

Official source: NABH Standards for Small Healthcare Organisations, 3rd Edition
(August 2022), Chapter 7 Responsibilities of Management, standard ROM.1 and
OEs ROM.1.a-e, printed page 110, PDF page index 116. Chapter intent and
summary: printed page 109, PDF page index 115. References: printed pages
112-114, PDF indices 118-120.

Asterisks verified 2026-08-17 against that page and scripts/shco_oe_asterisks.json
(owner-cleared ROM/FMS/HRM/IMS audit). ROM.1.a, ROM.1.b and ROM.1.e carry the
asterisk.

No named Act of Parliament is a numbered ROM chapter reference. Chapter
reference 23 is India Code as a lookup. Legal form of the organisation is
[Hospital to define], not a pasted Companies Act or Societies Registration Act.
"""
...
TIER1_OES = ["ROM.1.a", "ROM.1.b", "ROM.1.e"]

POLICY_TITLE = "Identification of Those Responsible for Governance"
...
This document is the identification of governance and the acts those persons perform under ROM.1. It is not day-to-day leadership (ROM.2), not approval of plans and budget (ROM.3.a), and not management's risk-management duty (ROM.4)."""

SCOPE = """This policy applies to those responsible for governance at {{HOSPITAL_NAME}} and to the people who record, publish and support the acts this document requires.
...
- MSME registration, if any, is not this standard."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} identifies those responsible for governance and documents their roles and responsibilities.
...
{{HOSPITAL_NAME}} does not treat an unnamed owner, a vision poster with no governing body behind it, or a copied Companies Act board charter in a hospital that is not a company, as this standard."""

PROCEDURE_STEPS = [
"""1. Those responsible for governance identified; roles and responsibilities defined and documented
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when the governing entity changed, or when ROM.2–4, PSQ.4 or PRE.1 that this document hands work to are revised.""",
]

RESPONSIBILITY = """The head of the institution is accountable for {{HOSPITAL_NAME}} having an identified governing entity under this document, and operates day-to-day under ROM.2.
...
All staff are expected to treat an unnamed governing entity, a mission statement the board never adopted, and a pasted Companies Act charter that does not match this hospital's legal form, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Chapter 7 ROM, standard ROM.1.
...
- Internal documents of {{HOSPITAL_NAME}}: the constitutive instrument; the documented roles of those responsible for governance; vision, mission and values; the ethical management framework; ROM.2–4; PSQ.4; PRE.1–3."""

DISTRIBUTION = """Controlled master copy: office of the head of the institution, {{HOSPITAL_NAME}}, with those responsible for governance and the quality or accreditation coordinator.
...
OE_MAPPING = [
    ...
    },
    {
        ...
        "steps": "Steps 3, 2, 6",
        "evidence": "The written method and sample records of governance monitoring against the stated mission; the recorded split that ROM.3.d, PSQ.2 and ROM.2.d may supply data and do not replace this OE; the audit sample at step 6",
        "responsible": "Those responsible for governance monitor; ROM.3/PSQ.2/ROM.2 remain those documents; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "ROM.1.d",
        "requirement": "Those responsible for governance appoint the senior leaders in the organisation.",
        "steps": "Steps 4, 6",
        "evidence": "The written list of senior-leader posts and sample appointment records as a governance act; the recorded split that HRM files the appointment and ROM.2 owns qualifications of the head; the audit sample at step 6",
        "responsible": "Those responsible for governance appoint; HRM files; ROM.2 owns the head's qualifications; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "ROM.1.e",
        "requirement": "Those responsible for governance support the ethical management framework of the organisation.",
        "steps": "Steps 5, 6",
        "evidence": "The written ethical management framework showing what it covers as organisational ethics rather than a pasted NMC code, MHCA 2017, CPA 2019 or CEA 2010; evidence of governance support (adoption, endorsement, resource or review by the entity at step 1) rather than a quality officer's private file; the recorded splits that PRE.1/PRE.2 own patient rights, PRE.3 owns consent method, and PSQ.4 owns culture of safety as a patient-safety programme requirement; the recorded use of Chatterjee and Srinivasan, Biller-Andorno and Bruning (chapter references 9, 6 and 8) as frameworks not pasted codes; induction or briefing of the governing entity on the framework; the location of the framework; the audit sample at step 6 of support that is an act of governance",
        "responsible": "Those responsible for governance support the framework; PRE and PSQ.4 remain those documents; quality or accreditation coordinator audits",
    },
]
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 7 ROM, printed page 110
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="rom1_draft.json",
        sql_name="rom1_insert.sql",
    )
