# -*- coding: utf-8 -*-
"""Parse official portal FMS chapter OCR into inventory JSON.

Authoritative source: policies/source/hco6_fms_ocr.txt (pdftotext of portal
Standards PDF pages 151–161 / printed 140–149). Does not use the Guidebook
for counts/levels/stars.
"""
...
OCR = ROOT / "policies/source/hco6_fms_ocr.txt"
OUT = ROOT / "policies/source/hco6_fms_inventory.json"

LIGATURE_MAP = str.maketrans({"\uf001": "fi", "\uf002": "fl", "\ufb01": "fi", "\ufb02": "fl"})
LETTER_RE = re.compile(
    r"^\s+(CORE|Commitment|Achievement|Excellence)\s+([a-lA-L])\.\s+(.*)$"
)
STD_RE = re.compile(r"^\s+FMS\.(\d+)\.\s*(.*)$")
...
def parse() -> dict:
    ...
    for line in body.splitlines():
        ...
        if mstd and (in_title or stripped.startswith("FMS.")):
            flush_oe()
            n = mstd.group(1)
        ...
        if in_title:
            ...
            if LETTER_RE.match(line):
                continue
            if stripped and not is_footer(stripped) and not stripped.startswith("FMS."):
...
def parse_summary_matrix(text: str) -> dict[str, dict[str, str]]:
    """Levels from the summary-of-OEs matrix (printed page 141). Body still wins."""
    # Rows a–h; columns FMS.1–7
    levels: dict[str, dict[str, str]] = {str(n): {} for n in range(1, 8)}
    # Not fully parsed by regex here; used as a manual check in main().
    return levels
