# -*- coding: utf-8 -*-
"""Builds the COP.9 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  Tier 1 (full treatment): COP.9.a only
  Tier 2 (lighter pass):   COP.9.b, COP.9.c, COP.9.d, COP.9.e
...
(August 2022), Chapter 2, standard COP.9 and OEs COP.9.a-e, read from the official
standards PDF (downloaded 2026-08-17 from the NABH website's Explore NABH Standards
page), printed page 66, PDF page index 72.
...
committed scripts/shco_oe_asterisks.json on all 408 entries) and the COP.9 page
read directly. COP.9.a carries the asterisk; COP.9.b-e are unasterisked.
"""
...
This document is not the anaesthesia policy and it is not the surgical-procedure policy. Procedural sedation is a distinct service. When sedation becomes general anaesthesia, care moves to the anaesthesia policy of {{HOSPITAL_NAME}}."""
...
- Cardio-pulmonary resuscitation, if it is required during sedation, is governed by the resuscitation policy of {{HOSPITAL_NAME}} (COP.3). This policy requires that resuscitation equipment and competent help are available where sedation is given; it does not write the resuscitation algorithm."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} administers procedural sedation in a consistent manner, against written guidance that is the same in every location where sedation is given. A location that cannot meet that guidance does not sedate.
...
{{HOSPITAL_NAME}} treats sedation that has become general anaesthesia as anaesthesia, and hands that patient to the anaesthesia policy."""

PROCEDURE_STEPS = [
"""1. Procedural sedation administered in a consistent manner
...
The written procedural-sedation guidance, including locations, depths, roles, room list and the rule for unplanned deepening, is held at [Hospital to define — where the written procedural-sedation guidance is held].""",

"""2. Informed consent for administration of procedural sedation
...
When the patient is a child, consent is obtained from the person entitled to consent under the patient-rights policies; age-specific care remains under the paediatric policy of {{HOSPITAL_NAME}}. When the patient cannot consent, the patient-rights policies govern the substitute. This step still requires that the record shows sedation consent before the drug.""",

"""3. Competent and trained persons perform and monitor sedation
...
Training required beyond professional registration, and how it is recorded, is [Hospital to define — the training required to perform and to monitor procedural sedation, and how it is recorded]. A person without that training is not assigned.""",

"""4. Intra-procedure monitoring
...
If a parameter cannot be obtained, the reason is recorded and the sedation does not continue as if the parameter were present, unless a documented clinical reason is recorded by the person administering sedation.""",

"""5. Post-procedure monitoring and objective discharge from recovery
...
A patient discharged from recovery who is not being admitted is still a patient of {{HOSPITAL_NAME}} until they leave the organisation under the discharge policy where that policy applies to the setting. This step owns only discharge from the recovery area.""",
...
RESPONSIBILITY = """The head of the institution is accountable for {{HOSPITAL_NAME}} administering procedural sedation only against written guidance, only in named locations, and only by persons who are registered and competent.
...
All staff are expected to treat sedation in an unnamed location, a missing monitor, consent after the drug, and discharge from recovery of a patient who does not meet the criteria, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Care of Patients chapter, standard COP.9.
...
OE_MAPPING = [
    ...
    },
    {
        ...
        "steps": "Steps 4, 6",
        "evidence": "The written recording interval, the scale used for level of sedation, and where observations are written; sample intra-procedure records showing all six parameters present (heart rate, cardiac rhythm, respiratory rate, blood pressure, oxygen saturation, level of sedation); records of a parameter that could not be obtained, with the reason and the decision; the audit sample at step 6 of all six parameters recorded",
        "responsible": "Person monitoring records the six parameters; person administering sedation does not continue as if a missing parameter were present without a documented reason; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "COP.9.e",
        ...
        "steps": "Steps 5, 6",
        "evidence": "The written post-procedure observations and objective criteria for discharge from the recovery area; the named person who judges discharge; sample recovery records showing observations documented and discharge against those criteria rather than against the theatre list; the recorded distinction that this step owns discharge from recovery, not discharge from the organisation",
        "responsible": "Person judging recovery discharge applies the objective criteria and records the judgement; named sedation lead holds the criteria; quality or accreditation coordinator audits",
    },
]
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 2, printed page 66
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="cop9_draft.json",
        sql_name="cop9_insert.sql",
    )
