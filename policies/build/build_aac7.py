# -*- coding: utf-8 -*-
"""Builds the AAC.7 master policy draft: JSON for review + SQL for later insert.
...
Tier is decided by doc_required / the asterisk in the official PDF:
  ...
  Tier 2 (lighter pass):   AAC.7.a, AAC.7.b, AAC.7.c, AAC.7.d.
...
(August 2022), Chapter 1, standard AAC.7 and OEs AAC.7.a-d, read from the official
standards PDF (downloaded 2026-08-17 from the NABH website's Explore NABH Standards
page), printed pages 53-54, PDF page index 59-60.
...
committed scripts/shco_oe_asterisks.json on all 408 entries) and the AAC.7 pages
read directly. ZERO OEs of AAC.7 carry the asterisk.
"""
...
POLICY_TITLE = "Continuity of Care, Handover and Internal Transfer"
...
A patient who changes shift, unit or clinician without a named person, without the information the next team needs, or without a safe move between units, is a patient whose care has stopped being continuous. This document is the process that prevents that."""

SCOPE = """This policy applies to every location in which a patient of {{HOSPITAL_NAME}} is under care and from which that patient may move to another location inside this hospital: the emergency area, every in-patient ward, day-care, the operation theatre and recovery, intensive or high-dependency areas where they exist, and any diagnostic or procedural area that receives a patient from a unit. It binds the individual identified as responsible for the patient's care, the medical, nursing and other care-providers who share information and make internal referrals, the staff who give and receive handover, and the staff who accompany an internal transfer.
...
- Initial assessment and the care plan are governed by the assessment policy of {{HOSPITAL_NAME}}. Discharge from the organisation is governed by the discharge policy of {{HOSPITAL_NAME}}."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} identifies a qualified individual as responsible for the patient's care during every phase of that care. A phase without a named person is a break in continuity.
...
{{HOSPITAL_NAME}} treats an unidentified responsible person, an undocumented handover, or an unaccompanied internal move as a defect, not as a shortcut taken under pressure."""

PROCEDURE_STEPS = [
"""1. Named responsible individual at every phase
...
This is the per-patient named individual. It is not the AAC.1.b requirement that a defined service be backed by suitably qualified personnel.""",

"""2. Sharing information among providers, including internal referral
...
The medical record that holds this information is governed by the information-management policies of {{HOSPITAL_NAME}}. This step owns that the information is communicated, not the design of the record.""",

"""3. Standardised handover at shift, between shifts, and between units
...
The transmission-based precaution category, when one is in force, appears in the nursing handover because HIC.2 requires that content. This step owns the method into which that content is placed. Personal protective equipment, isolation practice and the precaution assignment register remain in HIC.2 and are not restated here.""",

"""4. Safe internal transfer
...
The transfer is recorded: time of leaving, time of arrival, who accompanied, and condition on arrival. A patient is not sent to find the receiving unit, and is not moved through public areas without the accompaniment and monitoring this step requires.""",
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when a handover or internal-transfer incident exposes a gap, or when the registration-and-transfer, infection-control or information-management policies that this document hands work to are revised."""
]

RESPONSIBILITY = """The head of the institution is accountable for {{HOSPITAL_NAME}} identifying a responsible individual at every phase of care, for a working standardised handover, and for internal transfer that is performed as a clinical act rather than as a porterage.
...
All staff are expected to report a patient who had no named responsible individual, a handover that was unstructured, or an internal move that was improvised rather than performed under this policy."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Access, Assessment and Continuity of Care chapter, standard AAC.7.
...
STATUTE_CLAUSE = (
    "the professional registration statutes that govern who may be identified as responsible "
    "for a patient's care — the National Medical Commission Act, 2019 and State Medical Council "
    "registration, the Indian Nursing Council Act, 1947 and State Nursing Council registration, "
    "and the corresponding councils for the other professions {{HOSPITAL_NAME}} employs — and "
    "the Clinical Establishments (Registration and Regulation) Act, 2010 and the rules under it, "
    "where adopted by the State, or the corresponding State law"
)
...
OE_MAPPING = [
    {
        "oe_code": "AAC.7.a",
        "requirement": "During all phases of care, a qualified individual is identified as responsible for the patient's care",
        "steps": "Steps 1, 5",
        ...
        "responsible": "The named clinician remains responsible until another named individual has taken over; human-resource procedures verify professional registration; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "AAC.7.b",
        ...
        "steps": "Steps 2, 5",
        "evidence": "The written method for sharing clinical information and recording an internal referral; sample internal referrals showing reason, information sent (working diagnosis, findings, treatment and response, pending investigations) and receiving-department response; the audit sample at step 5 of internal referrals that show what was shared",
        "responsible": "Medical, nursing and other care-providers share information and record internal referrals; information-management policies own the record that holds it; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "AAC.7.c",
        "requirement": "Standardised hand-over communication is implemented during each staffing shift, between shifts and during transfers between units or departments",
        "steps": "Steps 3, 4, 5",
        "evidence": "The written structured handover method used at every shift and at every unit-to-unit transfer, naming the method chosen and not treating any one published tool as mandatory; sample handover records with receiving-staff confirmation, spanning shift handover and unit-to-unit handover; records showing the HIC.2 precaution category appears in the nursing handover when one is in force, without this document restating PPE or isolation; the audit sample at step 5 of handover that used the structured method",
        "responsible": "Nursing staff give and receive handover; sending and receiving units complete unit-to-unit handover before internal transfer; HIC.2 owns infection-control content of the handover; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "AAC.7.d",
        "requirement": "Patient transfer within the organisation is done safely",
        "steps": "Steps 4, 5",
        "evidence": "The written internal-transfer checklist and the written rule on who accompanies, by patient condition; sample internal-transfer records showing receiving-unit acceptance, named responsible individuals, handover, completed checklist, accompaniment, monitoring en route, time of leaving, time of arrival and condition on arrival; records distinguishable from AAC.2 between-organisation transfer; the audit sample at step 5 of internal transfers",
        "responsible": "Sending and receiving clinicians remain responsible until the receiving individual has accepted the patient; accompanying staff complete the checklist and monitor en route; quality or accreditation coordinator audits",
    },
]
...
SQL_HEADER = """-- Source: NABH SHCO Standards 3rd Edition (August 2022), Chapter 1, printed pages 53-54
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="aac7_draft.json",
        sql_name="aac7_insert.sql",
    )
