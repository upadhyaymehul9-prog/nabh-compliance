# -*- coding: utf-8 -*-
"""Builds the ROM.2 master policy draft: JSON for review + SQL for later insert.
...
NO OE CARRIES THE ASTERISK. The whole standard is Tier 2. ROM.2.c is Core and
still unasterisked — Core is not the tier trigger.

Official source: NABH SHCO 3rd Edition PDF, Chapter 7, ROM.2, printed page 110
(PDF page index 116). Intent printed page 109: the organisation is headed by a
leader responsible for operating it day-to-day; management executes its
responsibility for compliance with all applicable regulations.

Asterisks verified 2026-08-17: none of ROM.2.a-d is asterisked.

No named Act of Parliament is a numbered ROM chapter reference. Chapter
reference 23 is India Code as a lookup for the hospital's own applicable-
legislation register. Do not dump Companies Act / Societies / CEA / CPA / MHCA
as a checklist.
"""
...
POLICY_TITLE = "The Organisation's Leader"
...
This document is the day-to-day head. It is not the identification of those responsible for governance (ROM.1), not approval of the annual budget (ROM.3.a), and not management's organisation-wide risk duty (ROM.4)."""

SCOPE = """This policy applies to the person heading {{HOSPITAL_NAME}} and to those who appoint and review that person.
...
- India Code (chapter reference 23) is the lookup for central and state Acts. It is not itself a statute dumped into paragraph 2. The applicable list is stated at step 3 from this hospital's legal form and services. Companies Act, 2013, Societies Registration Act, 1860, CEA 2010, CPA 2019, MHCA 2017 and MSME registration are not a checklist."""

POLICY_STATEMENT = """{{HOSPITAL_NAME}} is headed by a leader who is responsible for operating the organisation on a day-to-day basis.
...
This policy is reviewed at [Hospital to define — the review interval for this policy], and sooner when the head changes, or when ROM.1, ROM.3, ROM.4 or AAC.1 that this document hands work to are revised.""",
]

RESPONSIBILITY = """Those responsible for governance (ROM.1) appoint the head and review that person's performance.
...
All staff are expected to treat a letterhead head with no recorded qualifications, and a copied Act list that does not apply here, as defects, and to report them."""

REFERENCES = """- National Accreditation Board for Hospitals and Healthcare Providers (NABH), Standards for Small Healthcare Organisations, 3rd Edition — Chapter 7 ROM, standard ROM.2.
...
- Internal documents of {{HOSPITAL_NAME}}: the head's appointment; qualification and experience records; the applicable-legislation register; ROM.1, ROM.3, ROM.4; AAC.1."""
...
OE_MAPPING = [
    {
        ...
        "steps": "Steps 1, 5",
        "evidence": "The written statement of requisite administrative qualifications and the current head's evidence; the recorded split that HRM files certificates; the audit sample at step 5",
        "responsible": "Those responsible for governance appoint a qualified head; HRM files; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "ROM.2.b",
        "requirement": "The person heading the organisation has requisite and appropriate administrative experience.",
        "steps": "Steps 2, 5",
        "evidence": "The written statement of requisite administrative experience and the current head's evidence; the recorded split that this is not step 1 counted twice; the audit sample at step 5",
        "responsible": "Those responsible for governance appoint an experienced head; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "ROM.2.c",
        ...
        "steps": "Steps 3, 5",
        "evidence": "The current applicable-legislation register matching this hospital's legal form and AAC.1 services, using India Code (chapter reference 23) as lookup rather than a pasted central-Act dump; sample compliance evidence; the recorded refusal to dump Companies Act, Societies Registration Act, CEA, CPA or MHCA as a checklist; the recorded split that HIC.3/MOM.8/AAC.5 (and other owning documents) keep their own statutes; the audit sample at step 5",
        "responsible": "The head is responsible for the applicable set; owning clinical/IPC/medication documents keep their statutes; quality or accreditation coordinator audits",
    },
    {
        "oe_code": "ROM.2.d",
        "requirement": "The performance of the organisation's leader is reviewed for effectiveness.",
        "steps": "Steps 4, 5",
        "evidence": "The written review method and sample reviews; the recorded split that ROM.1.c is organisation-against-mission, not this review of the leader; the audit sample at step 5",
        "responsible": "Those responsible for governance review the head; quality or accreditation coordinator audits",
    },
]
...
The values the hospital must supply: qualifications; experience; the applicable-legislation register and compliance evidence; leader-performance review method; the audit interval; the review interval; the intranet or folder location; and any additional local abbreviation."""
...
if __name__ == "__main__":
    emit_and_verify(
        ...
        json_name="rom2_draft.json",
        sql_name="rom2_insert.sql",
    )
