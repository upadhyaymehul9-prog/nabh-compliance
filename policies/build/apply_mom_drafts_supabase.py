# -*- coding: utf-8 -*-
"""Insert MOM.1–MOM.9 master-policy drafts into live Supabase. Never approves.

Owner confirmation for this write (2026-08-17): insert all nine rows, leave
status = 'draft' on every one, do not approve, do not generate test docs.
MOM.5 and MOM.8 were owner-approved as drafted (MOM.8 after the distinct
cytotoxic block and chemotherapy-safety citations). Approval of the live
row status is a later, separate act.
...
Usage:
  python3 policies/build/apply_mom_drafts_supabase.py --insert
  python3 policies/build/apply_mom_drafts_supabase.py --verify-only
  python3 policies/build/apply_mom_drafts_supabase.py --dry-run
"""
import argparse
import os
import sys
...
CODES = [f"MOM.{n}" for n in range(1, 10)]
_POLICIES = _HERE.parent


def draft_path_for(code):
    n = code.split(".", 1)[1]
    return _POLICIES / "drafts" / f"mom{n}_draft.json"


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--insert", action="store_true",
                    help="POST each missing MOM.1–MOM.9 row (status stays draft)")
    ap.add_argument("--verify-only", action="store_true",
                    help="only verify live rows against local drafts")
    ap.add_argument("--dry-run", action="store_true",
                    help="no network: validate all 9 drafts and print the plan")
    args = ap.parse_args()
    ...
    if args.dry_run:
        print("\nDRY RUN -- no network calls. Plan for each of MOM.1–MOM.9:")
